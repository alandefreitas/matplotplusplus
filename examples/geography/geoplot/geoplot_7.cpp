...
class americas_trip_solver {
   public:
    americas_trip_solver(const vector<double>& lat, const vector<double>& lon, const vector<string>& names,
                         axes_handle ax);
...
americas_trip_solver::americas_trip_solver(const vector<double>& lat, const vector<double>& lon,
...
void americas_trip_solver::setup_starting_point(size_t iteration) {
    ...
    if (iteration < names_.size()) {
        auto [lon_ignore, lat_ignore, tour] = greedy_tsp_with_idx(lon_, lat_, iteration);
        curr_tour_ = tour;
...
vector<vector<size_t>> americas_trip_solver::get_neighbors(const std::vector<size_t>& tour) {
...
double americas_trip_solver::tour_distance(const vector<size_t>& tour) {
    double sum = 0.;
    for (size_t i = 0; i < tour.size() - 1; ++i) {
        sum += distance(lon_[tour[i]], lat_[tour[i]], lon_[tour[i + 1]], lat_[tour[i + 1]]);
    }
    sum += distance(lon_[tour[tour.size() - 1]], lat_[tour[tour.size() - 1]], lon_[tour[0]], lat_[tour[0]]);
    return sum;
}
...
void americas_trip_solver::draw_if_improvement() {
    static auto last_draw = chrono::high_resolution_clock::now() - chrono::seconds(1);
...
int main() {
    vector<string> names = {
        "Marigot",    "The Valley",     "Saint John's",    "Buenos Aires",  "Oranjestad",     "Nassau",
        "Bridgetown", "Belmopan",       "Hamilton",        "La Paz",        "Brasilia",       "Road Town",
        "Ottawa",     "George Town",    "Santiago",        "Bogota",        "San Jose",       "Havana",
        "Willemstad", "Roseau",         "Santo Domingo",   "Quito",         "San Salvador",   "Stanley",
        "Nuuk",       "Saint George's", "Guatemala City",  "Georgetown",    "Port-au-Prince", "Tegucigalpa",
        "Kingston",   "Mexico City",    "Plymouth",        "Managua",       "Panama City",    "Asuncion",
        "Lima",       "San Juan",       "Gustavia",        "Basseterre",    "Castries",       "Saint-Pierre",
        "Kingstown",  "Philipsburg",    "Paramaribo",      "Port of Spain", "Grand Turk",     "Washington",
        "Montevideo", "Caracas",        "Charlotte Amalie"};

    vector<double> lat = {18.0731, 18.2166,  17.1166, -34.5833, 12.5166, 25.0833, 13.1,    17.25,   32.2833,
                          -16.5,   -15.7833, 18.4166, 45.4166,  19.3,    -33.45,  4.6,     9.9333,  23.1166,
                          12.1,    15.3,     18.4666, -0.2166,  13.7,    -51.7,   64.1833, 12.05,   14.6166,
                          6.8,     18.5333,  14.1,    18,       19.4333, 16.7,    12.1333, 8.9666,  -25.2666,
                          -12.05,  18.4666,  17.8833, 17.3,     14,      46.7666, 13.1333, 18.0166, 5.8333,
                          10.65,   21.4666,  38.8833, -34.85,   10.4833, 18.35};

    vector<double> lon = {-63.0822, -63.0500, -61.8500, -58.6666, -70.0333, -77.3500, -59.6166, -88.7666, -64.7833,
                          -68.1500, -47.9166, -64.6166, -75.7000, -81.3833, -70.6666, -74.0833, -84.0833, -82.3500,
                          -68.9166, -61.4000, -69.9000, -78.5000, -89.2000, -57.8500, -51.7500, -61.7500, -90.5166,
                          -58.1500, -72.3333, -87.2166, -76.8000, -99.1333, -62.2166, -86.2500, -79.5333, -57.6666,
                          -77.0500, -66.1166, -62.8500, -62.7166, -61.0000, -56.1833, -61.2166, -63.0333, -55.1666,
                          -61.5166, -71.1333, -77.0000, -56.1666, -66.8666, -64.9333};
