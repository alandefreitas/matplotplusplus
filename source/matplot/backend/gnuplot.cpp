...
bool gnuplot::output(const std::string &filename, const std::string &format) {
  ...
  constexpr auto exts = extension_terminal();
  auto it = std::find_if(exts.begin(), exts.end(),
                         [&](const auto &e) { return e.second == format; });
...
void gnuplot::width(unsigned int new_width) {
  ...
  if (terminal_has_size_option(terminal_)) {
    run_command("set terminal " + terminal_ + " size " + num2str(position_[2]) +
                "," + num2str(position_[3]));
  }
}
...
void gnuplot::new_frame() {
  if (!consumes_gnuplot_commands()) {
    throw std::logic_error(
        "There is no function to start new_frame in the gnuplot backend yet");
  } else {
    throw std::logic_error(
...
bool gnuplot::supports_fonts() { return terminal_has_font_option(terminal_); }

void gnuplot::run_command(const std::string &command) {
  if (!pipe_) {
    return;
  }
  size_t pipe_capacity = (pipe_->_bf._base != nullptr)
                             ? pipe_->_bf._size
                             : pipe_capacity_worst_case;
...
std::string gnuplot::default_terminal_type() {
  ...
  if (dont_know_term_type) {
    ...
    terminal_type = std::regex_replace(
        terminal_type, std::regex("[^]*terminal type is ([^ ]+)[^]*"), "$1");
...
std::pair<int, int> gnuplot::gnuplot_version() {
  ...
  if (dont_know_gnuplot_version) {
    ...
    std::string version_major = std::regex_replace(
        version_str, std::regex("[^]*gnuplot (\\d+)\\.\\d+ patchlevel \\d+ *"),
        "$1");
...
bool gnuplot::terminal_has_title_option(const std::string &t) {
  constexpr std::string_view whitelist[] = {"qt",      "aqua", "caca", "canvas",
                                            "windows", "wxt",  "x11"};
...
bool gnuplot::terminal_has_size_option(const std::string &t) {
  ...
  constexpr std::string_view whitelist[] = {
      "qt",  "aqua", "caca",    "canvas",   "eepic",   "emf", "gif", "jpeg",
      "pbm", "png",  "sixelgd", "tkcanvas", "windows", "wxt", "svg"};
  return std::find(std::begin(whitelist), std::end(whitelist), t) !=
         std::end(whitelist);
}
...
bool gnuplot::terminal_has_enhanced_option(const std::string &t) {
  constexpr std::string_view whitelist[] = {
      "canvas",  "postscript", "qt",       "aqua",       "caca",    "canvas",
      "dumb",    "emf",        "enhanced", "jpeg",       "pdf",     "pdfcairo",
      "pm",      "png",        "pngcairo", "postscript", "sixelgd", "tkcanvas",
      "windows", "wxt",        "x11",      "ext",        "wxt"};
  return std::find(std::begin(whitelist), std::end(whitelist), t) !=
         std::end(whitelist);
}
...
bool gnuplot::terminal_has_color_option(const std::string &t) {
  constexpr std::string_view whitelist[] = {
      "postscript", "aifm",    "caca",     "cairolatex", "context",  "corel",
      "eepic",      "emf",     "epscairo", "epslatex",   "fig",      "lua tikz",
      "mif",        "mp",      "pbm",      "pdf",        "pdfcairo", "pngcairo",
      "postscript", "pslatex", "pstex",    "tgif",       "windows"};
  return std::find(std::begin(whitelist), std::end(whitelist), t) !=
         std::end(whitelist);
}
...
bool gnuplot::terminal_has_font_option(const std::string &t) {
  ...
  constexpr std::string_view blacklist[] = {
      "dxf",      "eepic",    "emtex",    "hpgl",     "latex",   "mf",
      "pcl5",     "pslatex",  "pstex",    "pstricks", "qms",     "tek40xx",
      "tek410x",  "texdraw",  "tkcanvas", "vttek",    "xterm",   "jpeg",
      "dumb",     "tpic",     "sixelgd",  "png",      "lua",     "jpeg",
      "epscairo", "epslatex", "canvas",   "dumb",     "dxy800a", "emxvga",
      "pdfcairo", "cgi",      "gif",      "gpic",     "grass",   "hp2623a",
      "hp2648",   "hp500c",   "hpgl",     "pcl5",     "hpljii",  "hppj",
      "imagen",   "linux",    "lua",      "lua tikz", "mf",      "mif",
      "mp",       "pbm",      "pm",       "qms",      "regis",   "svga",
      "texdraw",  "tikz",     "tpic",     "vgagl",    "vws",     "pdf"};
