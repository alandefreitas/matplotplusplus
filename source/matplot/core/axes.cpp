        ...
        if (max_colors_ == 0) {
            ...
            for (size_t i = 0; i < colormap_.size(); ++i) {
                if (colormap_[i].size() == 4) {
                    ...
                    ss << "    " << i << "   " << colormap_[i][0] << " " << colormap_[i][1] << " " << colormap_[i][2];
                }
...
void axes::run_position_margin_command() {
    ...
    run_command("set tmargin at screen " + num2str(y + height() - height() * tmargin_multiplier - colorbar_tmargin));
...
void axes::run_grid_command() {
    ...
    // set grid xtics ytics mxtics mytics layerdefault linecolor 'blue' linetype 1
    // linewidth 2, linecolor 'red'
    if (x_grid_ || x_minor_grid_ || y_grid_ || y_minor_grid_ || z_grid_ || z_minor_grid_ || r_grid_ || r_minor_grid_) {
...
void axes::run_box_command() {
    ...
    if (!box_ || is_polar()) {
        ...
        if (is_3d()) {
            ...
            run_command("set border 15 linecolor \"" + to_string(box_color_) + "\" linewidth " + num2str(line_width()));
        }
    }
}
    ...
    auto set_or_unset_axis = [this](class axis &ax, std::string axis_name, bool minor_ticks = false) {
        ...
        if (ax.visible() && !ax.tick_values().empty()) {
            ...
            if (ax.tick_values_manual() || ax.tick_values().empty()) {
                run_command("set " + axis_name + "tics" + ax.tick_values_string(minor_ticks) + ax.tick_rotate_string());
            } else {
                if (ax.tickangle() != 0.) {
...
void axes::run_legend_command() {
    ...
    if (legend_ == nullptr || !legend_->visible()) {
        ...
        if (!legends_to_plot) {
            ...
            if (parent_->backend_->supports_fonts()) {
                cmd += " font \"" + legend_->font_name() + "," + num2str(legend_->font_size()) + "\" textcolor rgb '" +
            ...
            if (!legend_->title().empty()) {
                ...
                cmd += "\"";
                if (parent_->backend_->supports_fonts()) {
                    cmd += " font \"" + escape(legend_->font_name()) + "," + num2str(unsigned(legend_->font_size())) +
                           "\"";
...
void axes::run_background_command() {
    ...
    if (visible_) {
        ...
        if (color_ != parent()->color()) {
            if (is_2d() || is_3d_map()) {
                run_command(
                    "set object 2 rectangle from graph 0,0 to graph 1,1 behind "
                    "fillcolor rgb \"" +
         ...
         * std::to_string(x_origin()) + "," + std::to_string(y_origin()) + " to
         * screen " + std::to_string(x_origin() + width()) + "," +
         *              std::to_string(y_origin() + height()) + " behind fillcolor
         * rgb \"" + to_string(color_outside_) + "\" fillstyle solid 1.0 noborder");
         * }
         */
    }
}
...
void axes::run_command(const std::string &command) { parent_->run_command("    " + command); }

void axes::include_comment(const std::string &command) { parent_->run_command("    # " + command); }

bool axes::is_3d() { return children_.empty() ? false : children_[0]->is_3d(); }

bool axes::is_3d_map() { return children_.empty() ? false : children_[0]->is_3d_map(); }

bool axes::is_2d() { return children_.empty() ? true : children_[0]->is_2d(); }

bool axes::is_polar() { return children_.empty() ? false : children_[0]->is_polar(); }
...
std::array<double, 4> axes::child_limits() {
    ...
    auto xminit = std::min_element(children_.begin(), children_.end(),
                                   [](axes_object_handle a, axes_object_handle b) { return a->xmin() < b->xmin(); });
    auto xmaxit = std::max_element(children_.begin(), children_.end(),
                                   [](axes_object_handle a, axes_object_handle b) { return a->xmax() < b->xmax(); });
    auto yminit = std::min_element(children_.begin(), children_.end(),
                                   [](axes_object_handle a, axes_object_handle b) { return a->ymin() < b->ymin(); });
    auto ymaxit = std::max_element(children_.begin(), children_.end(),
                                   [](axes_object_handle a, axes_object_handle b) { return a->ymax() < b->ymax(); });
...
axes::axes(figure_handle parent, std::array<float, 4> position) : axes(parent.get(), position) {
    t_axis_.tick_label_format_ = "%gº";
    t_axis_.tick_values_ = iota(0, 30, 330);
    t_axis_.tick_values_automatic_ = false;
    font_ = parent->font();
...
const std::string &axes::xtickformat() const { return x_axis_.tick_label_format(); }

void axes::xtickformat(const std::string &str) { x_axis_.tick_label_format(str); }

const std::string &axes::x2tickformat() const { return x2_axis_.tick_label_format(); }

void axes::x2tickformat(const std::string &str) { x2_axis_.tick_label_format(str); }

const std::string &axes::ytickformat() const { return y_axis_.tick_label_format(); }

void axes::ytickformat(const std::string &str) { y_axis_.tick_label_format(str); }

const std::string &axes::y2tickformat() const { return y2_axis_.tick_label_format(); }

void axes::y2tickformat(const std::string &str) { y2_axis_.tick_label_format(str); }

const std::string &axes::ztickformat() const { return z_axis_.tick_label_format(); }

void axes::ztickformat(const std::string &str) { z_axis_.tick_label_format(str); }

const std::string &axes::rtickformat() const { return r_axis_.tick_label_format(); }

void axes::rtickformat(const std::string &str) { r_axis_.tick_label_format(str); }
...
const std::vector<double> &axes::xticks() const { return x_axis_.tick_values(); }

void axes::xticks(const std::vector<double> &ticks) { x_axis_.tick_values(ticks); }

const std::vector<double> &axes::x2ticks() const { return x2_axis_.tick_values(); }

void axes::x2ticks(const std::vector<double> &ticks) { x2_axis_.tick_values(ticks); }

const std::vector<double> &axes::yticks() const { return y_axis_.tick_values(); }

void axes::yticks(const std::vector<double> &ticks) { y_axis_.tick_values(ticks); }

const std::vector<double> &axes::y2ticks() const { return y2_axis_.tick_values(); }

void axes::y2ticks(const std::vector<double> &ticks) { y2_axis_.tick_values(ticks); }

const std::vector<double> &axes::zticks() const { return z_axis_.tick_values(); }

void axes::zticks(const std::vector<double> &ticks) { z_axis_.tick_values(ticks); }

const std::vector<double> &axes::rticks() const { return r_axis_.tick_values(); }

void axes::rticks(const std::vector<double> &ticks) { r_axis_.tick_values(ticks); }
...
float axes::title_font_size_multiplier() const { return title_font_size_multiplier_; }
...
const std::string &axes::title_font_weight() const { return title_font_weight_; }
...
const std::vector<color_array> &axes::colororder() const { return colororder_; }
...
void axes::colororder_index(size_t colororder_index) { colororder_index_ = colororder_index; }

void axes::bump_colororder_index() { colororder_index_ = (colororder_index_ + 1) % colororder_.size(); }
...
const color_array axes::grid_color() const { return grid_line_style_.color(); }
...
float axes::minor_grid_alpha() const { return minor_grid_line_style_.alpha(); }
...
const color_array axes::minor_grid_color() const { return minor_grid_line_style_.color(); }
...
line_spec axes::minor_grid_line_style() const { return minor_grid_line_style_; }
...
const std::string &axes::line_style_order() const { return line_style_order_; }
...
size_t axes::line_style_order_index() const { return line_style_order_index_; }
...
const std::vector<std::shared_ptr<class axes_object>> &axes::children() const { return children_; }
...
bool axes::limits_mode_automatic() const {
    return x_axis_.limits_mode_auto() && y_axis_.limits_mode_auto() && y2_axis_.limits_mode_auto() &&
...
bool axes::limits_mode_manual() const {
    return x_axis_.limits_mode_manual() && y_axis_.limits_mode_manual() && y2_axis_.limits_mode_manual() &&
...
bool axes::minor_grid() const { return x_minor_grid_ && y_minor_grid_ && z_minor_grid_; }
...
bool axes::axes_aspect_ratio_auto() const { return axes_aspect_ratio_auto_; }
...
std::pair<float, float> axes::view() const { return std::make_pair(azimuth_, elevation_); }
...
const std::vector<std::vector<double>> &axes::colormap() const { return colormap_; }
...
bool axes::color_box_log_scale() const { return cb_axis_.scale() == axis::axis_scale::log; }
...
void axes::color_box_range(double cb_min, double cb_max) { color_box_range({cb_min, cb_max}); }

const std::vector<std::string> &axes::xticklabels() const { return x_axis().ticklabels(); }
...
const std::vector<std::string> &axes::x2ticklabels() const { return x2_axis().ticklabels(); }
...
const std::vector<std::string> &axes::yticklabels() const { return y_axis().ticklabels(); }
...
const std::vector<std::string> &axes::y2ticklabels() const { return y2_axis().ticklabels(); }
...
const std::vector<std::string> &axes::zticklabels() const { return z_axis().ticklabels(); }
...
const std::vector<std::string> &axes::rticklabels() const { return r_axis().ticklabels(); }
...
const std::array<double, 2> &axes::xlim() const { return x_axis().limits(); }
...
const std::array<double, 2> &axes::x2lim() const { return x2_axis().limits(); }
...
const std::array<double, 2> &axes::ylim() const { return y_axis().limits(); }
...
const std::array<double, 2> &axes::y2lim() const { return y2_axis().limits(); }
...
const std::array<double, 2> &axes::zlim() const { return z_axis().limits(); }
...
const std::array<double, 2> &axes::rlim() const { return r_axis().limits(); }
...
const std::array<double, 2> &axes::tlim() const { return t_axis().limits(); }
...
void axes::xlim(keyword_automatic_type) { x_axis().limits_mode_manual(false); }

void axes::xlim(keyword_manual_type) { x_axis().limits_mode_manual(true); }

void axes::x2lim(keyword_automatic_type) { x2_axis().limits_mode_manual(false); }

void axes::x2lim(keyword_manual_type) { x2_axis().limits_mode_manual(true); }

void axes::ylim(keyword_automatic_type) { y_axis().limits_mode_manual(false); }

void axes::ylim(keyword_manual_type) { y_axis().limits_mode_manual(true); }

void axes::y2lim(keyword_automatic_type) { y2_axis().limits_mode_manual(false); }

void axes::y2lim(keyword_manual_type) { y2_axis().limits_mode_manual(true); }

void axes::zlim(keyword_automatic_type) { z_axis().limits_mode_manual(false); }

void axes::zlim(keyword_manual_type) { z_axis().limits_mode_manual(true); }

void axes::rlim(keyword_automatic_type) { r_axis().limits_mode_manual(false); }

void axes::rlim(keyword_manual_type) { r_axis().limits_mode_manual(true); }

void axes::tlim(keyword_automatic_type) { t_axis().limits_mode_manual(false); }
...
const std::array<double, 2> &axes::cblim() const { return cb_axis_.limits(); }

void axes::cblim(const std::array<double, 2> &a) { cb_axis_.limits(a); }

void axes::cblim(keyword_automatic_type) { cb_axis_.limits_mode_auto(true); }
...
const std::string &axes::cbtickformat() const { return cb_axis_.tick_label_format(); }

void axes::cbtickformat(const std::string &str) { cb_axis_.tick_label_format(str); }

const std::vector<double> &axes::cbticks() const { return cb_axis_.tick_values(); }

void axes::cbticks(const std::vector<double> &ticks) { cb_axis_.tick_values(ticks); }

const std::vector<std::string> &axes::cbticklabels() const { return cb_axis_.ticklabels(); }

void axes::cbticklabels(const std::vector<std::string> &labels) { cb_axis_.ticklabels(); }
...
const std::array<float, 4> &axes::cb_position() const { return cb_position_; }
...
class axes_silencer {
   public:
    explicit axes_silencer(class axes *ax) : ax_(ax), previous_quiet_mode_(ax->parent()->quiet_mode()) {
...
std::vector<line_handle> axes::plot(const std::vector<double> &x, const std::vector<std::vector<double>> &Y,
...
std::vector<line_handle> axes::rgbplot(const std::vector<std::vector<double>> &map) {
    ...
    double n = map.size();
    auto palette_enabler = plot(
        vector_1d{1, 1, 1, n, n, n},
        vector_1d{rgb[0].front(), rgb[1].front(), rgb[2].front(), rgb[0].back(), rgb[1].back(), rgb[2].back()}, ".");
...
std::vector<line_handle> axes::plot3(const std::vector<std::vector<double>> &X,
                                     const std::vector<std::vector<double>> &Y, const std::vector<double> &z,
                                     const std::string &line_spec) {
    axes_silencer s{this};
    auto it_x = X.begin();
    auto it_y = Y.begin();
...
stair_handle axes::stairs(const std::vector<double> &x, const std::vector<double> &y, const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
...
std::vector<stair_handle> axes::stairs(const std::vector<std::vector<double>> &X,
                                       const std::vector<std::vector<double>> &Y, const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
...
error_bar_handle axes::errorbar(const std::vector<double> &x, const std::vector<double> &y,
                                const std::vector<double> &error, error_bar::type type, const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
...
error_bar_handle axes::errorbar(const std::vector<double> &x, const std::vector<double> &y,
                                const std::vector<double> &y_neg_delta, const std::vector<double> &y_pos_delta,
                                const std::vector<double> &x_neg_delta, const std::vector<double> &x_pos_delta,
                                const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
    error_bar_handle l =
        std::make_shared<class error_bar>(this, x, y, y_neg_delta, y_pos_delta, x_neg_delta, x_pos_delta, line_spec);
    this->emplace_object(l);
    return l;
}

error_bar_handle axes::errorbar(const std::vector<double> &x, const std::vector<double> &y,
                                const std::vector<double> &y_error, const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
    return this->errorbar(x, y, y_error, error_bar::type::vertical, line_spec);
}

std::vector<filled_area_handle> axes::area(const std::vector<double> &x, const std::vector<std::vector<double>> &Y,
    ...
    for (auto [Y_it, c_it] = std::make_pair(Y.rbegin(), colors.rbegin()); Y_it != Y.rend(); ++Y_it, ++c_it) {
        const auto &y = *Y_it;
...
filled_area_handle axes::area(const std::vector<double> &x, const std::vector<double> &y, double base_value,
...
std::vector<filled_area_handle> axes::area(const std::vector<std::vector<double>> &Y, double base_value, bool stacked,
                                           const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
    return this->area(iota(1., Y.begin()->size()), Y, base_value, stacked, line_spec);
}

std::vector<filled_area_handle> axes::area(const std::vector<std::vector<double>> &Y, bool stacked,
                                           const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
    return this->area(iota(1., Y.begin()->size()), Y, 0., stacked, line_spec);
}

filled_area_handle axes::area(const std::vector<double> &y, double base_value, bool stacked,
                              const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
...
std::vector<string_function_handle> axes::fplot(std::vector<std::string> equations,
...
function_line_handle axes::fplot(function_line::function_type equation, const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
    function_line_handle obj =
        std::make_shared<class function_line>(this, equation, std::array<double, 2>({-5, 5}), line_spec);
    this->emplace_object(obj);
    obj->touch();
    return obj;
}
...
std::vector<function_line_handle> axes::fplot(std::vector<function_line::function_type> equations,
                                              std::array<double, 2> x_range, std::vector<std::string> line_specs) {
    axes_silencer temp_silencer_{this};
...
std::vector<function_line_handle> axes::fplot(std::vector<function_line::function_type> equations,
                                              std::vector<double> x_range, std::vector<std::string> line_specs) {
...
function_line_handle axes::fplot3(function_line::function_type function_x, function_line::function_type function_y,
    ...
    axes_silencer temp_silencer_{this};
    function_line_handle obj =
        std::make_shared<class function_line>(this, function_x, function_y, function_z, t_range, line_spec);
    this->emplace_object(obj);
    obj->touch();
    return obj;
}
...
axes_object_handle axes::binscatter(const std::vector<double> &x, const std::vector<double> &y,
...
axes_object_handle axes::binscatter(const std::vector<double> &x, const std::vector<double> &y,
                                    histogram::binning_algorithm algorithm, enum bin_scatter_style scatter_style,
                                    enum histogram::normalization normalization_alg) {
...
axes_object_handle axes::binscatter(const std::vector<double> &x, const std::vector<double> &y,
                                    enum bin_scatter_style scatter_style, histogram::binning_algorithm algorithm,
                                    enum histogram::normalization normalization_alg) {
    return this->binscatter(x, y, algorithm, scatter_style, normalization_alg);
}

axes_object_handle axes::binscatter(const std::vector<double> &x, const std::vector<double> &y, size_t nbins_x,
...
parallel_lines_handle axes::parallelplot(const std::vector<std::vector<double>> &X, const std::vector<double> &colors,
                                         const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
...
circles_handle axes::pie(const std::vector<double> &x, const std::vector<double> &explode,
...
line_handle axes::scatter(const std::vector<double> &x, const std::vector<double> &y, const std::vector<double> &sizes,
...
line_handle axes::scatter(const std::vector<double> &x, const std::vector<double> &y, double sizes,
                          const std::vector<double> &colors) {
    if (sizes == 0) {
...
labels_handle axes::wordcloud(const std::string &text, const std::vector<std::string> &black_list,
...
std::pair<bars_handle, line_handle> axes::pareto(const std::vector<double> &y, const std::vector<std::string> &names,
...
/// Core plot function
line_handle axes::stem(const std::vector<double> &x, const std::vector<double> &y, const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
...
std::vector<line_handle> axes::stem(const std::vector<std::vector<double>> &Y, const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
...
std::vector<line_handle> axes::stem3(const std::vector<std::vector<double>> &X,
                                     const std::vector<std::vector<double>> &Y, const std::vector<double> &z,
                                     const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
...
void axes::geolimits(const std::array<double, 2> &latitude, const std::array<double, 2> &longitude) {
    ...
    if (map) {
        ...
        auto &[map_x, map_y] =
            (min_km_per_pixel <= 10) ? world_map_10m() : (min_km_per_pixel <= 50) ? world_map_50m() : world_map_110m();
        ...
        for (size_t i = 0; i < map_x.size(); ++i) {
            ...
            auto inside_the_map = [&](double x, double y) {
                return isfinite(x) && isfinite(y) && x <= longitude[1] && x >= longitude[0] && y <= latitude[1] &&
                       y >= latitude[0];
            };
...
void axes::axis(keyword_automatic_type automatic) { this->limits_mode(automatic); }
...
void axes::axis(keyword_tight_type) {
    ...
    if (this->is_3d() && !this->is_3d_map()) {
        auto zminit =
            std::min_element(this->children().begin(), this->children().end(),
                             [](axes_object_handle a, axes_object_handle b) { return a->zmin() < b->zmin(); });
        auto zmaxit =
            std::max_element(this->children().begin(), this->children().end(),
                             [](axes_object_handle a, axes_object_handle b) { return a->zmax() < b->zmax(); });
...
std::vector<string_function_handle> axes::ezpolar(std::vector<std::string> equations,
                                                  std::vector<std::string> line_specs) {
    std::vector<string_function_handle> res;
    auto it_line_specs = line_specs.begin();
    for (const auto &equation : equations) {
...
function_line_handle axes::ezpolar(function_line::function_type equation, const std::string &line_spec) {
    ...
    function_line_handle obj =
        std::make_shared<class function_line>(this, equation, std::array<double, 2>({0, 2 * pi}), line_spec);
    this->emplace_object(obj);
    obj->polar(true);
    this->axis(equal);
    this->x_axis().visible(false);
...
contours_handle axes::fcontour(fcontour_function_type fn, const std::array<double, 4> &xy_range,
                               std::vector<double> levels, const std::string &line_spec, size_t n_levels) {
    axes_silencer temp_silencer_{this};
...
vectors_handle axes::feather(const std::vector<double> &u, const std::vector<double> &v, const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
...
vectors_handle axes::quiver(const std::vector<double> &x, const std::vector<double> &y, const std::vector<double> &u,
                            const std::vector<double> &v, double scale, const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
...
/// Quiver - 2d x,y,u,v
vectors_handle axes::quiver(const std::vector<std::vector<double>> &x, const std::vector<std::vector<double>> &y,
...
vectors_handle axes::quiver3(const std::vector<double> &x, const std::vector<double> &y, const std::vector<double> &z,
...
vectors_handle axes::quiver3(const std::vector<std::vector<double>> &x, const std::vector<std::vector<double>> &y,
                             const std::vector<std::vector<double>> &z, const std::vector<std::vector<double>> &u,
                             const std::vector<std::vector<double>> &v, const std::vector<std::vector<double>> &w,
                             double scale, const std::string &line_spec) {
...
vectors_handle axes::quiver3(const std::vector<std::vector<double>> &z, const std::vector<std::vector<double>> &u,
                             const std::vector<std::vector<double>> &v, const std::vector<std::vector<double>> &w,
                             double scale, const std::string &line_spec) {
    auto [n, m] = size(z);
...
/// Fence - Core function
surface_handle axes::fence(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                           const std::vector<std::vector<double>> &Z, const std::vector<double> &c) {
    axes_silencer temp_silencer_{this};
...
surface_handle axes::fmesh(fcontour_function_type fn, const std::array<double, 2> &x_range,
                           const std::array<double, 2> &y_range, double mesh_density) {
    axes_silencer temp_silencer_{this};
...
surface_handle axes::fmesh(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                           const std::array<double, 2> &u_range, const std::array<double, 2> &v_range,
                           double mesh_density) {
    axes_silencer temp_silencer_{this};
...
surface_handle axes::fmesh(fcontour_function_type fn, const std::array<double, 4> &xy_range, double mesh_density) {
    return this->fmesh(fn, {xy_range[0], xy_range[1]}, {xy_range[2], xy_range[3]}, mesh_density);
}

/// Lambda function mesh - Parametric
/// Parametric / Both ranges in the same array size 4
surface_handle axes::fmesh(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                           const std::array<double, 4> &uv_range, double mesh_density) {
    return this->fmesh(funx, funy, funz, {uv_range[0], uv_range[1]}, {uv_range[2], uv_range[3]}, mesh_density);
}

/// Function mesh
/// Grid / Both ranges in the same array size 2
surface_handle axes::fmesh(fcontour_function_type fn, const std::array<double, 2> &xy_range, double mesh_density) {
...
surface_handle axes::fmesh(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                           const std::array<double, 2> &uv_range, double mesh_density) {
...
surface_handle axes::fsurf(fcontour_function_type fn, const std::array<double, 2> &x_range,
                           const std::array<double, 2> &y_range, std::string line_spec, double mesh_density) {
    axes_silencer temp_silencer_{this};
...
surface_handle axes::fsurf(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                           const std::array<double, 2> &u_range, const std::array<double, 2> &v_range,
...
/// Function surf
/// Grid / Both ranges in the same array size 4
surface_handle axes::fsurf(fcontour_function_type fn, const std::array<double, 4> &xy_range, std::string line_spec,
                           double mesh_density) {
    return this->fsurf(fn, {xy_range[0], xy_range[1]}, {xy_range[2], xy_range[3]}, line_spec, mesh_density);
}

/// Parametric / Both ranges in the same array size 4
surface_handle axes::fsurf(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                           const std::array<double, 4> &uv_range, std::string line_spec, double mesh_density) {
    return this->fsurf(funx, funy, funz, {uv_range[0], uv_range[1]}, {uv_range[2], uv_range[3]}, line_spec,
                       mesh_density);
}

/// Function surf
/// Grid / Both ranges in the same array size 2
surface_handle axes::fsurf(fcontour_function_type fn, const std::array<double, 2> &xy_range, std::string line_spec,
                           double mesh_density) {
...
/// Parametric / Both ranges in the same array size 2
surface_handle axes::fsurf(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                           const std::array<double, 2> &uv_range, std::string line_spec, double mesh_density) {
    return this->fsurf(funx, funy, funz, uv_range, uv_range, line_spec, mesh_density);
}

/// Mesh - Core function
surface_handle axes::mesh(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                          const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C) {
    axes_silencer temp_silencer_{this};
...
/// Mesh with contour
surface_handle axes::meshc(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                           const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C) {
    axes_silencer temp_silencer_{this};
...
/// Mesh with curtain
/// Core function
surface_handle axes::meshz(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                           const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C) {
    axes_silencer temp_silencer_{this};
...
/// Ribbon
surface_handle axes::ribbon(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                            const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C,
                            double width) {
    axes_silencer temp_silencer_{this};
...
/// Surf - Core function
surface_handle axes::surf(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                          const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C,
...
surface_handle axes::surfc(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                           const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C) {
    axes_silencer temp_silencer_{this};
...
surface_handle axes::waterfall(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                               const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C) {
    axes_silencer temp_silencer_{this};
...
network_handle axes::graph(const std::vector<std::pair<size_t, size_t>> &edges, const std::vector<double> &weights,
                           size_t n_vertices, std::string line_spec) {
    axes_silencer temp_silencer_{this};
...
network_handle axes::graph(const std::vector<std::pair<size_t, size_t>> &edges, std::string line_spec) {
...
matrix_handle axes::image(const std::vector<std::vector<double>> &C, bool scaled_colorbar) {
    axes_silencer temp_silencer_{this};
...
matrix_handle axes::image(const std::vector<std::vector<double>> &r_channel,
                          ...
                          const std::vector<std::vector<double>> &b_channel, bool scaled_colorbar) {
    axes_silencer temp_silencer_{this};
...
matrix_handle axes::image(double x_min, double x_max, double y_min, double y_max,
                          const std::vector<std::vector<double>> &C, bool scaled_colorbar) {
    axes_silencer temp_silencer_{this};
...
matrix_handle axes::image(const std::tuple<std::vector<std::vector<double>>, std::vector<std::vector<double>>,
                                           std::vector<std::vector<double>>> &img) {
    return this->image(std::get<0>(img), std::get<1>(img), std::get<2>(img), false);
}
...
labels_handle axes::text(const vector_1d &x, const vector_1d &y, const std::string &str) {
...
vectors_handle axes::arrow(double x1, double y1, double x2, double y2) {
    ...
    double v = y2 - y1;
    vectors_handle q =
        std::make_shared<class vectors>(this, vector_1d{x1}, vector_1d{y1}, vector_1d{u}, vector_1d{v}, "-k");
    q->line_width(1.5);
...
std::pair<labels_handle, vectors_handle> axes::textarrow(double x1, double y1, double x2, double y2,
...
line_handle axes::rectangle(double x, double y, double w, double h, double curvature) {
    axes_silencer temp_silencer_{this};
    bool p2 = this->next_plot_replace();
    this->next_plot_replace(false);
...
std::pair<labels_handle, line_handle> axes::textbox(double x, double y, double w, double h, const std::string &str) {
    axes_silencer temp_silencer_{this};
    bool p2 = this->next_plot_replace();
    this->next_plot_replace(false);
...
line_handle axes::fill(const vector_1d &x, const vector_1d &y, const std::string &line_spec) {
    axes_silencer temp_silencer_{this};
    bool p2 = this->next_plot_replace();
    this->next_plot_replace(false);
