...
void axes::run_position_margin_command() {
  ...
  run_command("set lmargin at screen " +
              num2str(x + width() * lmargin_multiplier + colorbar_lmargin));
  run_command(
      "set rmargin at screen " +
      num2str(x + width() - width() * rmargin_multiplier - colorbar_rmargin));
  run_command("set bmargin at screen " +
              num2str(y + height() * bmargin_multiplier + colorbar_bmargin));
  run_command(
      "set tmargin at screen " +
      num2str(y + height() - height() * tmargin_multiplier - colorbar_tmargin));
...
void axes::run_box_command() {
  ...
  if (!box_ || is_polar()) {
    if (is_3d()) {
      ...
      // left vertical 16
      run_command("set border 21 linecolor \"" + to_string(x_axis().color()) +
    ...
    if (is_3d()) {
      if (box_full_) {
        run_command("set border 4095 linecolor \"" + to_string(box_color_) +
                    "\" linewidth " + num2str(line_width()));
      } else {
        // create borders
        // bottom left front 1
        ...
        // top left back 256
        // top right back 512
        run_command("set border 895 linecolor \"" + to_string(box_color_) +
                    "\" linewidth " + num2str(line_width()));
      }
    } else {
      // create all borders (bottom / left / top / right) in black
      run_command("set border 15 linecolor \"" + to_string(box_color_) +
                  "\" linewidth " + num2str(line_width()));
    }
  }
}
...
void axes::run_axes_command() {
  ...
  auto set_or_unset_axis = [this](class axis &ax, std::string axis_name,
    ...
    if (ax.visible() && !ax.tick_values().empty()) {
      ...
      std::string tics_str =
          "set " + axis_name + "tics textcolor \"" + to_string(ax.color());
      if (parent_->backend_->supports_fonts()) {
        tics_str +=
            "\" font \"" + font() + "," + num2str(unsigned(font_size())) + "\"";
      }
    ...
    if (!legends_to_plot) {
      ...
      if (parent_->backend_->supports_fonts()) {
        cmd += " font \"" + legend_->font_name() + "," +
               num2str(legend_->font_size()) + "\" textcolor rgb '" +
      ...
      if (legend_->num_columns() != 0) {
        ...
        cmd += " maxrows " + num2str(ceil(static_cast<double>(n_strings) /
                                          legend_->num_columns()));
      }
    ...
    if (color_ != parent()->color()) {
      if (is_2d() || is_3d_map()) {
        ...
        if (has_wall_option) {
          ...
          run_command(
              "set wall x0 fs transparent solid 0.5 border -1 fc 'slategray'");
          run_command(
              "set wall y1 fs transparent solid 0.5 border -1 fc 'slategray'");
        }
      } else if (is_polar()) {
        double m = children_[0]->xmax();
        run_command("set object 2 ellipse at first 0,0 size " + num2str(m * 2) +
                    "," + num2str(m * 2) + " angle 0 behind fillcolor rgb \"" +
                    to_string(color()) + "\" fillstyle solid 1.0 noborder");
      }
    }
...
void axes::run_plot_objects_command() {
  ...
  if (legend_ != nullptr && legend_->visible()) {
    ...
    for (auto child_it = children_.begin(); child_it != children_.end();
         ++child_it) {
      const bool no_legend =
          legend_it == legend_end && (*child_it)->display_name().empty();
      if (no_legend) {
        continue;
      }
...
std::vector<line_handle> axes::plot(const std::vector<double> &x,
...
std::vector<line_handle> axes::plot(const std::vector<std::vector<double>> &Y,
                                    const std::string &line_spec) {
  axes_silencer s{this};
  std::vector<line_handle> hs;
  bool previous_replace_status = this->next_plot_replace();
...
std::vector<line_handle> axes::rgbplot(
...
std::vector<stair_handle> axes::stairs(
...
error_bar_handle axes::errorbar(const std::vector<double> &x,
  ...
  axes_silencer temp_silencer_{this};
  error_bar_handle l =
      std::make_shared<class error_bar>(this, x, y, error, type, line_spec);
  this->emplace_object(l);
  return l;
}
...
error_bar_handle axes::errorbar(const std::vector<double> &x,
  ...
  axes_silencer temp_silencer_{this};
  error_bar_handle l =
      std::make_shared<class error_bar>(this, x, y, y_neg_delta, y_pos_delta,
                                        x_neg_delta, x_pos_delta, line_spec);
  this->emplace_object(l);
  return l;
}
...
std::vector<filled_area_handle> axes::area(
    const std::vector<double> &x, const std::vector<std::vector<double>> &Y,
...
filled_area_handle axes::area(const std::vector<double> &x,
                              const std::vector<double> &y, double base_value,
...
std::vector<filled_area_handle> axes::area(
    const std::vector<std::vector<double>> &Y, double base_value, bool stacked,
    const std::string &line_spec) {
  axes_silencer temp_silencer_{this};
...
std::vector<filled_area_handle> axes::area(
    const std::vector<std::vector<double>> &Y, bool stacked,
    const std::string &line_spec) {
  axes_silencer temp_silencer_{this};
  return this->area(iota(1., Y.begin()->size()), Y, 0., stacked, line_spec);
}

filled_area_handle axes::area(const std::vector<double> &y, double base_value,
                              bool stacked, const std::string &line_spec) {
  axes_silencer temp_silencer_{this};
...
std::vector<string_function_handle> axes::fplot(
    std::vector<std::string> equations, std::vector<std::string> line_specs) {
  axes_silencer temp_silencer_{this};
...
function_line_handle axes::fplot(function_line::function_type equation,
  ...
  function_line_handle obj =
      std::make_shared<class function_line>(this, equation, x_range, line_spec);
  this->emplace_object(obj);
  obj->touch();
  return obj;
}
...
std::vector<function_line_handle> axes::fplot(
    std::vector<function_line::function_type> equations,
...
histogram_handle axes::hist(const std::vector<double> &data,
  ...
  axes_silencer temp_silencer_{this};
  histogram_handle l = std::make_shared<class histogram>(this, data, algorithm,
                                                         normalization_alg);
  this->emplace_object(l);
  return l;
}
...
histogram_handle axes::hist(const std::vector<std::string> &data,
                            enum histogram::normalization normalization_alg) {
  axes_silencer temp_silencer_{this};
...
axes_object_handle axes::binscatter(
  ...
  // value stats
  double sum_values =
      normalization_alg == histogram::normalization::count ? x.size() : 0.;
      ...
      if (values[i][j] != 0.0) {
        ...
        switch (scatter_style) {
          ...
          case bin_scatter_style::jitter: {
            double n_expected_points_in_bin =
                values[i][j] / avg_square_value * avg_pts_per_square;
...
axes_object_handle axes::binscatter(
    const std::vector<double> &x, const std::vector<double> &y, size_t nbins_x,
...
std::vector<bars_handle> axes::barstacked(
    const std::vector<std::vector<double>> &Y) {
  axes_silencer temp_silencer_{this};
...
parallel_lines_handle axes::parallelplot(
    const std::vector<std::vector<double>> &X,
    const std::vector<double> &colors, const std::string &line_spec) {
  axes_silencer temp_silencer_{this};
...
parallel_lines_handle axes::parallelplot(
    const std::vector<std::vector<double>> &X, const std::string &line_spec) {
  axes_silencer temp_silencer_{this};
...
labels_handle axes::wordcloud(const std::vector<std::string> &words,
  ...
  labels_handle wordcloud =
      std::make_shared<class labels>(this, x, y, labels, colors, float_sizes);
...
labels_handle axes::wordcloud(const std::vector<std::string> &words,
                              ...
                              const std::vector<double> &custom_colors) {
  return this->wordcloud(words, std::vector<double>(sizes.begin(), sizes.end()),
                         custom_colors);
}
...
std::pair<bars_handle, line_handle> axes::pareto(
  ...
  cummulative_y = transform(cummulative_y,
                            [total_y](double x) { return x * 100. / total_y; });
...
std::vector<line_handle> axes::stem(const std::vector<double> &x,
                                    const std::vector<std::vector<double>> &Y,
                                    const std::string &line_spec) {
  axes_silencer temp_silencer_{this};
...
std::vector<line_handle> axes::stem(const std::vector<std::vector<double>> &Y,
                                    const std::string &line_spec) {
  axes_silencer temp_silencer_{this};
...
line_handle axes::stem3(const std::vector<double> &z,
                        const std::string &line_spec) {
  return this->stem3(iota(1, z.size()), std::vector<double>(z.size(), 1.), z,
                     line_spec);
}
...
void axes::geolimits(const std::array<double, 2> &latitude,
  ...
  if (map) {
    ...
    auto &[map_x, map_y] =
        (min_km_per_pixel <= 10)
            ? world_map_10m()
            : (min_km_per_pixel <= 50) ? world_map_50m() : world_map_110m();
    ...
    for (size_t i = 0; i < map_x.size(); ++i) {
      ...
      auto inside_the_map = [&](double x, double y) {
        return isfinite(x) && isfinite(y) && x <= longitude[1] &&
               x >= longitude[0] && y <= latitude[1] && y >= latitude[0];
      };
...
void axes::geolimits(double latitude_x, double latitude_y, double longitude_x,
                     double longitude_y) {
...
vectors_handle axes::compass(const std::vector<double> &x,
  ...
  auto theta =
      transform(x, y, [](double x, double y) { return vector_radians(x, y); });
  auto rho = transform(
...
std::vector<string_function_handle> axes::ezpolar(
    std::vector<std::string> equations, std::vector<std::string> line_specs) {
  std::vector<string_function_handle> res;
  auto it_line_specs = line_specs.begin();
  for (const auto &equation : equations) {
...
function_line_handle axes::ezpolar(function_line::function_type equation,
  ...
  function_line_handle obj =
      std::make_shared<class function_line>(this, equation, t_range, line_spec);
  this->emplace_object(obj);
  obj->polar(true);
...
std::vector<function_line_handle> axes::ezpolar(
    std::vector<function_line::function_type> equations,
    std::array<double, 2> x_range, std::vector<std::string> line_specs) {
  axes_silencer temp_silencer_{this};
  std::vector<function_line_handle> res;
...
vectors_handle axes::quiver(const std::vector<double> &x,
  ...
  x_copy.resize(
      std::distance(x_copy.begin(), std::unique(x_copy.begin(), x_copy.end())));
  ...
  y_copy.resize(
      std::distance(y_copy.begin(), std::unique(y_copy.begin(), y_copy.end())));
  ...
  auto u_scaled =
      (scale != 0.)
          ? transform(u, [&](double u) { return (u / u_max) * scale * x_max; })
          : u;
  auto v_scaled =
      (scale != 0.)
          ? transform(v, [&](double v) { return (v / v_max) * scale * y_max; })
          : v;
...
vectors_handle axes::quiver(const std::vector<std::vector<double>> &x,
  ...
  return this->quiver(flatten(x), flatten(y), flatten(u), flatten(v), scale,
                      line_spec);
}
...
vectors_handle axes::quiver3(const std::vector<double> &x,
  ...
  z_copy.resize(
      std::distance(z_copy.begin(), std::unique(z_copy.begin(), z_copy.end())));
  ...
  auto w_scaled =
      (scale != 0.)
          ? transform(w, [&](double w) { return (w / w_max) * scale * z_max; })
          : w;
...
surface_handle axes::fmesh(fcontour_function_type fn,
                           const std::array<double, 4> &xy_range,
                           double mesh_density) {
  return this->fmesh(fn, {xy_range[0], xy_range[1]}, {xy_range[2], xy_range[3]},
                     mesh_density);
}
...
surface_handle axes::fsurf(fcontour_function_type fn,
                           const std::array<double, 4> &xy_range,
                           std::string line_spec, double mesh_density) {
  return this->fsurf(fn, {xy_range[0], xy_range[1]}, {xy_range[2], xy_range[3]},
                     line_spec, mesh_density);
}
...
network_handle axes::graph(const std::vector<std::pair<size_t, size_t>> &edges,
  ...
  network_handle l = std::make_shared<class network>(this, edges, weights,
                                                     n_vertices, line_spec);
  this->emplace_object(l);
  this->x_axis().visible(false);
  this->y_axis().visible(false);
...
matrix_handle axes::imshow(
  ...
  axes_silencer temp_silencer_{this};
  // Create matrix in the xlim
  matrix_handle img = std::make_shared<class matrix>(this, r_channel, g_channel,
                                                     b_channel, a_channel);
  this->emplace_object(img);
  this->axis(equal);
  this->color(this->parent()->color());
...
matrix_handle axes::image(const std::vector<std::vector<double>> &r_channel,
  ...
  // create matrix in the xlim
  matrix_handle img =
      std::make_shared<class matrix>(this, r_channel, g_channel, b_channel);
  img->always_hide_labels(true);
  this->emplace_object(img);
  this->color(this->parent()->color());
  this->box(true);
...
vectors_handle axes::arrow(double x1, double y1, double x2, double y2) {
  ...
  double u = x2 - x1;
  double v = y2 - y1;
  vectors_handle q = std::make_shared<class vectors>(
      this, vector_1d{x1}, vector_1d{y1}, vector_1d{u}, vector_1d{v}, "-k");
  q->line_width(1.5);
...
std::pair<labels_handle, vectors_handle> axes::textarrow(
    double x1, double y1, double x2, double y2, const std::string &str) {
  axes_silencer temp_silencer_{this};
  bool p2 = this->next_plot_replace();
  this->next_plot_replace(false);
...
line_handle axes::rectangle(double x, double y, double w, double h,
  ...
  if (curvature == 0.) {
    ...
    x_ = concat(x_,
                transform(xne, [&](double c) { return x + w - radius + c; }));
    x_ = concat(x_, transform(xnw, [&](double c) { return x + radius + c; }));
    x_ = concat(x_, transform(xsw, [&](double c) { return x + radius + c; }));
    x_ = concat(x_,
                transform(xse, [&](double c) { return x + w - radius + c; }));
    x_.emplace_back(x_.front());

    std::vector<double> y_;
    y_ = concat(y_,
                transform(yne, [&](double c) { return y + h - radius + c; }));
    y_ = concat(y_,
                transform(ynw, [&](double c) { return y + h - radius + c; }));
...
std::pair<labels_handle, line_handle> axes::textbox(double x, double y,
                                                    double w, double h,
                                                    const std::string &str) {
  axes_silencer temp_silencer_{this};
  bool p2 = this->next_plot_replace();
