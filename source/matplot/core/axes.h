...
class axes : public std::enable_shared_from_this<class axes> {
   ...
   public /* functions that operate the axes */:
    ...
    void emplace_object(std::shared_ptr<T> obj) {
        std::enable_if_t<std::is_base_of_v<axes_object, T>, axes_object_handle> ah =
            std::dynamic_pointer_cast<axes_object>(obj);
        emplace_object(ah);
    }
   ...
   public /* create plots on the axes */:
    ...
    /// This function represents the case 1a, where 3rd parameter line spec is a
    ...
    ///     2a) the x2 for next plot (case 1 for next plot) or
    ///     2b) the y for next plot (case 2 for next plot)
    /// This function represents the case 1b, where 3rd parameter x2 is a divider
    /// between plots
    template <class... Args>
    ...
    ///     2a) the x2 for next plot (case 1 for next plot) or
    ///     2b) the y for next plot (case 2 for next plot)
    /// This function represents the case 2, where 2nd parameter line spec is a
    /// divider between plots
    template <class... Args>
    ...
    std::vector<line_handle> plot3(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
    ...
    template <class... Args>
    auto plot3(const std::vector<double> &x, const std::vector<double> &y, const std::vector<double> &z,
    ...
    stair_handle stairs(const std::vector<double> &x, const std::vector<double> &y, const std::string &line_spec = "");
    ...
    std::vector<stair_handle> stairs(const std::vector<std::vector<double>> &X,
    ...
    /// This function represents case 1a
    template <class... Args>
    auto stairs(const std::vector<double> &x, const std::vector<double> &y, const std::string &line_spec,
                Args... args) {
    ...
    /// This function represents case 1b
    template <class... Args>
    auto stairs(const std::vector<double> &x, const std::vector<double> &y, const std::vector<double> &x2,
                Args... args) {
        std::vector<stair_handle> result;
    ...
    error_bar_handle errorbar(const std::vector<double> &x, const std::vector<double> &y,
                              const std::vector<double> &error, error_bar::type type = error_bar::type::vertical,
                              const std::string &line_spec = "");
    ...
    /// If there is a string instead of a type the first version, we default to
    /// vertical
    error_bar_handle errorbar(const std::vector<double> &x, const std::vector<double> &y,
                              const std::vector<double> &y_error, const std::string &line_spec);

    /// Core area function (x is a vector, y is a matrix)
    std::vector<filled_area_handle> area(const std::vector<double> &x, const std::vector<std::vector<double>> &Y,
                                         double base_value = 0., bool stacked = true,
                                         const std::string &line_spec = "k-");

    /// Area: x is a vector, y is a vector
    filled_area_handle area(const std::vector<double> &x, const std::vector<double> &y, double base_value = 0.,
                            bool stacked = true, const std::string &line_spec = "k-");

    /// Area: Automatic x, y is a matrix
    std::vector<filled_area_handle> area(const std::vector<std::vector<double>> &Y, double base_value = 0.,
                                         bool stacked = true, const std::string &line_spec = "k-");

    /// Area: Skip the base value
    std::vector<filled_area_handle> area(const std::vector<std::vector<double>> &Y, bool stacked,
                                         const std::string &line_spec = "k-");
    ...
    std::vector<string_function_handle> fplot(std::vector<std::string> equations,
    ...
    std::vector<function_line_handle> fplot(std::vector<function_line::function_type> equations,
    ...
    line_handle fimplicit(implicit_function_type equation, const std::array<double, 4> &xy_interval = {-5, 5, -5, 5},
                          const std::string &line_spec = "");
    ...
    function_line_handle fplot3(function_line::function_type function_x, function_line::function_type function_y,
                                function_line::function_type function_z, const std::array<double, 2> &t_range = {-5, 5},
                                const std::string &line_spec = "");
    ...
    axes_object_handle binscatter(const std::vector<double> &x, const std::vector<double> &y, size_t nbins_x,
                                  size_t nbins_y, enum bin_scatter_style scatter_style = bin_scatter_style::automatic,
                                  enum histogram::normalization normalization_alg = histogram::normalization::count);
    ...
    parallel_lines_handle parallelplot(const std::vector<std::vector<double>> &X, const std::string &line_spec = "");
    ...
    line_handle scatter(const std::vector<double> &x, const std::vector<double> &y,
                        const std::vector<double> &sizes = {}, const std::vector<double> &colors = {});

    /// Scatter function with same size for all points
    line_handle scatter(const std::vector<double> &x, const std::vector<double> &y, double sizes,
    ...
    std::pair<bars_handle, line_handle> pareto(const std::vector<double> &y, const std::vector<std::string> &names = {},
    ...
    std::vector<line_handle> stem3(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
    ...
    vectors_handle compass(const std::vector<double> &x, const std::vector<double> &y,
                           const std::string &line_spec = "");

    /// Ezpolar - Plot function on polar plot
    string_function_handle ezpolar(const std::string &equation, const std::string &line_spec = "");

    std::vector<string_function_handle> ezpolar(std::vector<std::string> equations,
                                                std::vector<std::string> line_specs);
    ...
    contours_handle contour(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                            const std::vector<std::vector<double>> &Z, size_t n_levels = 0,
                            const std::string &line_spec = "");
    ...
    contours_handle fcontour(fcontour_function_type fn, const std::array<double, 4> &xy_range,
                             std::vector<double> levels = {}, const std::string &line_spec = "", size_t n_levels = 0);

    /// Lambda function contour - Manual number of levels (default = 9)
    contours_handle fcontour(fcontour_function_type fn, size_t n_levels = 9, const std::string &line_spec = "");
    ...
    vectors_handle feather(const std::vector<double> &u, const std::vector<double> &v,
                           const std::string &line_spec = "");

    /// Quiver - Core function
    vectors_handle quiver(const std::vector<double> &x, const std::vector<double> &y, const std::vector<double> &u,
                          const std::vector<double> &v, double scale = 1.0, const std::string &line_spec = "");

    /// Quiver - 2d x,y,u,v
    vectors_handle quiver(const std::vector<std::vector<double>> &x, const std::vector<std::vector<double>> &y,
    ...
    vectors_handle quiver3(const std::vector<double> &x, const std::vector<double> &y, const std::vector<double> &z,
                           const std::vector<double> &u, const std::vector<double> &v, const std::vector<double> &w,
                           double scale = 1.0, const std::string &line_spec = "");

    /// Quiver 3d - 2d vectors
    vectors_handle quiver3(const std::vector<std::vector<double>> &x, const std::vector<std::vector<double>> &y,
    ...
    vectors_handle quiver3(const std::vector<std::vector<double>> &z, const std::vector<std::vector<double>> &u,
                           const std::vector<std::vector<double>> &v, const std::vector<std::vector<double>> &w,
                           double scale = 1.0, const std::string &line_spec = "");

    /// Fence - Core function
    surface_handle fence(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
    ...
    surface_handle fmesh(fcontour_function_type fn, const std::array<double, 2> &x_range,
                         const std::array<double, 2> &y_range, double mesh_density = 40);

    /// Lambda function mesh - parametric function
    surface_handle fmesh(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
    ...
    surface_handle fmesh(fcontour_function_type fn, const std::array<double, 4> &xy_range, double mesh_density = 40);

    /// Lambda function mesh - Parametric
    /// Parametric / Both ranges in the same array size 4
    surface_handle fmesh(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                         const std::array<double, 4> &uv_range, double mesh_density = 40);

    /// Function mesh
    /// Grid / Both ranges in the same array size 2
    surface_handle fmesh(fcontour_function_type fn, const std::array<double, 2> &xy_range = {-5, +5},
                         double mesh_density = 40);

    /// Function mesh
    /// Parametric / Both ranges in the same array size 2
    surface_handle fmesh(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                         const std::array<double, 2> &uv_range = {-5, +5}, double mesh_density = 40);

    // z = f(x,y)
    using fsurf_function_type = std::function<double(double, double)>;

    /// Function surf - Core function
    surface_handle fsurf(fcontour_function_type fn, const std::array<double, 2> &x_range,
                         const std::array<double, 2> &y_range, std::string line_spec = "", double mesh_density = 40);

    /// Function surf - Parametric
    surface_handle fsurf(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                         const std::array<double, 2> &u_range, const std::array<double, 2> &v_range,
                         std::string line_spec = "", double mesh_density = 40);

    /// Function surf
    /// Grid / Both ranges in the same array size 4
    surface_handle fsurf(fcontour_function_type fn, const std::array<double, 4> &xy_range, std::string line_spec = "",
                         double mesh_density = 40);

    /// Parametric / Both ranges in the same array size 4
    surface_handle fsurf(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                         const std::array<double, 4> &uv_range, std::string line_spec = "", double mesh_density = 40);

    /// Function surf
    /// Grid / Both ranges in the same array size 2
    surface_handle fsurf(fcontour_function_type fn, const std::array<double, 2> &xy_range = {-5, +5},
                         std::string line_spec = "", double mesh_density = 40);

    /// Function surf
    /// Parametric / Both ranges in the same array size 2
    surface_handle fsurf(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
                         const std::array<double, 2> &uv_range = {-5, +5}, std::string line_spec = "",
                         double mesh_density = 40);

    /// Function surf
    /// Grid / Both ranges in the same array size 2
    surface_handle fsurf(fcontour_function_type fn, std::initializer_list<double> &xy_range, std::string line_spec = "",
                         double mesh_density = 40) {
        if (xy_range.size() == 2) {
    ...
    /// Function surf
    /// Parametric / Both ranges in the same array size 2
    inline surface_handle fsurf(fcontour_function_type funx, fcontour_function_type funy, fcontour_function_type funz,
    ...
    /// Mesh - Core function
    surface_handle mesh(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                        const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C = {});

    /// Mesh with contour
    surface_handle meshc(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                         const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C = {});

    /// Mesh with curtain
    /// Core function
    surface_handle meshz(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                         const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C = {});

    /// Ribbon
    surface_handle ribbon(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                          const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C = {},
                          double width = 0.75);

    /// Surf - Core function
    surface_handle surf(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                        const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C = {},
                        std::string line_spec = "");

    /// Surf with contour - Core function
    surface_handle surfc(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                         const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C = {});

    /// Waterfall - Core function
    surface_handle waterfall(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
                             const std::vector<std::vector<double>> &Z, const std::vector<std::vector<double>> &C = {});

    /// Graph - Undirected - Core function
    network_handle graph(const std::vector<std::pair<size_t, size_t>> &edges, const std::vector<double> &weights = {},
                         size_t n_vertices = 0, std::string line_spec = "-o");

    network_handle graph(const std::vector<std::pair<size_t, size_t>> &edges, std::string line_spec);
    ...
    matrix_handle image(const std::vector<std::vector<double>> &r_channel,
                        ...
                        const std::vector<std::vector<double>> &b_channel, bool scaled_colorbar = false);

    /// Show 2d-array as image
    matrix_handle image(double x_min, double x_max, double y_min, double y_max,
                        const std::vector<std::vector<double>> &C, bool scaled_colorbar = false);

    /// Show image from tuple of rgb 2d vectors
    matrix_handle image(const std::tuple<std::vector<std::vector<double>>, std::vector<std::vector<double>>,
    ...
    /// \param texts Texts
    /// \return
    labels_handle text(const std::vector<double> &x, const std::vector<double> &y,
    ...
    labels_handle text(const std::vector<double> &x, const std::vector<double> &y, const std::string &str);
    ...
    std::pair<labels_handle, vectors_handle> textarrow(double x1, double y1, double x2, double y2,
                                                       const std::string &str);

    /// Annotate plot with rectangle
    line_handle rectangle(double x, double y, double w, double h, double curvature = 0.);

    /// Annotate plot with text in a box
    std::pair<labels_handle, line_handle> textbox(double x, double y, double w, double h, const std::string &str);

    /// Annotate plot with a filled polygon
    line_handle fill(const std::vector<double> &x, const std::vector<double> &y, const std::string &line_spec = "");
   ...
   public /* create plots on the axes from ranges we can convert to
    ...
    template <class T1, class T2, class T3>
    std::enable_if_t<is_iterable_value_v<T3>, std::vector<line_handle>> plot3(const IterableIterables<T1> &X,
    ...
    template <class T1, class T2, class T3>
    std::enable_if_t<is_iterable_iterable_v<T3>, std::vector<line_handle>> plot3(const IterableIterables<T1> &X,
                                                                                 const IterableIterables<T2> &Y,
    ...
    template <class T1, class T2, class T3, class... Args>
    auto plot3(const IterableValues<T1> &x, const IterableValues<T2> &y, const IterableValues<T3> &z,
               const std::string &line_spec, Args... args) {
    ...
    auto plot3(const IterableValues<T1> &x, const IterableValues<T2> &y, const IterableValues<T3> &z,
               const IterableValues<T4> &x2, Args... args) {
    ...
    template <class T1, class T2>
    std::enable_if_t<is_iterable_value_v<T1>, std::vector<stair_handle>> stairs(const IterableValues<T1> &x,
                                                                                const IterableIterables<T2> &Y,
                                                                                const std::string &line_spec = "") {
    ...
    template <class T1, class T2>
    std::enable_if_t<is_iterable_iterable_v<T1>, std::vector<stair_handle>> stairs(const IterableIterables<T1> &X,
                                                                                   const IterableIterables<T2> &Y,
                                                                                   const std::string &line_spec = "") {
    ...
    error_bar_handle errorbar(const IterableValues<T1> &x, const IterableValues<T2> &y,
                              const IterableValues<T3> &y_neg_delta, const IterableValues<T4> &y_pos_delta,
                              const IterableValues<T5> &x_neg_delta, const IterableValues<T6> &x_pos_delta,
                              const std::string &line_spec = "") {
        return errorbar(to_vector_1d(x), to_vector_1d(y), to_vector_1d(y_neg_delta), to_vector_1d(y_pos_delta),
                        to_vector_1d(x_neg_delta), to_vector_1d(x_pos_delta), line_spec);
    }

    template <class T1, class T2, class T3>
    error_bar_handle errorbar(const IterableValues<T1> &x, const IterableValues<T2> &y,
                              const IterableValues<T3> &y_error, const std::string &line_spec) {
        return errorbar(to_vector_1d(x), to_vector_1d(y), to_vector_1d(y_error), line_spec);
    }

    template <class T1, class T2>
    std::vector<filled_area_handle> area(const IterableValues<T1> &x, const IterableIterables<T2> &Y,
                                         double base_value = 0., bool stacked = true,
                                         const std::string &line_spec = "k-") {
    ...
    template <class T1, class T2>
    filled_area_handle area(const IterableValues<T1> &x, const IterableValues<T2> &y, double base_value = 0.,
                            bool stacked = true, const std::string &line_spec = "k-") {
        return area(to_vector_1d(x), to_vector_1d(y), base_value, stacked, line_spec);
    }

    template <class T1>
    std::vector<filled_area_handle> area(const IterableIterables<T1> &Y, double base_value = 0., bool stacked = true,
                                         const std::string &line_spec = "k-") {
        return area(to_vector_2d(Y), base_value, stacked, line_spec);
    }

    template <class T1>
    std::vector<filled_area_handle> area(const IterableIterables<T1> &Y, bool stacked,
                                         const std::string &line_spec = "k-") {
    ...
    axes_object_handle binscatter(const IterableValues<T1> &x, const IterableValues<T2> &y,
                                  const IterableValues<T3> &edges_x, const IterableValues<T4> &edges_y,
                                  enum bin_scatter_style scatter_style = bin_scatter_style::automatic,
                                  enum histogram::normalization normalization_alg = histogram::normalization::count) {
        return binscatter(to_vector_1d(x), to_vector_1d(y), to_vector_1d(edges_x), to_vector_1d(edges_y), scatter_style,
                          normalization_alg);
    }
    ...
    template <class T1, class T2>
    axes_object_handle binscatter(const IterableValues<T1> &x, const IterableValues<T2> &y, size_t nbins_x,
                                  size_t nbins_y, enum bin_scatter_style scatter_style = bin_scatter_style::automatic,
                                  enum histogram::normalization normalization_alg = histogram::normalization::count) {
    ...
    template <class T1>
    std::enable_if_t<is_iterable_value_v<T1>, box_chart_handle> boxplot(const IterableValues<T1> &data) {
        return boxplot(to_vector_1d(data));
    }

    template <class T1>
    std::enable_if_t<is_iterable_iterable_v<T1>, box_chart_handle> boxplot(const IterableIterables<T1> &data) {
    ...
    template <class T1, class T2>
    std::enable_if_t<is_iterable_value_v<T2>, bars_handle> bar(const IterableValues<T1> &x,
                                                               const IterableValues<T2> &y) {
        return bar(to_vector_1d(x), to_vector_1d(y));
    }

    template <class T1>
    std::enable_if_t<is_iterable_value_v<T1>, bars_handle> bar(const IterableValues<T1> &y) {
        return bar(to_vector_1d(y));
    }

    template <class T1>
    std::enable_if_t<is_iterable_iterable_v<T1>, bars_handle> bar(const IterableIterables<T1> &Y) {
        return bar(to_vector_2d(Y));
    }

    template <class T1, class T2>
    std::enable_if_t<is_iterable_iterable_v<T2>, bars_handle> bar(const IterableValues<T1> &x,
                                                                  const IterableIterables<T2> &Y) {
    ...
    template <class T1>
    parallel_lines_handle parallelplot(const IterableIterables<T1> &X, const std::string &line_spec = "") {
    ...
    template <class T1, class T2, class T3, class T4>
    line_handle scatter(const IterableValues<T1> &x, const IterableValues<T2> &y, const IterableValues<T3> &sizes = {},
                        const IterableValues<T4> &colors = {}) {
        return scatter(to_vector_1d(x), to_vector_1d(y), to_vector_1d(sizes), to_vector_1d(colors));
    }

    template <class T1, class T2, class T3>
    line_handle scatter(const IterableValues<T1> &x, const IterableValues<T2> &y, double sizes,
    ...
    line_handle scatter3(const IterableValues<T1> &x, const IterableValues<T2> &y, const IterableValues<T3> &z,
                         ...
                         const std::string &line_spec = "o") {
        return scatter3(to_vector_1d(x), to_vector_1d(y), to_vector_1d(z), to_vector_1d(sizes), to_vector_1d(colors),
                        line_spec);
    }
    ...
    template <class T1>
    std::pair<bars_handle, line_handle> pareto(const IterableValues<T1> &y, const std::vector<std::string> &names = {},
                                               double p_threshold = 0.95, size_t item_threshold = 10) {
    ...
    template <class T1, class T2, class T3>
    std::vector<line_handle> stem3(const IterableIterables<T1> &X, const IterableIterables<T2> &Y,
    ...
    template <class T1, class T2>
    vectors_handle compass(const IterableValues<T1> &x, const IterableValues<T2> &y,
                           const std::string &line_spec = "") {
    ...
    template <class T1, class T2, class T3, class T4>
    line_handle polarscatter(const IterableValues<T1> &theta, const IterableValues<T2> &rho,
                             const IterableValues<T3> &sizes = std::vector<double>{},
                             const IterableValues<T4> &colors = std::vector<double>{},
                             const std::string &line_spec = "o") {
        return polarscatter(to_vector_1d(theta), to_vector_1d(rho), to_vector_1d(sizes), to_vector_1d(colors),
                            line_spec);
    }

    template <class T1, class T2>
    line_handle polarscatter(const IterableValues<T1> &theta, const IterableValues<T2> &rho, double size,
                             const std::string &line_spec = "o") {
    ...
    template <class T1, class T2, class T3, class T4>
    contours_handle contour(const IterableIterables<T1> &X, const IterableIterables<T2> &Y,
                            const IterableIterables<T3> &Z, const IterableValues<T4> &levels,
                            const std::string &line_spec = "", size_t n_levels = 0) {
    ...
    template <class T1, class T2, class T3>
    contours_handle contour(const IterableIterables<T1> &X, const IterableIterables<T2> &Y,
                            const IterableIterables<T3> &Z, size_t n_levels = 0, const std::string &line_spec = "") {
        return contour(to_vector_2d(X), to_vector_2d(Y), to_vector_2d(Z), n_levels, line_spec);
    }

    template <class T1, class T2, class T3>
    contours_handle contour(const IterableIterables<T1> &X, const IterableIterables<T1> &Y,
    ...
    template <class T1, class T2, class T3, class T4>
    contours_handle contourf(const IterableIterables<T1> &X, const IterableIterables<T2> &Y,
                             const IterableIterables<T3> &Z, const IterableValues<T4> &levels,
                             const std::string &line_spec = "", size_t n_levels = 0) {
    ...
    template <class T1>
    contours_handle fcontour(fcontour_function_type fn, const std::array<double, 4> &xy_range,
                             IterableValues<T1> levels = {}, const std::string &line_spec = "", size_t n_levels = 0) {
        return fcontour(fn, xy_range, to_vector_1d(levels), line_spec, n_levels);
    }

    template <class T1, class T2>
    vectors_handle feather(const IterableValues<T1> &u, const IterableValues<T2> &v,
                           const std::string &line_spec = "") {
        return feather(to_vector_1d(u), to_vector_1d(v), line_spec);
    }

    template <class T1, class T2, class T3, class T4>
    std::enable_if_t<is_iterable_value_v<T1> && is_iterable_value_v<T2> && is_iterable_value_v<T3> &&
                         is_iterable_value_v<T4>,
                     vectors_handle>
    quiver(const IterableValues<T1> &x, const IterableValues<T2> &y, const IterableValues<T3> &u,
           const IterableValues<T4> &v, double scale = 1.0, const std::string &line_spec = "") {
        return quiver(to_vector_1d(x), to_vector_1d(y), to_vector_1d(u), to_vector_1d(v), scale, line_spec);
    }

    template <class T1, class T2, class T3, class T4>
    std::enable_if_t<is_iterable_iterable_v<T1> && is_iterable_iterable_v<T2> && is_iterable_iterable_v<T3> &&
                         is_iterable_iterable_v<T4>,
                     vectors_handle>
    quiver(const IterableIterables<T1> &x, const IterableIterables<T2> &y, const IterableIterables<T3> &u,
           const IterableIterables<T4> &v, double scale = 1.0, const std::string &line_spec = "") {
        return quiver(to_vector_2d(x), to_vector_2d(y), to_vector_2d(u), to_vector_2d(v), scale, line_spec);
    }

    template <class T1, class T2, class T3, class T4, class T5, class T6>
    std::enable_if_t<is_iterable_value_v<T1> && is_iterable_value_v<T2> && is_iterable_value_v<T3> &&
                         is_iterable_value_v<T4> && is_iterable_value_v<T5> && is_iterable_value_v<T6>,
                     vectors_handle>
    quiver3(const IterableValues<T1> &x, const IterableValues<T2> &y, const IterableValues<T3> &z,
            const IterableValues<T4> &u, const IterableValues<T5> &v, const IterableValues<T6> &w, double scale = 1.0,
            const std::string &line_spec = "") {
        return quiver3(to_vector_1d(x), to_vector_1d(y), to_vector_1d(z), to_vector_1d(u), to_vector_1d(v),
                       to_vector_1d(w), scale, line_spec);
    }

    template <class T1, class T2, class T3, class T4, class T5, class T6>
    std::enable_if_t<is_iterable_iterable_v<T1> && is_iterable_iterable_v<T2> && is_iterable_iterable_v<T3> &&
                         is_iterable_iterable_v<T4> && is_iterable_iterable_v<T5> && is_iterable_iterable_v<T6>,
                     vectors_handle>
    quiver3(const IterableIterables<T1> &x, const IterableIterables<T2> &y, const IterableIterables<T3> &z,
            const IterableIterables<T4> &u, const IterableIterables<T5> &v, const IterableIterables<T6> &w,
            double scale = 1.0, const std::string &line_spec = "") {
        return quiver3(to_vector_2d(x), to_vector_2d(y), to_vector_2d(z), to_vector_2d(u), to_vector_2d(v),
                       to_vector_2d(w), scale, line_spec);
    }

    template <class T1, class T2, class T3, class T4>
    vectors_handle quiver3(const IterableIterables<T1> &z, const IterableIterables<T2> &u,
                           const IterableIterables<T3> &v, const IterableIterables<T4> &w, double scale = 1.0,
                           const std::string &line_spec = "") {
        return quiver3(to_vector_2d(z), to_vector_2d(u), to_vector_2d(v), to_vector_2d(w), scale, line_spec);
    }

    template <class T1, class T2, class T3, class T4>
    surface_handle fence(const IterableIterables<T1> &X, const IterableIterables<T2> &Y, const IterableIterables<T3> &Z,
                         const IterableValues<T4> &c = {}) {
        return fence(to_vector_2d(X), to_vector_2d(X), to_vector_2d(X), to_vector_1d(c));
    }

    template <class T1, class T2, class T3, class T4>
    surface_handle mesh(const IterableIterables<T1> &X, const IterableIterables<T2> &Y, const IterableIterables<T3> &Z,
                        const IterableIterables<T4> &C = {}) {
        return mesh(to_vector_2d(X), to_vector_2d(Y), to_vector_2d(Z), to_vector_2d(C));
    }

    template <class T1, class T2, class T3, class T4>
    surface_handle meshc(const IterableIterables<T1> &X, const IterableIterables<T2> &Y, const IterableIterables<T3> &Z,
                         const IterableIterables<T4> &C = {}) {
        return meshc(to_vector_2d(X), to_vector_2d(Y), to_vector_2d(Z), to_vector_2d(C));
    }

    template <class T1, class T2, class T3, class T4>
    surface_handle meshz(const IterableIterables<T1> &X, const IterableIterables<T2> &Y, const IterableIterables<T3> &Z,
                         const IterableIterables<T4> &C = {}) {
        return meshz(to_vector_2d(X), to_vector_2d(Y), to_vector_2d(Z), to_vector_2d(C));
    }

    template <class T1, class T2, class T3, class T4>
    surface_handle ribbon(const IterableIterables<T1> &X, const IterableIterables<T2> &Y,
                          const IterableIterables<T3> &Z, const IterableIterables<T4> &C = {}, double width = 0.75) {
        return ribbon(to_vector_2d(X), to_vector_2d(Y), to_vector_2d(Z), to_vector_2d(C), width);
    }

    template <class T1, class T2, class T3, class T4>
    surface_handle surf(const IterableIterables<T1> &X, const IterableIterables<T2> &Y, const IterableIterables<T3> &Z,
    ...
    template <class T1, class T2, class T3, class T4>
    surface_handle surfc(const IterableIterables<T1> &X, const IterableIterables<T2> &Y, const IterableIterables<T3> &Z,
                         const IterableIterables<T4> &C = {}) {
        return surfc(to_vector_2d(X), to_vector_2d(Y), to_vector_2d(Z), to_vector_2d(C));
    }

    template <class T1, class T2, class T3, class T4>
    surface_handle waterfall(const IterableIterables<T1> &X, const IterableIterables<T2> &Y,
                             const IterableIterables<T3> &Z, const IterableIterables<T4> &C = {}) {
        return waterfall(to_vector_2d(X), to_vector_2d(Y), to_vector_2d(Z), to_vector_2d(C));
    }

    template <class T1, class T2>
    network_handle graph(const IterableValues<T1> &edges, const IterableValues<T2> &weights = {}, size_t n_vertices = 0,
    ...
    template <class T1, class T2, class T3, class T4>
    matrix_handle imshow(const IterableIterables<T1> &r_channel, const IterableIterables<T2> &g_channel,
                         const IterableIterables<T3> &b_channel,
                         const IterableIterables<T4> &a_channel = std::vector<std::vector<unsigned char>>{}) {
        return imshow(to_vector_2d<T1, unsigned char>(r_channel), to_vector_2d<T1, unsigned char>(g_channel),
                      to_vector_2d<T1, unsigned char>(b_channel), to_vector_2d<T1, unsigned char>(a_channel));
    }
    ...
    template <class T1, class T2, class T3>
    matrix_handle image(const IterableIterables<T1> &r_channel, const IterableIterables<T2> &g_channel,
                        const IterableIterables<T3> &b_channel, bool scaled_colorbar = false) {
        return image(to_vector_2d(r_channel), to_vector_2d(g_channel), to_vector_2d(b_channel), scaled_colorbar);
    }

    template <class T1>
    matrix_handle image(double x_min, double x_max, double y_min, double y_max, const IterableIterables<T1> &C,
                        bool scaled_colorbar = false) {
        return image(x_min, x_max, y_min, y_max, to_vector_2d(C), scaled_colorbar);
    }

    template <class T1, class T2, class T3>
    matrix_handle image(const std::tuple<IterableIterables<T1>, IterableIterables<T2>, IterableIterables<T3>> &img) {
        std::tuple<std::vector<std::vector<double>>, std::vector<std::vector<double>>, std::vector<std::vector<double>>>
            t = std::make_tuple(to_vector_2d(std::get<0>(img)), to_vector_2d(std::get<1>(img)),
                                to_vector_2d(std::get<2>(img)));
        return image(t);
    }

    template <class T1, class T2>
    labels_handle text(const IterableValues<T1> &x, const IterableValues<T2> &y,
                       const std::vector<std::string> &texts) {
        return text(to_vector_1d(x), to_vector_1d(y), texts);
    }

    template <class T1, class T2>
    labels_handle text(const IterableValues<T1> &x, const IterableValues<T2> &y, const std::string &str) {
        return text(to_vector_1d(x), to_vector_1d(y), str);
    }

    template <class T1, class T2>
    line_handle fill(const IterableValues<T1> &x, const IterableValues<T2> &y, const std::string &line_spec = "") {
