...
line_spec::line_spec(const std::string &expr) : marker_style_(marker_style::none) { parse_string(expr); }
...
std::string line_spec::plot_string(style_to_plot sty, bool include_style) {
    // plot cos(x) with linespoints linecolor rgb "#000000" dashtype 3 linewidth 3
    // linetype 4
    std::string res;
    if (include_style) {
        switch (sty) {
...
bool line_spec::has_marker_face() { return has_non_custom_marker() && marker_face_; }

bool line_spec::line_and_marker_are_the_same_color() const {
    return std::equal(color_.begin(), color_.end(), marker_color_.begin()) && marker_and_face_are_the_same_color();
}
...
void line_spec::color(const std::string &c) { color(to_array(string_to_color(c))); }
...
enum line_spec::line_style line_spec::line_style() const { return line_style_; }
...
enum line_spec::marker_style line_spec::marker_style() const { return marker_style_; }
...
enum line_spec::marker_style line_spec::marker() const { return marker_style(); }

const std::string &line_spec::custom_marker() const { return custom_marker_; }
...
const std::array<float, 4> &line_spec::marker_color() const { return marker_color_; }
...
void line_spec::marker_user_color(bool user_color) { marker_user_color_ = user_color; }

const std::array<float, 4> &line_spec::marker_face_color() const { return marker_face_color_; }
...
void line_spec::marker_face_color(const std::string &color) { marker_face_color(to_array(color)); }

void line_spec::marker_face_color(enum color c) { marker_face_color(to_array(c)); }

void line_spec::marker_face_alpha(float alpha) { marker_face_color_[0] = alpha; }

bool line_spec::marker_face_user_color() const { return marker_face_user_color_; }

void line_spec::marker_face_user_color(bool user_color) { marker_face_user_color_ = user_color; }
