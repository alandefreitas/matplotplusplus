...
std::array<float, 4> figure::calculate_subplot_position(size_t rows,
  ...
  std::array<float, 4> outer_pos{
      axes::default_axes_position[0] + min_col * width - subplot_inset[0],
  ...
  if (max_col == cols - 1) {
    subplot_inset[2] = std::max(0.f, 1.f - axes::default_axes_position[0] -
                                         axes::default_axes_position[2]);
    outer_pos[2] = 1 - outer_pos[0];
  }
  if (max_row == rows - 1) {
    subplot_inset[3] = std::max(0., 1. - axes::default_axes_position[1] -
...
axes_handle figure::add_subplot(size_t rows, size_t cols, size_t plot_id,
  ...
  return this->add_subplot(calculate_subplot_position(rows, cols, plot_id),
...
axes_handle figure::nexttile() {
  ...
  if (next_tile_fits || !tiledlayout_flow_) {
    ...
    axes_handle h = this->add_subplot(tiledlayout_rows_, tiledlayout_cols_,
                                      current_tile_index_);
    current_tile_index_++;
    return h;
  } else {
    ...
    for (size_t j = 0; j < f->children().size(); ++j) {
      auto p =
          calculate_subplot_position(tiledlayout_rows_, tiledlayout_cols_, j);
...
std::tuple<std::vector<std::vector<scatter_handle>>,
           std::vector<histogram_handle>, std::vector<std::vector<axes_handle>>>
figure::plotmatrix(const std::vector<std::vector<double>>& X,
  ...
  if (histogram_on_diagonals) {
    ...
    for (size_t i = 0; i < X.size(); ++i) {
      histogram_handle h =
          std::dynamic_pointer_cast<histogram>(*axs[0][0]->children().begin());
      H_min[i] = *h->bin_edges().begin();
      H_max[i] = *std::prev(h->bin_edges().end());
    }
    for (size_t i = 0; i < axs.size(); ++i) {
      for (size_t j = 0; j < axs.size(); ++j) {
        axs[i][j]->x_axis().limits(
            {std::min(X_min[j], H_min[j]), std::max(X_max[j], H_max[j])});
      }
    }
  }
