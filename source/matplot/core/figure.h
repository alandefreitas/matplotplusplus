...
namespace matplot {
    ...
    public /* manage axes */:
        ...
        /// \param replace_if_same_position Replace any axes that are exactly in the same position
        /// \return Handle to new axes
        axes_handle add_axes(std::shared_ptr<class axes> ax, bool replace_if_overlap, bool replace_if_same_position);

        /// \brief Add a subplot
        /// The subplot is an axes with its position determined by the number of rows, columns, and id
        axes_handle add_subplot(size_t rows, size_t cols, size_t plot_id, bool replace_if_same_position = false);

        /// \brief Make axes a subplot of this figure
        axes_handle add_subplot(size_t rows, size_t cols, size_t plot_id, axes_handle axes);

        /// \brief Create new axes in a subplot covering all positions in P
        axes_handle add_subplot(size_t rows, size_t cols, std::initializer_list<size_t> P, bool replace_if_same_position = false);

        /// \brief Create new subplot in a specific position
        axes_handle add_subplot(std::array<float,4> position, bool replace_if_same_position = false);
        ...
        /// The position of old tiled is adjusted to the new number of rows and columns.
    ...
    protected:
        static std::array<float,4> calculate_subplot_position(size_t rows, size_t cols, size_t plot_id);
    ...
    public /* plot functions for figure */:
        ...
        plotmatrix(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y, const std::string &line_spec = "of", bool histogram_on_diagonals = false);

        /// Create matrix of axes with scatter plots - X / X
        auto plotmatrix(const std::vector<std::vector<double>> &X, const std::string &line_spec = "of", bool histogram_on_diagonals = false) {
