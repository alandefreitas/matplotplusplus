...
class figure {
   ...
   public /* manage axes */:
    ...
    axes_handle add_subplot(size_t rows, size_t cols, std::initializer_list<size_t> P,
   ...
   public /* plot functions for figure */:
    ...
    plotmatrix(const std::vector<std::vector<double>> &X, const std::vector<std::vector<double>> &Y,
               const std::string &line_spec = "of", bool histogram_on_diagonals = false);

    /// Create matrix of axes with scatter plots - X / X
    auto plotmatrix(const std::vector<std::vector<double>> &X, const std::string &line_spec = "of",
