...
axes_handle subplot(size_t rows, size_t cols, size_t plot_id, bool replace_if_same_position) {
    return subplot(gcf(), rows, cols, plot_id, replace_if_same_position);
}

axes_handle subplot(figure_handle f, size_t rows, size_t cols, size_t plot_id, bool replace_if_same_position) {
...
axes_handle subplot(size_t rows, size_t cols, std::initializer_list<size_t> P, bool replace_if_same_position) {
    return gcf()->add_subplot(rows, cols, P, replace_if_same_position);
}

axes_handle subplot(figure_handle parent, std::array<float, 4> position, bool replace_if_same_position) {
    return parent->add_subplot(position, replace_if_same_position);
}

axes_handle subplot(std::array<float, 4> position, bool replace_if_same_position) {
...
legend_handle legend(const std::vector<std::string>& names) { return legend(gca(), names); }
...
void view(axes_handle ax, float azimuth, float elevation) { ax->view(azimuth, elevation); }

void rotate(float azimuth, float elevation) { rotate(gca(), azimuth, elevation); }

void rotate(axes_handle ax, float azimuth, float elevation) { ax->rotate(azimuth, elevation); }

std::pair<float, float> view(float x, float y, float z) { return view(gca(), x, y, z); }
...
void sgtitle(axes_handle ax, const std::string& str) { ax->parent()->title(str); }
...
void xtickformat(axes_handle ax, const std::string& str) { ax->xtickformat(str); }

void ytickformat(const std::string& str) { ytickformat(gca(), str); }

void ytickformat(axes_handle ax, const std::string& str) { ax->ytickformat(str); }

void ztickformat(const std::string& str) { ztickformat(gca(), str); }

void ztickformat(axes_handle ax, const std::string& str) { ax->ztickformat(str); }
...
void xticks(axes_handle ax, const std::vector<double>& ticks) { ax->xticks(ticks); }

void yticks(const std::vector<double>& ticks) { yticks(gca(), ticks); }

void yticks(axes_handle ax, const std::vector<double>& ticks) { ax->yticks(ticks); }

void y2ticks(const std::vector<double>& ticks) { y2ticks(gca(), ticks); }

void y2ticks(axes_handle ax, const std::vector<double>& ticks) { ax->y2ticks(ticks); }

void zticks(const std::vector<double>& ticks) { zticks(gca(), ticks); }

void zticks(axes_handle ax, const std::vector<double>& ticks) { ax->zticks(ticks); }

void xticks(std::initializer_list<double> ticks) { xticks(std::vector<double>(ticks)); }

void xticks(axes_handle ax, std::initializer_list<double> ticks) { xticks(ax, std::vector<double>(ticks)); }

void yticks(std::initializer_list<double> ticks) { yticks(std::vector<double>(ticks)); }

void yticks(axes_handle ax, std::initializer_list<double> ticks) { yticks(ax, std::vector<double>(ticks)); }

void y2ticks(std::initializer_list<double> ticks) { y2ticks(std::vector<double>(ticks)); }

void y2ticks(axes_handle ax, std::initializer_list<double> ticks) { y2ticks(ax, std::vector<double>(ticks)); }

void zticks(std::initializer_list<double> ticks) { zticks(std::vector<double>(ticks)); }

void zticks(axes_handle ax, std::initializer_list<double> ticks) { zticks(ax, std::vector<double>(ticks)); }

void xticks(axes_handle ax, keyword_automatic_type) { ax->x_axis().tick_values_automatic(true); }

void yticks(axes_handle ax, keyword_automatic_type) { ax->y_axis().tick_values_automatic(true); }

void y2ticks(axes_handle ax, keyword_automatic_type) { ax->y2_axis().tick_values_automatic(true); }

void zticks(axes_handle ax, keyword_automatic_type) { ax->z_axis().tick_values_automatic(true); }
...
void xticklabels(const std::vector<std::string>& ticks) { xticklabels(gca(), ticks); }

void xticklabels(axes_handle ax, const std::vector<std::string>& ticks) { ax->xticklabels(ticks); }

void yticklabels(const std::vector<std::string>& ticks) { yticklabels(gca(), ticks); }

void yticklabels(axes_handle ax, const std::vector<std::string>& ticks) { ax->yticklabels(ticks); }

void y2ticklabels(const std::vector<std::string>& ticks) { y2ticklabels(gca(), ticks); }

void y2ticklabels(axes_handle ax, const std::vector<std::string>& ticks) { ax->y2ticklabels(ticks); }

void zticklabels(const std::vector<std::string>& ticks) { zticklabels(gca(), ticks); }

void zticklabels(axes_handle ax, const std::vector<std::string>& ticks) { ax->zticklabels(ticks); }

void xticklabels(std::initializer_list<std::string> ticks) { xticklabels(std::vector<std::string>(ticks)); }

void xticklabels(axes_handle ax, std::initializer_list<std::string> ticks) {
    xticklabels(ax, std::vector<std::string>(ticks));
}

void yticklabels(std::initializer_list<std::string> ticks) { yticklabels(std::vector<std::string>(ticks)); }

void yticklabels(axes_handle ax, std::initializer_list<std::string> ticks) {
    yticklabels(ax, std::vector<std::string>(ticks));
}

void y2ticklabels(std::initializer_list<std::string> ticks) { y2ticklabels(std::vector<std::string>(ticks)); }

void y2ticklabels(axes_handle ax, std::initializer_list<std::string> ticks) {
    y2ticklabels(ax, std::vector<std::string>(ticks));
}

void zticklabels(std::initializer_list<std::string> ticks) { zticklabels(std::vector<std::string>(ticks)); }

void zticklabels(axes_handle ax, std::initializer_list<std::string> ticks) {
    zticklabels(ax, std::vector<std::string>(ticks));
}

void xticklabels(axes_handle ax, keyword_automatic_type) { ax->x_axis().ticklabels_mode(false); }

void yticklabels(axes_handle ax, keyword_automatic_type) { ax->y_axis().ticklabels_mode(false); }

void y2ticklabels(axes_handle ax, keyword_automatic_type) { ax->y2_axis().ticklabels_mode(false); }

void zticklabels(axes_handle ax, keyword_automatic_type) { ax->z_axis().ticklabels_mode(false); }
...
void colormap(axes_handle ax, const std::vector<std::vector<double>>& map) { ax->colormap(map); }

void colormap(const std::vector<std::vector<double>>& map) { colormap(gca(), map); }

std::vector<std::vector<double>> colormap(axes_handle ax) { return ax->colormap(); }
...
void axis(std::initializer_list<axes_handle> handles, const std::array<double, 4>& limits_x_y) {
...
/// \brief Sets limits on current axes
void xrange(const std::array<double, 2>& limits_x_y) { xrange(gca(), limits_x_y); }
...
/// \brief Sets limits on current axes
void yrange(const std::array<double, 2>& limits_x_y) { yrange(gca(), limits_x_y); }
