...
namespace matplot {
    ...
    void tiledlayout(size_t rows) {
        gcf()->tiledlayout(rows,1);
    }
    ...
    axes_handle nexttile(size_t index) {
        return gcf()->nexttile(index);
    }

    /// \brief Get the current axes
    axes_handle gca() {
        return gcf()->current_axes();
    }

    void cla(axes_handle ax) {
        ax->clear();
    }
    ...
    legend_handle legend(bool visible) {
        return legend(gca(), visible);
    }
    ...
    void hold(bool v){
        gca()->next_plot_replace(!v);
    }

    void hold(axes_handle ax, bool v){
    ...
    void box(axes_handle ax, bool v){
        ax->box(v);
    }
    ...
    void xticklabels(keyword_automatic_type) {
        xticklabels(gca(), automatic);
    }

    void yticklabels(keyword_automatic_type) {
        yticklabels(gca(), automatic);
    }
    ...
    void ztickangle(axes_handle ax, double degrees) {
        ax->ztickangle(degrees);
    }

    void xtickangle(double degrees) {
        xtickangle(gca(),degrees);
    }

    void ytickangle(double degrees) {
        ytickangle(gca(),degrees);
    }

    void y2tickangle(double degrees) {
        y2tickangle(gca(),degrees);
    }

    void ztickangle(double degrees) {
        ztickangle(gca(),degrees);
    }

    double xtickangle(axes_handle ax) {
        return ax->xtickangle();
    }

    double ytickangle(axes_handle ax) {
        return ax->ytickangle();
    }

    double y2tickangle(axes_handle ax) {
        return ax->y2tickangle();
    }

    double ztickangle(axes_handle ax) {
        return ax->ztickangle();
    }

    double xtickangle() {
        return xtickangle(gca());
    }
    double ytickangle() {
        return ytickangle(gca());
    }
    double y2tickangle() {
        return y2tickangle(gca());
    }
    double ztickangle() {
        return ztickangle(gca());
    }
    ...
    void axis(axes_handle ax, keyword_tight_type) {
        ...
        if (ax->is_3d() && !ax->is_3d_map()) {
            auto zminit = std::min_element(ax->children().begin(), ax->children().end(),
                                           [](axes_object_handle a, axes_object_handle b) {
                                               return a->zmin() < b->zmin();
                                           });
            auto zmaxit = std::max_element(ax->children().begin(), ax->children().end(),
                                           [](axes_object_handle a, axes_object_handle b) {
