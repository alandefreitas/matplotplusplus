...
namespace matplot {
    ...
    histcounts(const std::vector<double>& data, enum histogram::binning_algorithm binning_alg, enum histogram::normalization normalization_alg) {
        ...
        return std::make_pair(
                histogram::histogram_normalize(bin_counts, edges, data.size(), normalization_alg),
                edges);
    }
    ...
    std::pair<std::vector<double>,std::vector<double>>
    histcounts(const std::vector<double>& data, enum histogram::normalization normalization_alg) {
        return histcounts(data, histogram::binning_algorithm::automatic, normalization_alg);
    }
    ...
    /// \return pair with <values,edges>
    std::pair<std::vector<double>,std::vector<double>>
    histcounts(const std::vector<double>& data, size_t nbins, enum histogram::normalization normalization_alg) {
        double minx = *std::min_element(data.begin(), data.end());
        double maxx = *std::max_element(data.begin(), data.end());
    ...
    std::vector<double>
    histcounts(const std::vector<double>& data, const std::vector<double>& edges, enum histogram::normalization normalization_alg) {
        auto bin_counts = histogram::histogram_count(data, edges);
        return histogram::histogram_normalize(bin_counts, edges, data.size(), normalization_alg);
    }

    /// Normalize the number of points in a bin
    std::vector<std::vector<double>>
    histnormalize2(const std::vector<std::vector<size_t>>& bin_count, const std::vector<double>& xbin_edges, const std::vector<double>& ybin_edges, size_t data_size, enum histogram::normalization normalization_algorithm){
        std::vector<std::vector<double>> values(bin_count.size(), std::vector<double>(bin_count[0].size(),0));
            ...
            case histogram::normalization::pdf:
                for (size_t i = 0; i < bin_count.size(); ++i) {
                    for (size_t j = 0; j < bin_count[0].size(); ++j) {
                        ...
                        const double y_bin_width_i = ybin_edges[j+1]-ybin_edges[j];
                        const double bin_area_i = x_bin_width_i*y_bin_width_i;
                        values[i][j] = static_cast<double>(bin_count[i][j])/(bin_area_i * data_size);
                    }
                }
                break;
                ...
                for (size_t i = 0; i < bin_count.size(); ++i) {
                    ...
                    for (size_t j = 1; j < bin_count[0].size(); ++j) {
                        if (i == 0) {
                            ...
                            values[i][j] = (bin_count[i][j] + line_sum) / data_size + values[i-1][j];
                        }
                        line_sum += bin_count[i][j];
                    }
                }
    ...
    std::vector<std::vector<double>>
    histcounts2(const std::vector<double>& x_data, const std::vector<double>& y_data, const std::vector<double>& x_edges, const std::vector<double>& y_edges, enum histogram::normalization normalization_algorithm) {
        std::vector<std::vector<size_t>> bin_counts(x_edges.size()-1, std::vector<size_t>(y_edges.size()-1,0));
        ...
        return histnormalize2(bin_counts, x_edges, y_edges, x_data.size(), normalization_algorithm);
    }
}