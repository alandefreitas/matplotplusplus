...
struct has_iterable_value_type {
  ...
  static true_type check(
      int, typename T::value_type::value_type = *(C().begin()->begin()));
...
struct is_iterable_value
    : public std::integral_constant<bool, is_iterable_v<C> &&
                                              !has_iterable_value_type_v<C>> {};
...
template <class T>
using is_string =
    std::integral_constant<bool, std::is_same_v<std::decay_t<T>, std::string> &&
...
struct iterable_traits {
  ...
  using key_type =
      std::conditional_t<is_map, typename value_type::first_type, value_type>;
  using mapped_type =
      std::conditional_t<is_map, typename value_type::second_type, value_type>;
};
}  // namespace matplot
