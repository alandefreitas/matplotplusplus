...
bool is_true(const std::string& str) { return iequals(str, "on") || iequals(str, "true") || iequals(str, "yes"); }

bool is_false(const std::string& str) { return iequals(str, "off") || iequals(str, "false") || iequals(str, "no"); }
...
std::string escape(const std::string& label) { return std::regex_replace(label, std::regex("\""), "\\\""); }
...
std::vector<double> linspace(double d1, double d2) { return linspace(d1, d2, 100); }

std::vector<double> logspace(double d1, double d2, size_t n) {
    return transform(linspace(d1, d2, n), [](double x) { return pow(10, x); });
}

std::vector<double> logspace(double d1, double d2) { return logspace(d1, d2, 50); }
...
std::vector<double> transform(const std::vector<double>& x, const std::vector<double>& y,
...
double vector_radians(double x, double y) { return to_positive_radian(atan2(y, x)); }

double vector_magnitude(double x, double y) { return sqrt(pow(x, 2) + pow(y, 2)); }

double vector_radians(std::complex<double> v) { return vector_radians(v.real(), v.imag()); }

double vector_magnitude(std::complex<double> v) { return vector_magnitude(v.real(), v.imag()); }
...
std::pair<vector_1d, vector_1d> pol2cart(vector_1d theta, vector_1d rho) {
    return std::make_pair(transform(theta, rho, [](double theta, double rho) { return rho * cos(theta); }),
...
std::pair<vector_1d, vector_1d> pol2cart(vector_1d theta, double rho) {
    return std::make_pair(transform(theta, [&](double theta) { return rho * cos(theta); }),
...
std::pair<vector_2d, vector_2d> meshgrid(const vector_1d& x_and_y) { return meshgrid(x_and_y, x_and_y); }
...
std::pair<vector_2d, vector_2d> gradient(const vector_2d& z, double spacing) { return gradient(z, spacing, spacing); }
...
vector_2d zeros(size_t rows, size_t cols) { return vector_2d(rows, vector_1d(cols, 0.)); }

vector_2d ones(size_t rows, size_t cols) { return vector_2d(rows, vector_1d(cols, 0.)); }
...
image_channels_t gray2rgb(const image_channels_t& A) { return gray2rgb(A[0]); }
...
std::pair<std::vector<std::string>, std::vector<size_t>> wordcount(const std::vector<std::string>& tokens,
...
std::pair<std::vector<std::string>, std::vector<size_t>> wordcount(const std::string& text,
                                                                   const std::vector<std::string>& black_list,
                                                                   const std::string& delimiters,
                                                                   size_t max_cloud_size) {
...
ticks_results calcticks(double limits_min, double limits_max, bool horizontal, double text_size, bool separateExp,
                        bool log) {
    ...
    std::vector<double> normRoughInts =
        transform(roughInts, decRoughInts, [](double x, double y) { return x / pow(10., y); });
    ...
    std::vector<double> maxAbs =
        transform(lLims, uLims, [](double x, double y) { return abs(x) > abs(y) ? abs(x) : abs(y); });
    ...
    std::vector<double> labelChars =
        transform(decInts, decMax, [&](double x, double y) { return max(y + 1, 1.) + ((x < 0) ? (1 - x) : 0.); });
    ...
    std::vector<double> nMaxScore =
        transform(nMax, [&](double x) { return min(x, initMaxTicks + 0.25 * (x - initMaxTicks)); });
    // Calculate a score based on the number of ticks relative to the maximum.
    std::vector<double> nTickScore =
        transform(nTicks, nMaxScore, [](double x, double y) { return 1 - pow((x / y - 0.7), 2); });
