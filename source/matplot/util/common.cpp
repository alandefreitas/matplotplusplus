...
std::pair<vector_1d, vector_1d> elliptic2cart(vector_1d theta, vector_1d rho_w,
                                              vector_1d rho_h) {
...
std::pair<vector_1d, vector_1d> pol2cart(vector_1d theta, vector_1d rho) {
  return std::make_pair(
      ...
      transform(theta, rho,
                [](double theta, double rho) { return rho * sin(theta); }));
}
...
std::pair<vector_2d, vector_2d> gradient(const vector_2d& z, double spacing_x,
                                         double spacing_y) {
  ...
  for (size_t row = 0; row < n1; ++row) {
    ...
    for (size_t col = 1; col < n2 - 1; ++col) {
      dx[row][col] =
          ((z[row][col] - z[row][col - 1]) + (z[row][col + 1] - z[row][col])) /
          spacing_x / 2.;
    }
  }
  for (size_t col = 0; col < n2; ++col) {
    ...
    for (size_t row = 1; row < n1 - 1; ++row) {
      dy[row][col] =
          ((z[row][col] - z[row - 1][col]) + (z[row + 1][col] - z[row][col])) /
          spacing_y / 2.;
    }
  }
...
image_channels_t cimg2channels(
    const cimg_library::CImg<unsigned char>& cimg_image) {
...
cimg_library::CImg<unsigned char> channels2cimg(const image_channels_t& A) {
  cimg_library::CImg<unsigned char> cimg_image(A[0].size(), A[0][0].size(), 1,
                                               A.size());
...
std::pair<std::vector<std::string>, std::vector<size_t>> wordcount(
    const std::vector<std::string>& tokens,
    const std::vector<std::string>& black_list, const std::string& delimiters,
  ...
  for (const auto& token : tokens) {
    const bool is_in_black_list =
        bl_sorted
            ...
            : std::find(black_list.begin(), black_list.end(), token) !=
                  black_list.end();
    if (!is_in_black_list) {
...
std::pair<std::vector<std::string>, std::vector<size_t>> wordcount(
    const std::string& text, const std::vector<std::string>& black_list,
    const std::string& delimiters, size_t max_cloud_size) {
  auto tokens = tokenize(text);
  return wordcount(tokens, black_list, delimiters, max_cloud_size);
}

ticks_results calcticks(double limits_min, double limits_max, bool horizontal,
                        double text_size, bool separateExp, bool log) {
  ...
  if (log) {
    std::invalid_argument(
        "Not implemented yet. The library does not need that as it is.");
  }
  ...
  std::vector<double> roughInts =
      transform(testTickCounts, [&](double x) { return range / (x - 1); });
  ...
  std::vector<double> lLims =
      transform(niceInts, [&](double x) { return floor(limits_min / x) * x; });
  std::vector<double> uLims =
      transform(niceInts, [&](double x) { return ceil(limits_max / x) * x; });
  ...
  std::vector<double> decMax =
      transform(maxAbs, [](double x) { return floor(log10(x)); });  // - nDec;
  std::vector<double> decInts =
      transform(niceInts, [](double x) { return floor(log10(x)); });  // - nDec;
  ...
  if (separateExp) {
    for (size_t i = 0; i < decInts.size(); ++i) {
      ...
      // Small numbers:
      isExp[i] =
          isExp[i] || (decMax[i] < -3) || (decMax[i] == -3 && decInts[i] < -5);
    }
  } else {
    for (size_t i = 0; i < decInts.size(); ++i) {
      // Large numbers:
  ...
  if (separateExp) {
    ...
    expChars = transform(decInts, decMax, [&](double x, double y) {
      return 2 +
             max(0., min(max(0., static_cast<double>(maxChars) - 7), y - x)) +
             4;
    });
  }
  ...
  if (horizontal) {
    labelSize = transform(
        labelChars, [&](double labelChars) { return text_size * labelChars; });
  } else {
    labelSize =
        transform(labelChars, [&](double labelChars) { return text_size; });
  }
  ...
  std::vector<double> nTickScore =
      transform(nTicks, nMaxScore,
                [](double x, double y) { return 1 - pow((x / y - 0.7), 2); });
  // Severe penalty for more than nMax ticks
  std::vector<double> penalty =
      transform(nTicks, nMax, [](double x, double y) { return 4 * x / y; });
  for (size_t i = 0; i < nTicks.size(); ++i) {
  ...
  for (size_t i = 0; i < hitsEnds.size(); ++i) {
    hitsEnds[i] = (abs(lLims[i] - limits_min) < 100 * lEps) &&
                  (abs(uLims[i] - limits_max) < 100 * uEps);
  }
  ...
  if (isExp[bestIdx] && separateExp) {
    ...
    int n = max(
        1., min(max(0., static_cast<double>(maxChars) - 3), decMax_d - decInt));
    std::string fStr = "%." + num2str(n) + 'f';
    // Normalize the ticks to the correct scale and create the labels
    std::vector<double> normTicks =
        transform(ticks, [&](double x) { return x / pow(10, decMax_d); });
    ...
    // Determine maximum number of decimals and set formatting string
    int n = max(
        0., min(max(0., static_cast<double>(maxChars) - 7), decMax_d - decInt));
