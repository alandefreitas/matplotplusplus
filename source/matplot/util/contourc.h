...
class QuadContourGenerator {
   public:
    ...
    // https://github.com/matplotlib/matplotlib/blob/master/lib/matplotlib/contour.py
    QuadContourGenerator(const vector_2d &x, const vector_2d &y, const vector_2d &z, bool corner_mask, long chunk_size);
   ...
   private:
    ...
    void append_contour_line_to_vertices(ContourLine &contour_line, vertices_list_type &vertices_list) const;
    ...
    void append_contour_to_vertices_and_codes(Contour &contour, vertices_list_type &vertices_list,
    ...
    unsigned int follow_boundary(ContourLine &contour_line, QuadEdge &quad_edge, const double &lower_level,
    ...
    void follow_interior(ContourLine &contour_line, QuadEdge &quad_edge, unsigned int level_index, const double &level,
    ...
    void get_chunk_limits(long ijchunk, long &ichunk, long &jchunk, long &istart, long &iend, long &jstart, long &jend);
    ...
    void single_quad_filled(Contour &contour, long quad, const double &lower_level, const double &upper_level);
    ...
    ContourLine *start_filled(long quad, Edge edge, unsigned int start_level_index, HoleOrNot hole_or_not,
    ...
    // VISITED(quad,1).
    bool start_line(vertices_list_type &vertices_list, long quad, Edge edge, const double &level);
...
/// \return     Consecutive indices of vertices of segment in *xys* where *proj*
/// is.
...
std::vector<contour_line_type> contourc(const vector_2d &z, size_t n_levels = 7);
