...
class QuadContourGenerator {
 public:
  ...
  QuadContourGenerator(const vector_2d &x, const vector_2d &y,
                       const vector_2d &z, bool corner_mask, long chunk_size);
  ...
  std::pair<vertices_list_type, codes_list_type> create_filled_contour(
 ...
 private:
  ...
  unsigned int follow_boundary(ContourLine &contour_line, QuadEdge &quad_edge,
  ...
  void follow_interior(ContourLine &contour_line, QuadEdge &quad_edge,
                       ...
                       bool want_initial_point, const QuadEdge *start_quad_edge,
                       unsigned int start_level_index, bool set_parents);

  // Return the index limits of a particular chunk.
  void get_chunk_limits(long ijchunk, long &ichunk, long &jchunk, long &istart,
                        long &iend, long &jstart, long &jend);
...
find_closest_point_on_path(const std::vector<double> &xs,
                           const std::vector<double> &ys, double px, double py);
