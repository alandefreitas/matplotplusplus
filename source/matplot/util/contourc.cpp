...
void ParentCache::set_parent(long quad, ContourLine &contour_line) {
  ...
  if (_lines[index] == 0)
    _lines[index] =
        (contour_line.is_hole() ? contour_line.get_parent() : &contour_line);
}
...
void QuadContourGenerator::append_contour_line_to_vertices(
  ...
  for (ContourLine::const_iterator point = contour_line.begin();
    ...
    bool is_x_jump_to_origin = is_origin && inserted != 0 &&
                               abs(vertices_list.first.back()) > 3 * x_diff;
    bool is_y_jump_to_origin = is_origin && inserted != 0 &&
                               abs(vertices_list.second.back()) > 3 * y_diff;
    ...
    if (!is_end_of_segment) {
      ...
      bool last_segment_is_not_empty = inserted > 0 &&
                                       isfinite(vertices_list.first.back()) &&
                                       isfinite(vertices_list.first.back());
...
QuadContourGenerator::vertices_list_type QuadContourGenerator::create_contour(
    const double &level) {
  ...
  for (long ijchunk = 0; ijchunk < _chunk_count; ++ijchunk) {
    ...
    for (long j = jstart; j < jend; ++j) {
      ...
      for (long quad = istart + j * _nx; quad < quad_end; ++quad) {
        ...
        follow_interior(contour_line, quad_edge, 1, level, !ignore_first,
                        &start_quad_edge, 1, false);
...
unsigned int QuadContourGenerator::follow_boundary(
    ...
    const double &upper_level, unsigned int level_index,
  ...
  while (true) {
    ...
    if (stop) {
      ...
      contour_line.push_back(
          edge_interp(quad_edge, level_index == 1 ? lower_level : upper_level));
      break;
    }
  ...
  while (true) {
    ...
    if (_cache[quad] & saddle_mask) {
      ...
      if (config == 1) {
        ...
        double zmid = 0.25 * (get_point_z(POINT_SW) + get_point_z(POINT_SE) +
        ...
        if ((zmid > level) ^ (level_index == 2)) {
          ...
          dir = Dir_Left;
          _cache[quad] |=
              (level_index == 1 ? MASK_SADDLE_LEFT_1 : MASK_SADDLE_LEFT_2);
        }
        ...
        dir =
            (config == 0 ? Dir_Left : (config == 3 ? Dir_Right : Dir_Straight));
        _cache[quad] |= visited_mask;
      }
    }
...
void QuadContourGenerator::single_quad_filled(Contour &contour, long quad,
  ...
  if (BOUNDARY_N(quad) && EXISTS_N_EDGE(quad) && !VISITED_S(quad + _nx) &&
      Z_NW == 1 && Z_NE == 1)
    contour.push_back(start_filled(quad, Edge_N, 1, Hole, Boundary, lower_level,
                                   upper_level));
}

ContourLine *QuadContourGenerator::start_filled(
    long quad, Edge edge, unsigned int start_level_index, HoleOrNot hole_or_not,
  ...
  while (true) {
    if (boundary_or_interior == Interior) {
      ...
      follow_interior(*contour_line, quad_edge, level_index, level, false,
                      &start_quad_edge, start_level_index, true);
    } else {
      level_index = follow_boundary(*contour_line, quad_edge, lower_level,
                                    upper_level, level_index, start_quad_edge);
    }

    if (quad_edge == start_quad_edge &&
        (boundary_or_interior == Boundary || level_index == start_level_index))
      break;
...
void QuadContourGenerator::write_cache_quad(long quad, bool grid_only) const {
  ...
  if (!grid_only) {
    std::cout << " Z=" << Z_LEVEL(quad) << " SAD=" << SADDLE(quad, 1)
              ...
              << SADDLE_LEFT(quad, 2) << " NW=" << SADDLE_START_SW(quad, 1)
...
find_closest_point_on_path(const std::vector<double> &xs,
  ...
  return std::make_tuple(d2s[imin], std::make_pair(projxs[imin], projys[imin]),
...
std::vector<contour_line_type> contourc(const vector_2d &x, const vector_2d &y,
                                        const vector_2d &z,
...
std::vector<contour_line_type> contourc(const vector_2d &x, const vector_2d &y,
                                        const vector_2d &z, size_t n_levels) {
  double zmin = z[0][0];
  double zmax = z[0][0];
