...
bool QuadEdge::operator==(const QuadEdge &other) const { return quad == other.quad && edge == other.edge; }

bool QuadEdge::operator!=(const QuadEdge &other) const { return !operator==(other); }
...
bool XY::operator==(const XY &other) const { return x == other.x && y == other.y; }

bool XY::operator!=(const XY &other) const { return x != other.x || y != other.y; }

XY XY::operator*(const double &multiplier) const { return XY(x * multiplier, y * multiplier); }
...
XY XY::operator+(const XY &other) const { return XY(x + other.x, y + other.y); }

XY XY::operator-(const XY &other) const { return XY(x - other.x, y - other.y); }

std::ostream &operator<<(std::ostream &os, const XY &xy) { return os << '(' << xy.x << ' ' << xy.y << ')'; }

ContourLine::ContourLine(bool is_hole) : std::vector<XY>(), _is_hole(is_hole), _parent(0) {}
...
void ContourLine::push_back(const XY &point) {
    if (empty() || point != back()) std::vector<XY>::push_back(point);
}
...
void ContourLine::write() const {
    ...
    for (const_iterator it = begin(); it != end(); ++it) std::cout << ' ' << *it;
    if (is_hole())
        ...
        if (!_children.empty()) {
            std::cout << ", children=";
            for (Children::const_iterator it = _children.begin(); it != _children.end(); ++it) std::cout << *it << ' ';
        }
    }
    std::cout << std::endl;
}
...
void Contour::write() const {
    ...
    for (const_iterator it = begin(); it != end(); ++it) (*it)->write();
}
...
long ParentCache::quad_to_index(long quad) const {
    ...
    assert(index >= 0 && index < static_cast<long>(_lines.size()) && "ParentCache index outside chunk");
...
void ParentCache::set_parent(long quad, ContourLine &contour_line) {
    ...
    long index = quad_to_index(quad);
    if (_lines[index] == 0) _lines[index] = (contour_line.is_hole() ? contour_line.get_parent() : &contour_line);
}

QuadContourGenerator::QuadContourGenerator(const CoordinateArray &x, const CoordinateArray &y, const CoordinateArray &z,
                                           bool corner_mask, long chunk_size)
    : _x(x),
      _y(y),
      _z(z),
      ...
      _chunk_size(chunk_size > 0 ? std::min(chunk_size, std::max(_nx, _ny) - 1) : std::max(_nx, _ny) - 1),
      _nxchunk(calc_chunk_count(_nx)),
      _nychunk(calc_chunk_count(_ny)),
      _chunk_count(_nxchunk * _nychunk),
      _cache(std::vector<CacheItem>(_n)),
      _parent_cache(_nx, chunk_size > 0 ? chunk_size + 1 : _nx, chunk_size > 0 ? chunk_size + 1 : _ny) {
    assert(!_x.empty() && !_y.empty() && !_z.empty() && "Empty array");
    assert(_y.size() == _x.size() && _y[0].size() == _x[0].size() && "Different-sized y and x arrays");
    assert(_z.size() == _x.size() && _z[0].size() == _x[0].size() && "Different-sized z and x arrays");

    init_cache_grid();
}

void QuadContourGenerator::append_contour_line_to_vertices(ContourLine &contour_line,
    ...
    size_t i = 0;
    size_t inserted = 0;
    for (ContourLine::const_iterator point = contour_line.begin(); point != contour_line.end(); ++point, ++i) {
        ...
        if (!is_end_of_segment) {
            ...
            bool last_segment_is_not_empty =
                inserted > 0 && isfinite(vertices_list.first.back()) && isfinite(vertices_list.first.back());
...
void QuadContourGenerator::append_contour_to_vertices_and_codes(Contour &contour, vertices_list_type &vertices_list,
    ...
    for (Contour::iterator line_it = contour.begin(); line_it != contour.end(); ++line_it) {
        ...
        if (line.is_hole()) {
            ...
            auto is_valid_point = [](double x, double y) { return isfinite(x) && isfinite(y) && x != 0. && y != 0.; };
...
long QuadContourGenerator::calc_chunk_count(long point_count) const {
    ...
    if (_chunk_size > 0) {
        ...
        if (count * _chunk_size < point_count - 1) ++count;
...
QuadContourGenerator::vertices_list_type QuadContourGenerator::create_contour(const double &level) {
    ...
    for (long ijchunk = 0; ijchunk < _chunk_count; ++ijchunk) {
        ...
        for (long j = jstart; j < jend; ++j) {
            ...
            for (long quad = istart + j * _nx; quad < quad_end; ++quad) {
                ...
                if (BOUNDARY_S(quad) && Z_SW >= 1 && Z_SE < 1 && start_line(vertices_list, quad, Edge_S, level))
                    continue;

                if (BOUNDARY_W(quad) && Z_NW >= 1 && Z_SW < 1 && start_line(vertices_list, quad, Edge_W, level))
                    continue;

                if (BOUNDARY_N(quad) && Z_NE >= 1 && Z_NW < 1 && start_line(vertices_list, quad, Edge_N, level))
                    continue;

                if (BOUNDARY_E(quad) && Z_SE >= 1 && Z_NE < 1 && start_line(vertices_list, quad, Edge_E, level))
                    continue;
    ...
    for (long ijchunk = 0; ijchunk < _chunk_count; ++ijchunk) {
        ...
        for (long j = jstart; j < jend; ++j) {
            ...
            for (long quad = istart + j * _nx; quad < quad_end; ++quad) {
                ...
                if (start_edge == Edge_None) continue;
                ...
                follow_interior(contour_line, quad_edge, 1, level, !ignore_first, &start_quad_edge, 1, false);
                if (ignore_first && !contour_line.empty()) contour_line.push_back(contour_line.front());
                append_contour_line_to_vertices(contour_line, vertices_list);

                // Repeat if saddle point but not visited.
                if (SADDLE(quad, 1) && !VISITED(quad, 1)) --quad;
            }
        }
    }
...
QuadContourGenerator::create_filled_contour(const double &lower_level, const double &upper_level) {
    ...
    for (long ijchunk = 0; ijchunk < _chunk_count; ++ijchunk) {
        ...
        for (long j = jstart; j < jend; ++j) {
            long quad_end = iend + j * _nx;
            for (long quad = istart + j * _nx; quad < quad_end; ++quad) {
                if (!EXISTS_NONE(quad)) single_quad_filled(contour, quad, lower_level, upper_level);
            }
        }
        ...
        if (jchunk < _nychunk - 1) {
            ...
            for (long quad = istart + jend * _nx; quad < quad_end; ++quad) _cache[quad] &= ~MASK_VISITED_S;
        }

        if (ichunk < _nxchunk - 1) {
            long quad_end = iend + jend * _nx;
            for (long quad = iend + jstart * _nx; quad < quad_end; quad += _nx) _cache[quad] &= ~MASK_VISITED_W;
        }
...
XY QuadContourGenerator::edge_interp(const QuadEdge &quad_edge, const double &level) {
    assert(quad_edge.quad >= 0 && quad_edge.quad < _n && "Quad index out of bounds");
    assert(quad_edge.edge != Edge_None && "Invalid edge");
    return interp(get_edge_point_index(quad_edge, true), get_edge_point_index(quad_edge, false), level);
}

unsigned int QuadContourGenerator::follow_boundary(ContourLine &contour_line, QuadEdge &quad_edge,
    ...
    assert(start_quad_edge.quad >= 0 && start_quad_edge.quad < _n && "Start quad index out of bounds");
    ...
    while (true) {
        ...
        unsigned int start_level = (first_edge ? Z_LEVEL(get_edge_point_index(quad_edge, true)) : end_level);
        ...
        if (stop) {
            ...
            contour_line.push_back(edge_interp(quad_edge, level_index == 1 ? lower_level : upper_level));
            break;
        }
        ...
        switch (quad_edge.edge) {
            ...
            case Edge_S:
            case Edge_SE:
                if (!EXISTS_SE_CORNER(quad)) _parent_cache.set_parent(quad, contour_line);
                break;
            case Edge_E:
            case Edge_NE:
            case Edge_N:
            case Edge_NW:
                if (!EXISTS_SW_CORNER(quad)) _parent_cache.set_parent(quad + 1, contour_line);
                break;
            default:
                assert(0 && "Invalid edge");
                break;
...
void QuadContourGenerator::follow_interior(ContourLine &contour_line, QuadEdge &quad_edge, unsigned int level_index,
                                           const double &level, bool want_initial_point,
                                           const QuadEdge *start_quad_edge, unsigned int start_level_index,
                                           bool set_parents) {
    assert(quad_edge.quad >= 0 && quad_edge.quad < _n && "Quad index out of bounds.");
    assert(quad_edge.edge != Edge_None && "Invalid edge");
    assert((level_index == 1 || level_index == 2) && "level index must be 1 or 2");
    assert((start_quad_edge == 0 || (start_quad_edge->quad >= 0 && start_quad_edge->quad < _n)) &&
           "Start quad index out of bounds.");
    assert((start_quad_edge == 0 || start_quad_edge->edge != Edge_None) && "Invalid start edge");
    assert((start_level_index == 1 || start_level_index == 2) && "start level index must be 1 or 2");

    long &quad = quad_edge.quad;
    Edge &edge = quad_edge.edge;

    if (want_initial_point) contour_line.push_back(edge_interp(quad_edge, level));
    ...
    while (true) {
        ...
        if (_cache[quad] & saddle_mask) {
            ...
            if (config == 1) {
                ...
                if (edge == Edge_N || edge == Edge_E) {
                    ...
                    _cache[quad] |= (level_index == 1 ? MASK_SADDLE_START_SW_1 : MASK_SADDLE_START_SW_2);
                }
            } else {
        ...
        if (is_edge_a_boundary(quad_edge)) break;

        move_to_next_quad(quad_edge);
        assert(quad_edge.quad >= 0 && quad_edge.quad < _n && "Quad index out of bounds");

        // Return if reached start point of contour line.
        if (start_quad_edge != 0 && quad_edge == *start_quad_edge && level_index == start_level_index) break;
    }
}

void QuadContourGenerator::get_chunk_limits(long ijchunk, long &ichunk, long &jchunk, long &istart, long &iend,
                                            long &jstart, long &jend) {
...
Edge QuadContourGenerator::get_corner_start_edge(long quad, unsigned int level_index) const {
    ...
    unsigned int config = (Z_LEVEL(point1) >= level_index) << 2 | (Z_LEVEL(point2) >= level_index) << 1 |
    ...
    if (level_index == 2) config = 7 - config;
...
long QuadContourGenerator::get_edge_point_index(const QuadEdge &quad_edge, bool start) const {
    assert(quad_edge.quad >= 0 && quad_edge.quad < _n && "Quad index out of bounds");
    assert(quad_edge.edge != Edge_None && "Invalid edge");
...
Edge QuadContourGenerator::get_exit_edge(const QuadEdge &quad_edge, Dir dir) const {
    assert(quad_edge.quad >= 0 && quad_edge.quad < _n && "Quad index out of bounds");
    assert(quad_edge.edge != Edge_None && "Invalid edge");
    ...
    if (EXISTS_ANY_CORNER(quad)) {
        ...
        switch (edge) {
            case Edge_E:
                return (EXISTS_SE_CORNER(quad) ? (dir == Dir_Left ? Edge_S : Edge_NW)
                                               : (dir == Dir_Right ? Edge_N : Edge_SW));
            case Edge_N:
                return (EXISTS_NW_CORNER(quad) ? (dir == Dir_Right ? Edge_W : Edge_SE)
                                               : (dir == Dir_Left ? Edge_E : Edge_SW));
            case Edge_W:
                return (EXISTS_SW_CORNER(quad) ? (dir == Dir_Right ? Edge_S : Edge_NE)
                                               : (dir == Dir_Left ? Edge_N : Edge_SE));
            case Edge_S:
                return (EXISTS_SW_CORNER(quad) ? (dir == Dir_Left ? Edge_W : Edge_NE)
...
XY QuadContourGenerator::get_point_xy(long point) const {
    ...
    return XY(_x[0].data()[static_cast<size_t>(point)], _y[0].data()[static_cast<size_t>(point)]);
}
...
Edge QuadContourGenerator::get_quad_start_edge(long quad, unsigned int level_index) const {
    assert(quad >= 0 && quad < _n && "Quad index out of bounds");
    assert((level_index == 1 || level_index == 2) && "level index must be 1 or 2");
    assert(EXISTS_QUAD(quad) && "Quad does not exist");

    unsigned int config =
        (Z_NW >= level_index) << 3 | (Z_NE >= level_index) << 2 | (Z_SW >= level_index) << 1 | (Z_SE >= level_index);

    // Upper level (level_index == 2) polygons are reversed compared to lower
    // level ones, i.e. higher values on the right rather than the left.
    if (level_index == 2) config = 15 - config;
...
Edge QuadContourGenerator::get_start_edge(long quad, unsigned int level_index) const {
    if (EXISTS_ANY_CORNER(quad))
...
void QuadContourGenerator::init_cache_grid() {
    ...
    for (j = 0; j < _ny; ++j) {
        for (i = 0; i < _nx; ++i, ++quad) {
            ...
            if (i < _nx - 1 && j < _ny - 1) _cache[quad] |= MASK_EXISTS_QUAD;

            if ((i % _chunk_size == 0 || i == _nx - 1) && j < _ny - 1) _cache[quad] |= MASK_BOUNDARY_W;

            if ((j % _chunk_size == 0 || j == _ny - 1) && i < _nx - 1) _cache[quad] |= MASK_BOUNDARY_S;
        }
    }
}

void QuadContourGenerator::init_cache_levels(const double &lower_level, const double &upper_level) {
    assert(upper_level >= lower_level && "upper and lower levels are wrong way round");

    bool two_levels = (lower_level != upper_level);
    CacheItem keep_mask = (_corner_mask ? MASK_EXISTS | MASK_BOUNDARY_S | MASK_BOUNDARY_W
    ...
    if (two_levels) {
        ...
        for (long quad = 0; quad < _n; ++quad, ++z_ptr) {
            _cache[quad] &= keep_mask;
            if (*z_ptr > lower_level) _cache[quad] |= MASK_Z_LEVEL_1;
        }
    }
}

XY QuadContourGenerator::interp(long point1, long point2, const double &level) const {
...
void QuadContourGenerator::single_quad_filled(Contour &contour, long quad, const double &lower_level,
    ...
    if (BOUNDARY_S(quad) && EXISTS_S_EDGE(quad)) {
        ...
        if (!VISITED_S(quad) && Z_SW >= 1 && Z_SE == 0)
            contour.push_back(start_filled(quad, Edge_S, 1, NotHole, Interior, lower_level, upper_level));

        // Upper-level start from S boundary into interior.
        if (!VISITED_S(quad) && Z_SW < 2 && Z_SE == 2)
            contour.push_back(start_filled(quad, Edge_S, 2, NotHole, Interior, lower_level, upper_level));

        // Lower-level start following S boundary from W to E.
        if (!VISITED_S(quad) && Z_SW <= 1 && Z_SE == 1)
            contour.push_back(start_filled(quad, Edge_S, 1, NotHole, Boundary, lower_level, upper_level));

        // Upper-level start following S boundary from W to E.
        if (!VISITED_S(quad) && Z_SW == 2 && Z_SE == 1)
            contour.push_back(start_filled(quad, Edge_S, 2, NotHole, Boundary, lower_level, upper_level));
    }
    ...
    if (BOUNDARY_W(quad) && EXISTS_W_EDGE(quad)) {
        ...
        if (!VISITED_W(quad) && Z_NW >= 1 && Z_SW == 0)
            contour.push_back(start_filled(quad, Edge_W, 1, NotHole, Interior, lower_level, upper_level));

        // Upper-level start from W boundary into interior.
        if (!VISITED_W(quad) && Z_NW < 2 && Z_SW == 2)
            contour.push_back(start_filled(quad, Edge_W, 2, NotHole, Interior, lower_level, upper_level));

        // Lower-level start following W boundary from N to S.
        if (!VISITED_W(quad) && Z_NW <= 1 && Z_SW == 1)
            contour.push_back(start_filled(quad, Edge_W, 1, NotHole, Boundary, lower_level, upper_level));

        // Upper-level start following W boundary from N to S.
        if (!VISITED_W(quad) && Z_NW == 2 && Z_SW == 1)
            contour.push_back(start_filled(quad, Edge_W, 2, NotHole, Boundary, lower_level, upper_level));
    }
    ...
    if (EXISTS_SW_CORNER(quad)) {  // i.e. BOUNDARY_NE
        ...
        if (!VISITED_CORNER(quad) && Z_NW == 1 && Z_SE == 1)
            contour.push_back(start_filled(quad, Edge_NE, 1, Hole, Boundary, lower_level, upper_level));
    }
    // Possible starts from SE boundary.
        ...
        if (!VISITED(quad, 1) && Z_NW == 0 && Z_SW == 0 && Z_NE >= 1)
            contour.push_back(start_filled(quad, Edge_N, 1, NotHole, Interior, lower_level, upper_level));

        // Upper-level start from SE to N, hole.
        if (!VISITED(quad, 2) && Z_NW < 2 && Z_SW < 2 && Z_NE == 2)
            contour.push_back(start_filled(quad, Edge_SE, 2, Hole, Interior, lower_level, upper_level));

        // Upper-level start from N to SE.
        if (!VISITED(quad, 2) && Z_NW == 2 && Z_SW == 2 && Z_NE < 2)
            contour.push_back(start_filled(quad, Edge_N, 2, NotHole, Interior, lower_level, upper_level));

        // Lower-level start from SE to N, hole.
        if (!VISITED(quad, 1) && Z_NW >= 1 && Z_SW >= 1 && Z_NE == 0)
            contour.push_back(start_filled(quad, Edge_SE, 1, Hole, Interior, lower_level, upper_level));
    }
    // Possible starts from NW boundary.
        ...
        if (!VISITED(quad, 1) && Z_SW == 0 && Z_SE == 0 && Z_NE >= 1)
            contour.push_back(start_filled(quad, Edge_NW, 1, NotHole, Interior, lower_level, upper_level));

        // Upper-level start from E to NW, hole.
        if (!VISITED(quad, 2) && Z_SW < 2 && Z_SE < 2 && Z_NE == 2)
            contour.push_back(start_filled(quad, Edge_E, 2, Hole, Interior, lower_level, upper_level));

        // Upper-level start from NW to E.
        if (!VISITED(quad, 2) && Z_SW == 2 && Z_SE == 2 && Z_NE < 2)
            contour.push_back(start_filled(quad, Edge_NW, 2, NotHole, Interior, lower_level, upper_level));

        // Lower-level start from E to NW, hole.
        if (!VISITED(quad, 1) && Z_SW >= 1 && Z_SE >= 1 && Z_NE == 0)
            contour.push_back(start_filled(quad, Edge_E, 1, Hole, Interior, lower_level, upper_level));
    }
    // Possible starts from SW boundary.
        ...
        if (!VISITED_CORNER(quad) && Z_NW >= 1 && Z_SE == 0)
            contour.push_back(start_filled(quad, Edge_SW, 1, NotHole, Interior, lower_level, upper_level));

        // Upper-level start from SW boundary into interior.
        if (!VISITED_CORNER(quad) && Z_NW < 2 && Z_SE == 2)
            contour.push_back(start_filled(quad, Edge_SW, 2, NotHole, Interior, lower_level, upper_level));

        // Lower-level start following SW boundary from NW to SE.
        if (!VISITED_CORNER(quad) && Z_NW <= 1 && Z_SE == 1)
            contour.push_back(start_filled(quad, Edge_SW, 1, NotHole, Boundary, lower_level, upper_level));

        // Upper-level start following SW boundary from NW to SE.
        if (!VISITED_CORNER(quad) && Z_NW == 2 && Z_SE == 1)
            contour.push_back(start_filled(quad, Edge_SW, 2, NotHole, Boundary, lower_level, upper_level));
    }
    ...
    if (EXISTS_QUAD(quad) || EXISTS_NE_CORNER(quad)) {
        ...
        if (!VISITED(quad, 1) && Z_NW == 0 && Z_SE == 0 && Z_NE >= 1 && (!SADDLE(quad, 1) || SADDLE_LEFT(quad, 1)))
            contour.push_back(start_filled(quad, Edge_N, 1, NotHole, Interior, lower_level, upper_level));

        // Upper-level start from E to N, hole.
        if (!VISITED(quad, 2) && Z_NW < 2 && Z_SE < 2 && Z_NE == 2 && (!SADDLE(quad, 2) || !SADDLE_LEFT(quad, 2)))
            contour.push_back(start_filled(quad, Edge_E, 2, Hole, Interior, lower_level, upper_level));
        ...
        if (!VISITED(quad, 1) && Z_NW >= 1 && Z_SE >= 1 && Z_NE == 0 && (!SADDLE(quad, 1) || !SADDLE_LEFT(quad, 1)))
            contour.push_back(start_filled(quad, Edge_E, 1, Hole, Interior, lower_level, upper_level));
    ...
    if (BOUNDARY_N(quad) && EXISTS_N_EDGE(quad) && !VISITED_S(quad + _nx) && Z_NW == 1 && Z_NE == 1)
        contour.push_back(start_filled(quad, Edge_N, 1, Hole, Boundary, lower_level, upper_level));
}

ContourLine *QuadContourGenerator::start_filled(long quad, Edge edge, unsigned int start_level_index,
    ...
    while (true) {
        if (boundary_or_interior == Interior) {
            ...
            follow_interior(*contour_line, quad_edge, level_index, level, false, &start_quad_edge, start_level_index,
                            true);
        } else {
            level_index =
                follow_boundary(*contour_line, quad_edge, lower_level, upper_level, level_index, start_quad_edge);
        }

        if (quad_edge == start_quad_edge && (boundary_or_interior == Boundary || level_index == start_level_index))
            break;
...
bool QuadContourGenerator::start_line(vertices_list_type &vertices_list, long quad, Edge edge, const double &level) {
...
void QuadContourGenerator::write_cache(bool grid_only) const {
    std::cout << "-----------------------------------------------" << std::endl;
    for (long quad = 0; quad < _n; ++quad) write_cache_quad(quad, grid_only);
    std::cout << "-----------------------------------------------" << std::endl;
}
...
void QuadContourGenerator::write_cache_quad(long quad, bool grid_only) const {
    ...
    if (_corner_mask)
        std::cout << " CORNER=" << EXISTS_SW_CORNER(quad) << EXISTS_SE_CORNER(quad) << EXISTS_NW_CORNER(quad)
                  << EXISTS_NE_CORNER(quad);
    std::cout << " BNDY=" << (BOUNDARY_S(quad) > 0) << (BOUNDARY_W(quad) > 0);
    if (!grid_only) {
        std::cout << " Z=" << Z_LEVEL(quad) << " SAD=" << SADDLE(quad, 1) << SADDLE(quad, 2)
                  << " LEFT=" << SADDLE_LEFT(quad, 1) << SADDLE_LEFT(quad, 2) << " NW=" << SADDLE_START_SW(quad, 1)
                  << SADDLE_START_SW(quad, 2) << " VIS=" << VISITED(quad, 1) << VISITED(quad, 2) << VISITED_S(quad)
                  << VISITED_W(quad) << VISITED_CORNER(quad);
    }
    std::cout << std::endl;
}

std::tuple<double, std::pair<double, double>, std::pair<size_t, size_t>> find_closest_point_on_path(
...
contour_line_type contour_line(const vector_2d &X, const vector_2d &Y, const vector_2d &Z, double level) {
    ...
    // range). We then make sure this contour line matches the next contour line.
    double x_min = min(X);
    double x_max = max(X);
    double y_min = min(X);
    double y_max = max(X);
...
std::vector<contour_line_type> contourc(const vector_2d &x, const vector_2d &y, const vector_2d &z,
...
std::vector<contour_line_type> contourc(const vector_2d &x, const vector_2d &y, const vector_2d &z, size_t n_levels) {
    double zmin = z[0][0];
    double zmax = z[0][0];
...
std::vector<contour_line_type> contourc(const vector_2d &z, const vector_1d &levels) {
    auto [x, y] = meshgrid(iota(1, 1, z[0].size()), iota(1, 1, z.size()));
    return contourc(x, y, z, levels);
}

std::vector<contour_line_type> contourc(const vector_2d &z, size_t n_levels) {
    auto [x, y] = meshgrid(iota(1, 1, z[0].size()), iota(1, 1, z.size()));
