...
class contours : public axes_object {
   public:
    ...
    contours(class axes* parent, const vector_2d& X, const vector_2d& Y, const vector_2d& Z,
    ...
    template <class... Args>
    contours(const axes_handle& parent, Args... args) : contours(parent.get(), args...) {}
   ...
   protected:
    ...
    std::pair<vector_1d, vector_1d> fill_border_jump(double start_x, double start_y, double end_x, double end_y,
   ...
   public:
    ...
    /// Determine some automatic contour levels for data ranging from z_min to
    /// z_max Each library will use a different algorithm to determine levels. In
    /// our case, this is similar to the algorithm for automatic histogram edges.
    /// The difference is that the final histogram edges fall outside the range
    /// [z_min,z_max] while we want all contour lines to fall inside [z_min,z_max]
    ...
    ///        with line contours.
    static std::vector<double> determine_contour_levels(double z_min, double z_max, size_t n_levels_,
                                                        extend_option ext = extend_option::neither);
    static std::vector<double> determine_contour_levels(const vector_2d& Z, size_t n_levels_,
                                                        extend_option ext = extend_option::neither);
