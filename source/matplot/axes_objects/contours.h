...
class contours : public axes_object {
 ...
 protected:
  ...
  bool is_lower_level(size_t line_index, size_t segment_begin,
                      size_t segment_end);
  std::pair<vector_1d, vector_1d> fill_border_jump(
 ...
 public:
  ...
  ///        with line contours.
  static std::vector<double> determine_contour_levels(
      double z_min, double z_max, size_t n_levels_,
      extend_option ext = extend_option::neither);
  static std::vector<double> determine_contour_levels(
      const vector_2d& Z, size_t n_levels_,
      extend_option ext = extend_option::neither);
 ...
 protected:
  ...
  using parent_and_children_type =
      std::tuple<line_segment_type, std::vector<line_segment_type>, area_type>;
