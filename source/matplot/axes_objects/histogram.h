...
class histogram : public axes_object {
   public:
    ...
    enum class binning_algorithm {
        ...
        fd,         // Freedman-Diaconis rule - bin width = 2*IQR(X(:))*numel(X)^(-1/3)
                    // (for heavy tailed distributions, less sensitive to outliers) - IQR =
   ...
   public:
    ...
    histogram(class axes* parent, const std::vector<double>& data, size_t n_bins,
              enum histogram::normalization normalization_alg = histogram::normalization::count);
    histogram(class axes* parent, const std::vector<double>& data, const std::vector<double>& edges,
              enum histogram::normalization normalization_alg = histogram::normalization::count);
    histogram(class axes* parent, const std::vector<double>& data,
              binning_algorithm algorithm = binning_algorithm::automatic,
              histogram::normalization normalization_alg = histogram::normalization::count);

    /// If we receive an axes_handle, we can convert it to a raw
    /// pointer because there is no ownership involved here
    template <class... Args>
    histogram(const axes_handle& parent, Args... args) : histogram(parent.get(), args...) {}
   ...
   public /* useful functions for histograms */:
    ...
    static std::vector<double> histogram_edges(const std::vector<double>& data, double minx, double maxx,
                                               binning_algorithm algorithm = binning_algorithm::automatic,
    ...
    static std::vector<double> histogram_normalize(const std::vector<size_t>& bin_count,
                                                   const std::vector<double>& bin_edges, size_t data_size,
