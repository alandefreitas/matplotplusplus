...
namespace matplot {
    ...
    public:
        ...
        enum class binning_algorithm {
            automatic, // Scott's rule for doubles or integers rule for data distributed around integers
            scott,     // Scott’s rule - bin_width = 3.5*std(x)*|X|^(-1/3)
            fd,        // Freedman-Diaconis rule - bin width = 2*IQR(X(:))*numel(X)^(-1/3) (for heavy tailed distributions, less sensitive to outliers) - IQR = interquartile range of X
    ...
    public:
        ...
        histogram(class axes* parent, const std::vector<double>& data, size_t n_bins, enum histogram::normalization normalization_alg = histogram::normalization::count);
        histogram(class axes* parent, const std::vector<double>& data, const std::vector<double>& edges, enum histogram::normalization normalization_alg = histogram::normalization::count);
        histogram(class axes* parent, const std::vector<double>& data, binning_algorithm algorithm = binning_algorithm::automatic, histogram::normalization normalization_alg = histogram::normalization::count);
    ...
    public /* useful functions for histograms */:
        ...
        /// Find appropriate edges given a fixed number of bins and target bin width
        /// If nbins == 0, round the bin width and create (xmax-xmin)/bin_width bins
        /// If nbins != 0, bin width is set to (xmax-xmin)/nbins and create nbins bins
        static std::vector<double> bin_picker(double xmin, double xmax, size_t nbins, double bin_width);

        /// Find appropriate edges with a given algorithm
        static std::vector<double> histogram_edges(const std::vector<double>& data, double minx, double maxx, binning_algorithm algorithm = binning_algorithm::automatic, bool hard_limits = false);

        /// Count points within each pair of edges
        static std::vector<size_t> histogram_count(const std::vector<double>& values, const std::vector<double>& edges);

        /// Normalize the instagram count
        static std::vector<double> histogram_normalize(const std::vector<size_t>& bin_count, const std::vector<double>& bin_edges, size_t data_size, enum normalization normalization_algorithm);
    ...
    public /* getters and setters */:
        ...
        static std::vector<double> scotts_rule(const std::vector<double>& x, double minx, double maxx, bool hard_limits);
        static std::vector<double> fd_rule(const std::vector<double>& x, double minx, double maxx, bool hard_limits);
        static std::vector<double> integers_rule(const std::vector<double>& x, double minx, double maxx, bool hard_limits);
        static std::vector<double> sqrt_rule(const std::vector<double>& x, double minx, double maxx, bool hard_limits);
        static std::vector<double> sturges_rule(const std::vector<double>& x, double minx, double maxx, bool hard_limits);
        static std::vector<double> automatic_rule(const std::vector<double>& x, double minx, double maxx, bool hard_limits);
    ...
    protected:
        ...
        // target num of bins (0 indicates no target: the algorithm should decide)
        size_t num_bins_{0};
        ...
        enum binning_mode_type {
            ...
            use_fixed_edges,      // use fixed edges, including edges of variable size
        };
