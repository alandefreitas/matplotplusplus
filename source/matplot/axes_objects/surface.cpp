...
surface::surface(class axes* parent, const vector_2d& X, const vector_2d& Y, const vector_2d& Z, const vector_2d& C,
...
//    surface::surface(class xlim* parent, const vector_1d& x, const vector_1d&
//    y, const vector_1d& z, const vector_1d& c, const std::string& line_spec)
//            : axes_object(parent), X_data_({x}), Y_data_({y}), Z_data_({z}),
//            C_data_({c}), line_spec_(this, line_spec),
...
std::string surface::set_variables_string() {
    ...
    //            false) << "\n";
    //        } else {
    //            ss << "    set style line " << line_index << " linecolor rgb
    //            \"black\" dashtype 1 linewidth 0.5 linetype -1\n";
    //        }
    ...
    if (contour) {
        ...
        if (contour_text_) {
            ss << "    set style textbox opaque margins 0.5, 0.5 fc bgnd noborder "
                  "linewidth  1.0\n";
        ...
        for (size_t i = 0; i < n_contour_lines; ++i) {
            ...
            if (contour_line_spec_.user_color()) {
                ...
                if (contour_values_.empty()) {
                    ...
                    const bool not_using_palette = line_spec_.user_color() && !palette_map_at_bottom_ &&
                                                   !palette_map_at_top_ && !palette_map_at_surface_;
                    ...
                    ss << " linecolor rgb '"
                       << to_string(parent_->colormap_interpolation((zmax - zmin) - (contour_values_[i] - zmin), 0.,
                                                                    zmax - zmin))
                       << "'";
                }
            }
...
std::string surface::plot_string() {
    ...
    size_t n_plots = (waterfall_ || fences_) ? Z_data_.size() : ribbons_ ? Z_data_[0].size() : 1;
        ...
        if (is_solid_surface) {
            ...
            if (!fences_) {
                ...
                if (line_spec_.user_color()) {
                    ...
                    ss << " "
                       << " linecolor rgb \"black\" dashtype 1 linewidth 0.5 linetype -1";
                }
            } else {
                ss << " zerrorfill";
                if (!line_spec_.user_color()) {
    ...
    if (contour && contour_text_) {
        ...
        const bool only_contour =
            !surface_visible_ && !palette_map_at_bottom_ && !palette_map_at_surface_ && !palette_map_at_surface_;
...
std::string surface::grid_data_string() {
    ...
    auto send_point_fill = [](std::stringstream& ss, double x, double y, double z, double zlow, double zhigh,
                              double c) {
        ss << "    " << x;
        ss << "  " << y;
        ss << "  " << z;
    ...
    for (size_t data_replicate = 0; data_replicate < replicates; ++data_replicate) {
        if (curtain_) {
            ...
            send_point(ss, X_data_[i][Y_data_[i].size() - 1], Y_data_[i][Y_data_[i].size() - 1], zmin_,
        ...
        for (long i = Y_data_.size() - 1; i >= 0; --i) {
            ...
            for (size_t j = 0; j < Y_data_[i].size(); ++j) {
                if (!fences_) {
                    ...
                    send_point_fill(ss, X_data_[i][j], Y_data_[i][j], Z_data_[i][j], zmin_, Z_data_[i][j],
...
std::string surface::data_string() { return !ribbons_ ? grid_data_string() : ribbon_data_string(); }
...
const std::array<float, 4>& surface::edge_color() const { return line_spec().color(); }
...
bool surface::palette_map_at_bottom() const { return palette_map_at_bottom_; }
...
bool surface::palette_map_at_surface() const { return palette_map_at_surface_; }
...
const std::vector<double>& surface::contour_values() const { return contour_values_; }
...
const class line_spec& surface::contour_line_spec() const { return contour_line_spec_; }
