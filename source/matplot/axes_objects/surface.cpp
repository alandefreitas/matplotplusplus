...
std::string surface::set_variables_string() {
  ...
  const bool palette_map_3d =
      palette_map_at_bottom_ || palette_map_at_surface_ || palette_map_at_top_;
  if (palette_map_3d) {
    ss << "    set pm3d";
    ss << " noborder";
  ...
  if (use_gnuplots_grid3d) {
    ...
    if (!manual_color) {
      ...
      ss << "    set dgrid3d " << Y_data_.size() << "," << Y_data_[0].size()
         << " qnorm " << norm_ << "\n";
    }
  }
  ...
  if (contour) {
    ...
    size_t n_contour_lines =
        explicit_contour_levels ? contour_values_.size() : contour_levels_;

    // line types for contour
    auto [min_level_it, max_level_it] =
        std::minmax_element(contour_values_.begin(), contour_values_.end());
      ...
      if (contour_line_spec_.user_color()) {
        ...
        if (contour_values_.empty()) {
          ...
          const bool not_using_palette =
              line_spec_.user_color() && !palette_map_at_bottom_ &&
          ...
          if (using_palette) {
            ...
            ss << " linecolor rgb '"
               << to_string(
                      parent_->colormap_interpolation(i, 0., n_contour_lines))
               << "'";
          }
        } else {
          ...
          ss << " linecolor rgb '"
             << to_string(parent_->colormap_interpolation(
                    (zmax - zmin) - (contour_values_[i] - zmin), 0.,
                    zmax - zmin))
             << "'";
        }
      }
...
std::string surface::plot_string() {
      ...
      if (!fences_) {
        ...
        if (!line_spec_.user_color()) {
          ...
          if (C_data_.empty()) {
            ...
            c = parent_->colormap_interpolation(C_data_[0][i], *min_it,
                                                *max_it);
          }
  ...
  if (contour && contour_text_) {
    ss << ", '-' with labels ";
    const bool only_contour = !surface_visible_ && !palette_map_at_bottom_ &&
...
std::string surface::grid_data_string() {
  ...
  auto send_point = [](std::stringstream& ss, double x, double y, double z,
                       double c) {
    ss << "    " << x;
    ss << "  " << y;
    ss << "  " << z;
  ...
  auto send_point_fill = [](std::stringstream& ss, double x, double y, double z,
                            double zlow, double zhigh, double c) {
    ss << "    " << x;
    ss << "  " << y;
    ss << "  " << z;
    ss << "  " << zlow;
  ...
  for (size_t data_replicate = 0; data_replicate < replicates;
    ...
    if (curtain_) {
      ...
      send_point(ss, X_data_[i][Y_data_[i].size() - 1],
    ...
    for (long i = Y_data_.size() - 1; i >= 0; --i) {
      ...
      for (size_t j = 0; j < Y_data_[i].size(); ++j) {
        if (!fences_) {
          send_point(ss, X_data_[i][j], Y_data_[i][j], Z_data_[i][j],
                     color_value(data_replicate, i, j));
        } else {
          send_point_fill(ss, X_data_[i][j], Y_data_[i][j], Z_data_[i][j],
                          zmin_, Z_data_[i][j],
                          color_value(data_replicate, i, j));
        }
      }
...
std::string surface::ribbon_data_string() {
  ...
  for (size_t i = 0; i < n_cols; ++i) {
    ...
    for (size_t j = 0; j < n_rows; ++j) {
      send_point(ss, X_data_[j][i] - absolute_width / 2., Y_data_[j][i],
                 Z_data_[j][i], color_value(j, i));
      send_point(ss, X_data_[j][i] + absolute_width / 2., Y_data_[j][i],
                 Z_data_[j][i], color_value(j, i));
      ss << "\n";
    }
...
class surface& surface::contour_values(
...
class surface& surface::contour_line_spec(
