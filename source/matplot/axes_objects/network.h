...
class network : public axes_object {
   ...
   public:
    ...
    network(class axes* parent, const std::vector<std::pair<size_t, size_t>>& edges, const std::vector<double>& weights,
            size_t n_vertices, const std::string& line_spec = "");

    /// If we receive an axes_handle, we can convert it to a raw
    /// pointer because there is no ownership involved here
    template <class... Args>
    network(const axes_handle& parent, Args... args) : network(parent.get(), args...) {}
