...
namespace matplot {
    box_chart::box_chart(class axes* parent)
            : axes_object(parent) {}

    box_chart::box_chart(class axes* parent, const std::vector<double>& y_data, const std::vector<double>& groups)
    ...
    std::string box_chart::plot_string() {
        ...
        for (size_t i = 0; i < unique_groups.size(); ++i) {
            ...
            if (box_style_ == box_chart::box_style_option::filled) {
                res += " '-' with boxplot fillstyle solid border rgb '" + to_string(edge_color_) + "' fillcolor '" +
                       to_string(face_color_) + " linewidth " + num2str(edge_width_) + "' pointsize " +
                       num2str(whisker_size_);
            } else {
                res += " '-' with boxplot fillstyle solid 0.0 border rgb '" + to_string(edge_color_) + "' fillcolor '" +
                       to_string(face_color_) + " linewidth " + num2str(edge_width_) + "' pointsize " +
                       num2str(whisker_size_);
            }
        ...
        for (size_t i = 0; i < unique_groups.size(); ++i) {
            ...
            for (size_t j = 0; j < y_data_.size(); ++j) {
                ...
                if (unique_groups[i] == x_data_[j]) {
                    ss << "    " << unique_groups[i] << "  " << y_data_[j] << "  " << box_width_ << "\n";
                }
            }
            ss << "e\n";
        }
    ...
    bool box_chart::requires_colormap() {
        return false;
    }
    ...
    class box_chart& box_chart::box_style(box_chart::box_style_option box_style) {
    ...
    const std::vector<double> &box_chart::y_data() const {
        return y_data_;
    }
    ...
    const std::vector<double> &box_chart::x_data() const {
        return x_data_;
    }
    ...
    const color_array &box_chart::face_color() const {
        return face_color_;
    }
    ...
    bool box_chart::manual_face_color() const {
    ...
    const color_array &box_chart::edge_color() const {
        return edge_color_;
    }
    ...
    float box_chart::edge_width() const {
        return edge_width_;
    }
    ...
    class box_chart& box_chart::whisker_style(enum line_spec::marker_style whisker_style) {
        whisker_style_ = whisker_style;
        touch();
        return *this;
    }

    float box_chart::whisker_size() const {
        return whisker_size_;
    }
    ...
    bool box_chart::whisker_face() const {
        return whisker_face_;
    }
    ...
    bool box_chart::jitter_outliers() const {
        return jitter_outliers_;
    }
    ...
    float box_chart::box_width() const {
        return box_width_;
    }
    ...
    float box_chart::cap_size() const {
        return cap_size_;
    }
