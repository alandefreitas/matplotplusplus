...
namespace matplot {
contours::contours(class axes* parent, const vector_2d& X, const vector_2d& Y,
  ...
  contour_generator_ =
      QuadContourGenerator(X_data_, Y_data_, Z_data_, _corner_mask, nchunk_);
}
    ...
    if (plot_background) {
      ...
      if (parent_is_lower_level) {
        ...
        line_spec_.color(parent_->colormap_interpolation(
            background_z_level, contour_min_level, contour_max_level));
      } else {
      ...
      ls = std::regex_replace(ls, std::regex(" linecolor rgb +[^ ]+ "),
                              " linecolor palette ");
      ss << ls;
...
std::vector<double> contours::determine_contour_levels(double z_min,
  ...
  if (missing_zero || excess) {
    ...
    remove_levels_out_of_range(levels);
    missing_zero = zero_is_in_the_range &&
                   std::find(levels.begin(), levels.end(), 0.) == levels.end();
    if (missing_zero) {
...
bool contours::is_lower_level(size_t line_index, size_t segment_begin,
  ...
  auto it_y = std::find_if(Y_data_.begin(), Y_data_.end(),
...
std::pair<vector_1d, vector_1d> contours::fill_border_jump(
    double start_x, double start_y, double end_x, double end_y, double x_min,
    double x_max, double y_min, double y_max, bool is_parent) {
  ...
  if (filled_) {
    ...
    for (size_t i = 0; i < line_segments_.size(); ++i) {
      ...
      for (size_t j = begin_index; j < end_index; ++j) {
        ...
        if (is_one_before_last) {
          ...
          bool is_jump_to_border = (next_x <= _xmin) || (next_x >= _xmax) ||
          ...
          if (is_jump_to_border) {
            ...
            if (going_anticlockwise) {
              ...
              std::array<double, 4> border_distances = {
                  (_xmax - x), (x - _xmin), (_ymax - y), (y - _ymin)};
              auto it = std::min_element(border_distances.begin(),
              ...
              ss << "    " << x << "  " << y << "  " << segment_z_level << "\n";
              auto [xs, ys] = fill_border_jump(x, y, next_x, next_y, _xmin,
                                               _xmax, _ymin, _ymax, true);
      ...
      for (size_t j = 0; j < child_segments.size(); ++j) {
        ...
        for (size_t k = child_begin_index; k < child_end_index; ++k) {
          ...
          bool is_ll = is_lower_level(child_line_index, child_begin_index,
                                      child_end_index);
          double segment_z_level = is_ll ? lower_levels[child_line_index]
    ...
    if (z_has_nans) {
      ...
      for (size_t i = 0; i < nan_lines.size(); ++i) {
        ...
        double yend =
            end_line < Y_data_.size() - 1 ? Y_data_[end_line + 1][0] : _ymax;
        ss << "    " << _xmin << "  " << ybegin << "\n";
        ss << "    " << _xmin << "  " << yend << "\n";
        ss << "    " << _xmax << "  " << yend << "\n";
        ss << "    " << _xmax << "  " << ybegin << "\n";
      ...
      for (size_t i = 0; i < nan_columns.size(); ++i) {
        ...
        double xend =
            end_col < X_data_[0].size() - 1 ? X_data_[0][end_col + 1] : _xmax;
        ss << "    " << xbegin << "  " << _ymin << "\n";
        ss << "    " << xbegin << "  " << _ymax << "\n";
        ss << "    " << xend << "  " << _ymax << "\n";
        ss << "    " << xend << "  " << _ymin << "\n";
    ...
    while (point_index != n_points) {
      ...
      for (size_t i = 0; i < lines_.size(); ++i) {
        ...
        for (; point_index[i] < lines_[i].first.size(); ++point_index[i]) {
          ...
          for (size_t k = 0;
               !too_close_to_another_label && k < labels_xs.size(); ++k) {
            for (size_t l = 0;
                 !too_close_to_another_label && l < labels_xs[k].size(); ++l) {
          ...
          if (has_previous) {
            ...
            bool has_next = j != lines_[i].first.size() - 1 &&
                            isfinite(lines_[i].first[j + 1]);
            if (has_next) {
...
void square_trace(
  ...
  auto are_neighbors = [](const auto& a, const auto& b) {
    size_t distance_x =
        std::max(static_cast<size_t>(a.second), static_cast<size_t>(b.second)) -
        std::min(static_cast<size_t>(a.second), static_cast<size_t>(b.second));
    size_t distance_y =
        std::max(static_cast<size_t>(a.first), static_cast<size_t>(b.first)) -
  ...
  while (current_quadrant != start) {
    ...
    bool inside_boundaries =
        current_quadrant.first <= n_rows - 1 && current_quadrant.first >= 1 &&
...
void contours::process_all_segs_and_all_kinds() {
  ...
  if (filled_) {
    ...
    for (size_t i = 0; i < lowers.size(); ++i) {
      ...
      auto [vertices, kinds] =
          contour_generator_.create_filled_contour(level, level_upper);
...
class contours& contours::colormap_line_when_filled(
    bool colormap_line_when_filled) {
