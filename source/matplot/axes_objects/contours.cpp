...
namespace matplot {
contours::contours(class axes* parent, const vector_2d& X, const vector_2d& Y, const vector_2d& Z,
    ...
    if (filled_) {
        ...
        if (plot_background) {
            ...
            if (parent_is_lower_level) {
                ...
                line_spec_.color(
                    parent_->colormap_interpolation(background_z_level, contour_min_level, contour_max_level));
            } else {
            ...
            std::string ls =
                " '-' with filledcurve " + line_spec_.plot_string(line_spec::style_to_plot::plot_line_only, false);
        ...
        for (size_t i = 0; i < line_segments_.size(); ++i) {
            ...
            for (size_t j = 0; j < children_segments.size(); ++j) {
                ...
                line_spec_.color(
                    parent_->colormap_interpolation(segment_z_level, contour_min_level, contour_max_level));
                std::string ls =
                    " '-' with filledcurve " + line_spec_.plot_string(line_spec::style_to_plot::plot_line_only, false);
                ls = std::regex_replace(ls, std::regex(" linecolor rgb +[^ ]+ "), " linecolor palette ");
                ss << ls;
...
std::vector<double> contours::determine_contour_levels(double z_min, double z_max, size_t n_levels_,
                                                       extend_option ext) {
    ...
    std::vector<double> levels =
        histogram::bin_picker(z_min, z_max, target_n_edges, (z_max - z_min) / (target_n_edges));
...
std::string contours::legend_string(const std::string& title) {
    ...
    for (size_t i = 0; i < lines_.size(); ++i) {
        ...
        ss << " keyentry " << line_spec_.plot_string(line_spec::style_to_plot::plot_line_only) << " title \""
           << escape(title) << "\" ";
        if (!previous_color_manual) {
            line_spec_.color(previous_color);
            line_spec_.user_color(previous_color_manual);
...
bool contours::is_lower_level(size_t line_index, size_t segment_begin, size_t segment_end) {
    ...
    auto is_on_border = [&]() {
        return (x1 <= _xmin || x1 >= _xmax || x2 <= _xmin || x2 >= _xmax || y1 <= _ymin || y1 >= _ymax || y2 <= _ymin ||
                y2 >= _ymax);
    };
    ...
    auto it_y = std::find_if(Y_data_.begin(), Y_data_.end(), [&](const auto& y_row) { return y_row[0] > avg_y; });
    auto it_x = std::find_if(X_data_[0].begin(), X_data_[0].end(),
...
std::pair<vector_1d, vector_1d> contours::fill_border_jump(double start_x, double start_y, double end_x, double end_y,
...
std::string contours::data_string() {
            ...
            for (size_t j = begin_index; j < end_index; ++j) {
                ...
                if (is_one_before_last) {
                    ...
                    bool is_jump_to_border =
                        (next_x <= _xmin) || (next_x >= _xmax) || (next_y <= _ymin) || (next_y >= _ymax);
                    if (is_jump_to_border) {
                        ...
                        if (going_anticlockwise) {
                            ...
                            std::array<double, 4> border_distances = {(_xmax - x), (x - _xmin), (_ymax - y),
                                                                      (y - _ymin)};
    ...
    auto is_separator = [](double x, double y) { return !isfinite(x) || !isfinite(y); };
    ...
    for (size_t i = 0; i < lines_.size(); ++i) {
        ...
        for (size_t j = 0; j < lines_[i].first.size(); ++j) {
            if (!is_separator(lines_[i].first[j], lines_[i].second[j])) {
                ...
                // Include an empty line to indicate this polygon or line segment is
                // over
                ss << "  \n";
                continue;
            }
...
enum axes_object::axes_category contours::axes_category() { return axes_object::axes_category::two_dimensional; }
...
const std::array<float, 4>& contours::color() const { return line_spec().color(); }
...
void square_trace(size_t start_i, size_t start_j, const vector_2d& X, const vector_2d& Y, const vector_2d& Z,
                  double level,
                  std::unordered_set<std::pair<size_t, size_t>, pair_hash<size_t, size_t>>& quadrants_visited,
    ...
    auto are_neighbors = [](const auto& a, const auto& b) {
        size_t distance_x = std::max(static_cast<size_t>(a.second), static_cast<size_t>(b.second)) -
                            std::min(static_cast<size_t>(a.second), static_cast<size_t>(b.second));
        size_t distance_y = std::max(static_cast<size_t>(a.first), static_cast<size_t>(b.first)) -
                            std::min(static_cast<size_t>(a.first), static_cast<size_t>(b.first));
        return distance_x <= 1 && distance_y <= 1;
    };

    // Nan pair
    constexpr auto nan_pair =
        std::make_pair(std::numeric_limits<size_t>::quiet_NaN(), std::numeric_limits<size_t>::quiet_NaN());
    ...
    while (current_quadrant != start) {
        ...
        bool inside_boundaries = current_quadrant.first <= n_rows - 1 && current_quadrant.first >= 1 &&
                                 current_quadrant.second >= 0 && current_quadrant.second <= n_cols - 2;
        ...
        if (is_inflection_point) {
            if (!are_neighbors(current_quadrant, boundary_quadrants.back())) {
                ...
                // find last neighbor
                for (auto iter = boundary_quadrants.rbegin() + 1; iter != boundary_quadrants.rend(); ++iter) {
                    if (*iter == nan_pair) {
                        break;
                    }
...
bool contours::colormap_line_when_filled() const { return colormap_line_when_filled_; }
