...
stair::stair(class axes* parent, const std::vector<double>& x_data, const std::vector<double>& y_data,
...
std::vector<line_spec::style_to_plot> stair::styles_to_plot() {
    ...
    if (it != result.end()) {
        ...
        // we need to sort it to make sure line comes before marker and marker
        // before marker face
        ...
        if (style == line_spec::style_to_plot::plot_line_only) {
            ...
            switch (stair_style_) {
                ...
                case stair_style::fill:
                    res +=
                        " '-' with fillsteps fillstyle solid 0.25 fillcolor \"" + to_string(line_spec_.color()) + "\"";
                    break;
            }
        } else {
