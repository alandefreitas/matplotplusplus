...
std::string parallel_lines::set_variables_string() {
  ...
  if (parent_->color() != parent_->parent()->color()) {
    ...
    res += "    set object 2 rectangle from 1,0 to " + num2str(data_.size()) +
           ",1 behind fillcolor rgb \"" + to_string(parent_->color()) +
...
std::string parallel_lines::plot_string() {
  ...
  std::string res =
      " '-' " +
      line_spec_.plot_string(line_spec::style_to_plot::plot_line_only, true);
...
std::string parallel_lines::data_string() {
    ...
    if (axis_[i].tick_values_manual()) {
      ...
      for (size_t j = 0; j < ticks[i].ticks.size(); ++j) {
        if (ticks[i].tickLabels.size() <= j) {
          ticks[i].tickLabels.emplace_back(
              num2str(ticks[i].ticks[j], axis_[i].tick_label_format()));
        }
      }
    } else {
      ticks[i] =
          calcticks(X_min[i], X_max[i], false, parent_->font_size() * 0.125);
    }
  }
  ...
  for (size_t dimension = 0; dimension < data_.size(); ++dimension) {
    ...
    for (size_t i = 0; i < n_ticks + 1; ++i) {
      ...
      ss << "    "
         << dimension + 1 + (!is_last_dimension ? -tick_size : +tick_size)
         << "  " << y << "\n\n";
    }
  }
  ss << "e\n";
  ...
  for (size_t dimension = 0; dimension < data_.size(); ++dimension) {
    ...
    for (size_t i = 0; i < n_ticks; ++i) {
      ...
      ss << "    "
         << dimension + 1 +
                (!is_last_dimension ? -tick_size - 0.01 : +tick_size + 0.01)
         << "  " << y << "  " << std::quoted(ticks[dimension].tickLabels[i])
         << "\n\n";
    }
...
class parallel_lines& parallel_lines::line_spec(
    const class line_spec& line_spec) {
...
class parallel_lines& parallel_lines::data(
    const std::vector<std::vector<double>>& data) {
  data_ = data;
  touch();
  return *this;
...
class parallel_lines& parallel_lines::axis(
    const std::vector<struct axis>& axis) {
  axis_ = axis;
  touch();
  return *this;
}
...
class parallel_lines& parallel_lines::line_widths(
...
class parallel_lines& parallel_lines::line_colors(
    const std::vector<double>& line_colors) {
