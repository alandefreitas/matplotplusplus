...
namespace matplot {
    ...
    parallel_lines::parallel_lines(class axes* parent, const std::vector<std::vector<double>>& data, const std::string& line_spec)
    ...
    std::string parallel_lines::set_variables_string() {
        ...
        if (parent_->color() != parent_->parent()->color()) {
            ...
            res += "    set object 2 rectangle from 1,0 to " + num2str(data_.size()) + ",1 behind fillcolor rgb \"" + to_string(parent_->color()) + "\" fillstyle solid 1.0 noborder\n";
        }
        ...
        const bool main_x_axis_uses_default_format = parent_->x_axis().tick_label_format() == "%g" && parent_->x_axis().tick_values_automatic();
        if (main_x_axis_uses_default_format) {
            res += "    set xtics 1 format \"x%g\" scale 0,0\n";
        }
        // turn on axis tics for the parallel xlim (not anymore because we were forced to use line plots now)
//        for (size_t i = 0; i < axis_.size(); ++i) {
//            auto& ax = axis_[i];
//            std::string axis_name = "paxis " + num2str(i+1) + " ";
//            if (ax.visible() && !ax.tick_values().empty()) {
//                res += "    set " + axis_name + "tics textcolor \"" + to_string(ax.color()) + "\" font \"" + parent_->font() + "," + num2str(unsigned(parent_->font_size())) + "\" format \"" + escape(ax.tick_label_format()) + "\" nomirror in scale 0.75 \n";
//                if (ax.tick_values_manual()) {
//                    res += "    set " + axis_name + "tics " + ax.tick_values_string() + ax.tick_rotate_string() + "\n";
//                }
//            } else {
...
//        }
    ...
    std::string parallel_lines::data_string() {
        ...
        for (size_t dimension = 0; dimension < data_.size(); ++dimension) {
            ...
            for (size_t i = 0; i < n_ticks; ++i) {
                ...
                const bool is_last_dimension = dimension == data_.size() - 1;
                ss << "    " << dimension + 1 + (!is_last_dimension ? - tick_size - 0.01 : + tick_size + 0.01) << "  " << y << "  " << std::quoted(ticks[dimension].tickLabels[i]) <<  "\n\n";
            }
    ...
    std::string parallel_lines::unset_variables_string() {
        return "";
    }
    ...
    const line_spec &parallel_lines::line_spec() const {
        return line_spec_;
    }
    ...
    std::vector<struct axis> &parallel_lines::axis() {
        return axis_;
    }
    ...
    bool parallel_lines::visible() const {
        return visible_;
    }
    ...
    bool parallel_lines::jitter() const {
        return jitter_;
    }
