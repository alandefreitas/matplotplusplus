...
parallel_lines::parallel_lines(class axes* parent, const std::vector<std::vector<double>>& data,
...
std::string parallel_lines::set_variables_string() {
    ...
    if (parent_->color() != parent_->parent()->color()) {
        res += "    unset object 2\n";
        auto [min_x1, max_x1] = std::minmax_element(data_[0].begin(), data_[0].end());
        double r = *max_x1 - *min_x1;
        res += "    set object 2 rectangle from 1,0 to " + num2str(data_.size()) + ",1 behind fillcolor rgb \"" +
               to_string(parent_->color()) + "\" fillstyle solid 1.0 noborder\n";
    }
    ...
    const bool main_x_axis_uses_default_format =
        parent_->x_axis().tick_label_format() == "%g" && parent_->x_axis().tick_values_automatic();
    if (main_x_axis_uses_default_format) {
        res += "    set xtics 1 format \"x%g\" scale 0,0\n";
    }
    // turn on axis tics for the parallel xlim (not anymore because we were forced
    // to use line plots now)
    ...
    //                + num2str(unsigned(parent_->font_size())) + "\" format \"" +
    //                escape(ax.tick_label_format()) + "\" nomirror in scale 0.75
    //                \n"; if (ax.tick_values_manual()) {
...
std::string parallel_lines::plot_string() {
    ...
    std::string res = " '-' " + line_spec_.plot_string(line_spec::style_to_plot::plot_line_only, true);
    if (color_is_variable) {
        res = std::regex_replace(res, std::regex(" linecolor rgb +[^ ]+ "), " linecolor palette ");
    }
    ...
    // Have a look at: https://www.mathworks.com/help/matlab/ref/parallelplot.html
...
std::string parallel_lines::data_string() {
    ...
    for (size_t i = 0; i < data_.size(); ++i) {
        auto [min_it, max_it] = std::minmax_element(data_[i].begin(), data_[i].end());
        X_min.emplace_back(*min_it);
        X_max.emplace_back(*max_it);
        X_range.emplace_back(X_max.back() - X_min.back());
        expanded_X_min.emplace_back(*min_it - expand_factor * X_range.back());
        expanded_X_max.emplace_back(*max_it + expand_factor * X_range.back());
        expanded_X_range.emplace_back(expanded_X_max.back() - expanded_X_min.back());
    }

    auto jitter = [&](double y, size_t dimension) {
        double y2 = y + randn(0, (X_max[dimension] - X_min[dimension]) / 100);
        // reflect
    ...
    auto normalize = [&](double y, size_t dimension) {
        return (y - expanded_X_min[dimension]) / expanded_X_range[dimension];
    };
    ...
    for (size_t i = 0; i < data_[0].size(); ++i) {
        ...
        for (size_t dimension = 0; dimension < data_.size(); ++dimension) {
            ...
            if (!jitter_) {
                ...
                ss << normalize(jitter(data_[dimension][i], dimension), dimension) << "  ";
            }
            if (color_is_variable) {
                ss << line_colors_[i] << "  ";
            }
        ...
        if (axis_[i].tick_values_manual()) {
            ...
            for (size_t j = 0; j < ticks[i].ticks.size(); ++j) {
                if (ticks[i].tickLabels.size() <= j) {
                    ticks[i].tickLabels.emplace_back(num2str(ticks[i].ticks[j], axis_[i].tick_label_format()));
                }
            }
        } else {
            ticks[i] = calcticks(X_min[i], X_max[i], false, parent_->font_size() * 0.125);
        }
    }
    ...
    for (size_t dimension = 0; dimension < data_.size(); ++dimension) {
        ...
        for (size_t i = 0; i < n_ticks + 1; ++i) {
            ...
            ss << "    " << dimension + 1 + (!is_last_dimension ? -tick_size : +tick_size) << "  " << y << "\n\n";
        }
    }
    ss << "e\n";
    ...
    for (size_t dimension = 0; dimension < data_.size(); ++dimension) {
        ...
        for (size_t i = 0; i < n_ticks; ++i) {
            ...
            const bool is_last_dimension = dimension == data_.size() - 1;
            ss << "    " << dimension + 1 + (!is_last_dimension ? -tick_size - 0.01 : +tick_size + 0.01) << "  " << y
...
enum axes_object::axes_category parallel_lines::axes_category() { return axes_object::axes_category::two_dimensional; }

const line_spec& parallel_lines::line_spec() const { return line_spec_; }

class parallel_lines& parallel_lines::line_spec(const class line_spec& line_spec) {
    line_spec_ = line_spec;
    touch();
    return *this;
}

const std::vector<std::vector<double>>& parallel_lines::data() const { return data_; }

class parallel_lines& parallel_lines::data(const std::vector<std::vector<double>>& data) {
    data_ = data;
    touch();
    return *this;
}

const std::vector<struct axis>& parallel_lines::axis() const { return axis_; }

std::vector<struct axis>& parallel_lines::axis() { return axis_; }

class parallel_lines& parallel_lines::axis(const std::vector<struct axis>& axis) {
    axis_ = axis;
    touch();
    return *this;
}
...
const std::vector<float>& parallel_lines::line_widths() const { return line_widths_; }

class parallel_lines& parallel_lines::line_widths(const std::vector<float>& line_widths) {
    line_widths_ = line_widths;
    return *this;
}

const std::vector<double>& parallel_lines::line_colors() const { return line_colors_; }

class parallel_lines& parallel_lines::line_colors(const std::vector<double>& line_colors) {
