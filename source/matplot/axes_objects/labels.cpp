...
namespace matplot {
labels::labels(class axes* parent, const std::vector<double>& x, const std::vector<double>& y,
               const std::vector<std::string>& labels, const std::vector<double>& colors,
               const std::vector<double>& sizes)
    : axes_object(parent), x_(x), y_(y), labels_(labels), colors_(colors), sizes_(sizes) {}
...
std::string labels::set_variables_string() {
    ...
    if (rectangles_) {
        ...
        for (size_t i = 0; i < labels_.size(); ++i) {
            ...
            res += "    set object " + num2str(1000 + i) + " rectangle at " + num2str(x_[i]) + ", " + num2str(y_[i]) +
                   " size " + num2str(s * labels_[i].size() * width_factor) + ", " + num2str(s * height_factor) +
                   " fc lt 2 \n";
        }
    }
    return res;
...
enum axes_object::axes_category labels::axes_category() { return axes_object::axes_category::two_dimensional; }
...
const std::vector<std::string>& labels::label_values() const { return labels_; }
