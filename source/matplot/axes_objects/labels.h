...
class labels : public axes_object {
   ...
   public:
    ...
    labels(class axes* parent, const std::vector<double>& x, const std::vector<double>& y,
           const std::vector<std::string>& labels = {}, const std::vector<double>& color = {},
           const std::vector<double>& sizes = {});

    /// If we receive an axes_handle, we can convert it to a raw
    /// pointer because there is no ownership involved here
    template <class... Args>
    labels(const axes_handle& parent, Args... args) : labels(parent.get(), args...) {}
