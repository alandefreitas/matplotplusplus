...
filled_area::filled_area(class axes* parent, const std::vector<double>& x, const std::vector<double>& y,
                         const std::vector<double>& base_values, bool stacked, const std::string& line_spec)
    : line(parent, x, y, line_spec), stacked_(stacked), base_data_(base_values) {}
    ...
    if (!stacked_) {
        ...
        for (size_t i = 0; i < y_data_.size(); ++i) {
            if (isfinite(y_data_[i])) {
                double base_value = base_data_.empty() ? 0.0 : base_data_.size() == 1 ? base_data_[0] : base_data_[i];
        ...
        for (auto children_it = parent_->children().rbegin(); children_it != parent_->children().rend();
             ++children_it) {
...
const std::vector<double>& filled_area::base_data() const { return base_data_; }
