...
function_line::function_line(class axes* parent, const function_type& function,
...
class function_line& function_line::fn(
...
class function_line& function_line::t_range(
...
void function_line::make_sure_data_is_preprocessed() {
  if (fn_x_ && !fn_y_) {
    ...
    if (!mesh_is_ok || x_data_.empty() || x_data_.front() != t_range_[0] ||
        x_data_.back() != t_range_[1]) {
      x_data_ = linspace(t_range_[0], t_range_[1],
    ...
    if (!mesh_is_ok || t_data_.empty() || t_data_.front() != t_range_[0] ||
        t_data_.back() != t_range_[1]) {
      t_data_ = linspace(t_range_[0], t_range_[1],
                         automatic_mesh_density_ ? 500 : mesh_density_);
...
class function_line& function_line::automatic_mesh_density(
