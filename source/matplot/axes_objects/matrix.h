...
class matrix : public axes_object {
   ...
   public:
    ...
    matrix(class axes* parent, const std::vector<std::vector<double>>& red_channel,
           const std::vector<std::vector<double>>& green_channel, const std::vector<std::vector<double>>& blue_channel,
    ...
    /// Matrix with an rgb image
    matrix(class axes* parent, const image_channel_t& red_channel, const image_channel_t& green_channel,
    ...
    template <class... Args>
    matrix(const axes_handle& parent, Args... args) : matrix(parent.get(), args...) {}
   ...
   private:
    ...
    inline double x_width() { return (w_ - 1) / (matrices_[0][0].size() - 1); }
