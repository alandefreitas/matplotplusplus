...
std::string matrix::plot_string() {
  ...
  if (should_plot_labels()) {
    ...
    res += ", '-' with labels font \"" + escape(parent_->font()) + "," +
           num2str(round(parent_->font_size())) + "\" textcolor 'black'";
    // high values in white
    res += ", '-' with labels font \"" + escape(parent_->font()) + "," +
           num2str(round(parent_->font_size())) + "\" textcolor 'white'";
  }
  return res;
}
    ...
    for (size_t j = 0; j < matrix_[i].size(); ++j) {
      ...
      if (alpha_ == 0.) {
        ...
        color_array c = parent_->colormap_interpolation(
            z, use_cb_range ? cb_min : 0., use_cb_range ? cb_max : 255);
        ss << "  " << c[1] * 255 << "  " << c[2] * 255 << "  " << c[3] * 255
           << "  " << (1. - alpha_) * 255;
      }
      ss << "\n";
    }
  ...
  if (should_plot_labels()) {
    ...
    for (size_t i = 0; i < matrix_.size(); ++i) {
      for (size_t j = 0; j < matrix_[i].size(); ++j) {
        ...
        if (normalization_ == color_normalization::columns) {
          normalized_value =
              (normalized_value - value_min[j]) / (value_max[j] - value_min[j]);
          normalized_threshold = 0.7;
        } else if (normalization_ == color_normalization::rows) {
          normalized_value =
              (normalized_value - value_min[i]) / (value_max[i] - value_min[i]);
          normalized_threshold = 0.7;
        }
...
class matrix& matrix::normalization(
    matplot::matrix::color_normalization normalization) {
...
class matrix& matrix::matrix_g(
    const std::vector<std::vector<double>>& matrix_g) {
  if (matrices_.size() < 2) {
    matrices_.resize(2);
  }
