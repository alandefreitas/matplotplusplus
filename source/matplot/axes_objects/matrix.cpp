...
matrix::matrix(class axes* parent, const std::vector<std::vector<double>>& red_channel,
    ...
    : axes_object(parent),
      matrices_(alpha_channel.empty() ? std::vector{red_channel, green_channel, blue_channel}
...
matrix::matrix(class axes* parent, const image_channel_t& red_channel, const image_channel_t& green_channel,
               const image_channel_t& blue_channel, const image_channel_t& alpha_channel)
    : axes_object(parent),
...
std::string matrix::plot_string() {
    ...
    if (should_plot_labels()) {
        // low values in black
        res += ", '-' with labels font \"" + escape(parent_->font()) + "," + num2str(round(parent_->font_size())) +
               "\" textcolor 'black'";
    ...
    for (size_t i = 0; i < matrix_.size(); ++i) {
        for (size_t j = 0; j < matrix_[i].size(); ++j) {
            ...
            if (alpha_ == 0.) {
                ...
                color_array c =
                    parent_->colormap_interpolation(z, use_cb_range ? cb_min : 0., use_cb_range ? cb_max : 255);
...
std::string matrix::data_string() { return matrices_.size() > 1 ? image_data_string() : matrix_data_string(); }
...
enum axes_object::axes_category matrix::axes_category() { return axes_object::axes_category::two_dimensional; }

matplot::matrix::color_normalization matrix::normalization() const { return normalization_; }
...
bool matrix::has_alpha() const { return matrices_.size() == 4 || alpha_ != 0.; }

const std::vector<std::vector<double>>& matrix::matrix_r() const { return matrices_[0]; }
...
const std::vector<std::vector<double>>& matrix::matrix_g() const { return matrices_[1]; }
...
const std::vector<std::vector<double>>& matrix::matrix_b() const { return matrices_[2]; }
...
const std::vector<std::vector<double>>& matrix::matrix_a() const { return matrices_[3]; }
