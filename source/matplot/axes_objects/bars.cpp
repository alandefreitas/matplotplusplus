...
namespace matplot {
    ...
    bars::bars(class axes* parent, const std::vector<std::vector<double>>& Y)
        ...
        if (parent_->y_axis().limits_mode_auto()) {
            ...
            for (size_t i = 0; i < ys_.size(); ++i) {
                min_min = std::min(min_min, *std::min_element(ys_[i].begin(), ys_[i].end()));
            }
            if (min_min > 0) {
    ...
    bars::bars(class axes* parent, const std::vector<double>& x, const std::vector<double>& y)
            : bars(parent, x, std::vector<std::vector<double>>({y})) {}

    bars::bars(class axes* parent, const std::vector<double>& x, const std::vector<std::vector<double>>& Y)
        ...
        if (parent_->children().empty()) {
            ...
            parent_->x_axis().limits({x.front() - min_x_diff, x.back() + min_x_diff});
            if (ys_[0].size() <= 15) {
    ...
    std::string bars::plot_string() {
        ...
        std::string res;
        for (size_t i = 0; i < ys_.size(); ++i) {
            res += " '-' with boxes linewidth " + num2str(line_width_) + " fillstyle solid border rgb '" + to_string(edge_color_) + "'";
    ...
    std::string bars::legend_string(const std::string& title) {
        std::string res;
        for (size_t i = 0; i < ys_.size(); ++i) {
            res += " keyentry with boxes fillstyle solid border rgb '" + to_string(edge_color_) + "' fillcolor '" + to_string(face_colors_[i]) + "' title \"" + escape(title) + "\"";
            if (i != ys_.size() - 1) {
                res += ", ";
            }
        }
    ...
    std::string bars::data_string() {
        ...
        for (size_t bar_group = 0; bar_group < ys_.size(); ++bar_group) {
            for (size_t i = 0; i < ys_[bar_group].size(); ++i) {
                ...
                ss << "    " << x_end_point(bar_group, i) << " " << ys_[bar_group][i] << " " << m * cw << "\n";
            }
            ss << "e\n";
        }
        return ss.str();
    ...
    double bars::ymax() {
        ...
        for (size_t i = 0; i < ys_.size(); ++i) {
            max_max = std::max(max_max, *std::max_element(ys_[i].begin(), ys_[i].end()));
        }
        return max_max;
    }
    ...
    const color_array &bars::face_color() const {
        return face_colors_[0];
    }
    ...
    bool bars::manual_face_color() const {
    ...
    const color_array &bars::edge_color() const {
        return edge_color_;
    }
    ...
    const line_spec &bars::edge_style() const {
        return edge_style_;
    }
    ...
    bool bars::vertical_orientation() const {
    ...
    class bars& bars::vertical_orientation(bool vertical_orientation) {
        ...
        if (!vertical_orientation_) {
            std::cerr << "Cannot set horizontal orientation in gnuplot yet" << std::endl;
            vertical_orientation_ = true;
        }
        return *this;
    }

    bool bars::visible() const {
        return visible_;
    }
    ...
    std::vector<color_array> &bars::face_colors() {
        return face_colors_;
    }
