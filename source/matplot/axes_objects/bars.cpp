...
bars::bars(class axes* parent, const std::vector<double>& y) : bars(parent, std::vector<std::vector<double>>({y})) {}

bars::bars(class axes* parent, const std::vector<std::vector<double>>& Y) : axes_object(parent), ys_(Y) {
...
std::string bars::plot_string() {
    ...
    std::string res;
    for (size_t i = 0; i < ys_.size(); ++i) {
        res += " '-' with boxes linewidth " + num2str(line_width_) + " fillstyle solid border rgb '" +
...
std::string bars::legend_string(const std::string& title) {
    std::string res;
    for (size_t i = 0; i < ys_.size(); ++i) {
        res += " keyentry with boxes fillstyle solid border rgb '" + to_string(edge_color_) + "' fillcolor '" +
...
enum axes_object::axes_category bars::axes_category() { return axes_object::axes_category::two_dimensional; }
...
const std::vector<color_array>& bars::face_colors() const { return face_colors_; }
