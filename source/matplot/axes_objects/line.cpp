...
line::line(class axes* parent, const std::vector<double>& x_data, const std::vector<double>& y_data,
           const std::string& line_spec)
    : axes_object(parent), x_data_(x_data), y_data_(y_data), line_spec_(this, line_spec) {}
...
std::string line::data_string() {
    ...
    const bool can_plot_together =
        markers_are_automatic && line_spec_.can_plot_line_and_marker_together() && has_line_and_marker;
    ...
    for (const auto& style : styles_to_plot()) {
        if (visible_) {
            ...
            const double marker_size_denominator = style == line_spec::style_to_plot::plot_marker_face_only ? 10 : 6;
...
const std::vector<size_t>& line::marker_indices() const { return marker_indices_; }
...
enum line_spec::marker_style line::marker_style() const { return line_spec().marker(); }

enum line_spec::marker_style line::marker() const { return line_spec().marker(); }
...
const std::array<float, 4>& line::color() const { return line_spec().color(); }

const std::array<float, 4>& line::marker_color() const { return line_spec().marker_color(); }

const std::array<float, 4>& line::marker_face_color() const { return line_spec().marker_face_color(); }
