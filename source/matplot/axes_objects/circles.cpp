...
namespace matplot {
circles::circles(class axes* parent, const std::vector<double>& x, const std::vector<double>& y,
                 const std::vector<double>& radius, const std::vector<double>& start_angle,
...
std::string circles::legend_string(const std::string& title) {
    return " keyentry with circles linecolor var lw 4 fillstyle solid border "
           "linecolor 'black' title \"" +
           escape(title) + "\"";
}
...
std::string circles::data_string() {
    ...
    for (size_t i = 0; i < x_.size(); ++i) {
        ...
        if (color_.empty()) {
            ss << "    " << x_[i] << " " << y_[i] << " " << value_or_default(radius_, i, 1) << " "
               << value_or_default(start_angle_, i, 0) << " " << value_or_default(end_angle_, i, 360) << "\n";
        } else {
            ss << "    " << x_[i] << " " << y_[i] << " " << value_or_default(radius_, i, 1) << " "
               << value_or_default(start_angle_, i, 0) << " " << value_or_default(end_angle_, i, 360) << " "
...
enum axes_object::axes_category circles::axes_category() { return axes_object::axes_category::two_dimensional; }
...
const std::vector<double>& circles::start_angle() const { return start_angle_; }
