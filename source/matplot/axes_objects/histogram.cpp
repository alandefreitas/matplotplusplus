...
std::string histogram::legend_string(const std::string& title) {
  return " keyentry with boxes fillstyle solid border rgb '" +
         to_string(edge_color_) + "' fillcolor '" + to_string(face_color_) +
         "' title \"" + escape(title) + "\"";
}
    ...
    if (!stairs_only_) {
      for (size_t i = 0; i < values_.size(); ++i) {
        ...
        auto arc_between_edges =
            linspace(bin_edges_[i], bin_edges_[i + 1], ceil(points_per_bin));
  ...
  if (!data_is_ok) {
    switch (binning_mode_) {
      ...
      case binning_mode_type::use_bin_limits: {
        bin_edges_ = histogram_edges(data_, bin_limits_min_, bin_limits_max_,
                                     algorithm_, true);
        break;
      }
...
std::vector<double> histogram::bin_picker(double xmin, double xmax,
  ...
  if (data_is_not_constant) {
    ...
    if (nbins == 0) {
      ...
      left_edge = std::max(std::min(bin_width * floor(xmin / bin_width), xmin),
                           -std::numeric_limits<double>::max());
      n_bins_actual = std::max(1., ceil((xmax - left_edge) / bin_width));
      right_edge =
          std::min(std::max(left_edge + n_bins_actual * bin_width, xmax),
                   std::numeric_limits<double>::max());
    } else {
    ...
    double bin_range =
        std::max(1., ceil(nbins * nextafter(xscale, xscale + 1) - xscale));
  ...
  if (!isfinite(bin_width)) {
    ...
    auto middle_edges = transform(iota(1, n_bins_actual - 1), [&](double x) {
      return left_edge + x * bin_width;
    });
...
std::vector<double> bin_pickerbl(double xmin, double xmax, double minlimit,
  ...
  if (non_constant_data) {
    size_t nbins =
        std::max(static_cast<size_t>(((maxlimit - minlimit) / bin_width) + 0.5),
                 static_cast<size_t>(1));
...
std::vector<double> histogram::automatic_rule(const std::vector<double>& x,
                                              ...
                                              bool hard_limits) {
  double xrange = maxx - minx;
  bool is_around_integers = std::all_of(
      x.begin(), x.end(), [](double x) { return abs(x - round(x)) < 0.01; });
...
std::vector<double> histogram::histogram_edges(const std::vector<double>& data,
                                               double minx, double maxx,
                                               binning_algorithm algorithm,
                                               bool hard_limits) {
  switch (algorithm) {
...
std::vector<size_t> histogram::histogram_count(
...
std::vector<double> histogram::histogram_normalize(
    const std::vector<size_t>& bin_count, const std::vector<double>& bin_edges,
  ...
  switch (normalization_algorithm) {
    ...
    case normalization::pdf:
      for (size_t i = 0; i < bin_count.size(); ++i) {
        const double bin_width_i = bin_edges[i + 1] - bin_edges[i];
        values[i] =
            static_cast<double>(bin_count[i]) / (bin_width_i * data_size);
      }
      break;
...
class histogram& histogram::algorithm(
    matplot::histogram::binning_algorithm algorithm) {
  algorithm_ = algorithm;
  touch();
  return *this;
}
...
class histogram& histogram::normalization(
    enum histogram::normalization normalization) {
