...
histogram::histogram(class axes* parent, const std::vector<double>& data, size_t n_bins,
...
histogram::histogram(class axes* parent, const std::vector<double>& data, const std::vector<double>& edges,
                     enum histogram::normalization normalization_alg)
    : axes_object(parent),
      data_(data),
      bin_edges_(edges),
...
histogram::histogram(class axes* parent, const std::vector<double>& data, binning_algorithm algorithm,
                     enum histogram::normalization normalization_alg)
    : axes_object(parent),
      data_(data),
...
std::string histogram::legend_string(const std::string& title) {
    return " keyentry with boxes fillstyle solid border rgb '" + to_string(edge_color_) + "' fillcolor '" +
...
std::string histogram::data_string() {
    ...
    if (!is_polar()) {
        for (size_t i = 0; i < values_.size(); ++i) {
            ...
            ss << "    " << (bin_edges_[i] + bin_edges_[i + 1]) * 0.5 << " " << values_[i] << " "
    ...
    if (!data_is_ok) {
        switch (binning_mode_) {
            ...
            case binning_mode_type::use_fixed_bin_width: {
                ...
                size_t nbins =
                    std::max(static_cast<size_t>(1), static_cast<size_t>(ceil((maxx - left_edge) / bin_width_)));
...
std::vector<double> histogram::bin_picker(double xmin, double xmax, size_t nbins, double bin_width) {
    ...
    const bool data_is_not_constant =
        xrange > std::max(sqrt(nextafter(xscale, xscale + 1) - xscale), std::numeric_limits<double>::min());
    if (data_is_not_constant) {
        ...
        if (nbins == 0) {
            ...
            left_edge =
                std::max(std::min(bin_width * floor(xmin / bin_width), xmin), -std::numeric_limits<double>::max());
            n_bins_actual = std::max(1., ceil((xmax - left_edge) / bin_width));
            right_edge =
                std::min(std::max(left_edge + n_bins_actual * bin_width, xmax), std::numeric_limits<double>::max());
        } else {
...
std::vector<double> histogram::automatic_rule(const std::vector<double>& x, double minx, double maxx,
                                              bool hard_limits) {
    double xrange = maxx - minx;
    bool is_around_integers = std::all_of(x.begin(), x.end(), [](double x) { return abs(x - round(x)) < 0.01; });
...
std::vector<double> histogram::histogram_edges(const std::vector<double>& data, double minx, double maxx,
                                               binning_algorithm algorithm, bool hard_limits) {
    switch (algorithm) {
...
std::vector<double> histogram::histogram_normalize(const std::vector<size_t>& bin_count,
...
const std::vector<size_t>& histogram::bin_counts() const { return bin_counts_; }
...
matplot::histogram::binning_algorithm histogram::algorithm() const { return algorithm_; }
...
enum histogram::normalization histogram::normalization() const { return normalization_; }
...
bool histogram::vertical_orientation() const { return vertical_orientation_; }
