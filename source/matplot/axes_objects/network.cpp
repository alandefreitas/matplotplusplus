...
network::network(class axes* parent, const std::vector<std::pair<size_t, size_t>>& edges, const vector_1d& weights,
                 size_t n_vertices, const std::string& line_spec)
    : axes_object(parent), edges_(edges), weights_(weights), n_vertices_(n_vertices), line_spec_(this, line_spec) {
    line_spec_.marker_face(true);
}

std::vector<line_spec::style_to_plot> network::styles_to_plot() {
    return {line_spec::style_to_plot::plot_line_only, line_spec::style_to_plot::plot_marker_only};
}
...
std::string network::plot_string() {
    ...
    for (const auto& style : styles_to_plot()) {
        ...
        const bool we_are_plotting_line = style == line_spec::style_to_plot::plot_line_only;
        const bool line_width_is_variable = !line_widths_.empty() && z_data_.empty();
        ...
        if (we_are_plotting_line && directed_) {
            str += " '-' with vectors " + line_spec_.plot_string(line_spec::style_to_plot::plot_line_only, false);
        } else if (we_are_plotting_line && line_width_is_variable) {
            str += " '-' with filledcurves " + line_spec_.plot_string(line_spec::style_to_plot::plot_line_only, false);
        } else {
...
std::string network::legend_string(const std::string& title) {
    if (line_spec_.has_line() && line_spec_.has_non_custom_marker()) {
        return " keyentry " + line_spec_.plot_string(line_spec::style_to_plot::plot_line_and_marker) + " title \"" +
               escape(title) + "\"";
    } else if (line_spec_.has_line()) {
        return " keyentry " + line_spec_.plot_string(line_spec::style_to_plot::plot_line_only) + " title \"" +
               escape(title) + "\"";
    } else {
        return " keyentry " + line_spec_.plot_string(line_spec::style_to_plot::plot_marker_only) + " title \"" +
               escape(title) + "\"";
    }
}
    ...
    if (!directed_) {
        ...
        for (size_t i = 0; i < edges_.size(); ++i) {
            ...
            if (!line_width_is_variable) {
                ...
                ss << "  " << y_data_[edges_[i].first] - line_widths_[i] / 2.;
                ss << "  " << y_data_[edges_[i].first] + line_widths_[i] / 2.;
            }
            if (plot_z_data) {
            ...
            if (!line_width_is_variable) {
                ...
                ss << "  " << y_data_[edges_[i].second] - line_widths_[i] / 2.;
                ss << "  " << y_data_[edges_[i].second] + line_widths_[i] / 2.;
            }
            if (plot_z_data) {
        ...
        for (size_t i = 0; i < edges_.size(); ++i) {
            ...
            if (!plot_z_data) {
                ...
                double w1 = zmean - z1;
                double w2 = z2 - zmean;
                ss << "    " << x1 << "  " << y1 << "  " << z1 << "  " << u1 << "  " << v1 << "  " << w1 << "\n";
                ss << "    " << xmean << "  " << ymean << "  " << zmean << "  " << u2 << "  " << v2 << "  " << w2
                   << "\n";
            }
        }
        ss << "e\n";
    ...
    if (show_labels_ || !edge_labels_.empty()) {
        ...
        if (!edge_labels_.empty()) {
            for (size_t i = 0; i < edge_labels_.size(); ++i) {
                ss << "    " << 0.5 * (x_data_[edges_[i].first] + x_data_[edges_[i].second]);
                ss << "  " << 0.5 * (y_data_[edges_[i].first] + y_data_[edges_[i].second]);
                if (plot_z_data) {
                    ss << "  " << 0.5 * (z_data_[edges_[i].first] + z_data_[edges_[i].second]);
                }
...
void network::process_kawai_layout() {
    ...
    std::vector<nodesoup::Point2D> positions = nodesoup::kamada_kawai(g, width, height, k, energy_threshold_);
    for (size_t i = 0; i < positions.size(); ++i) {
        x_data_.emplace_back(positions[i].x);
        y_data_.emplace_back(positions[i].y);
    }
...
double network::xmax() {
    ...
    auto [xmin_it, xmax_it] = std::minmax_element(x_data_.begin(), x_data_.end());
...
double network::ymax() {
    ...
    auto [ymin_it, ymax_it] = std::minmax_element(y_data_.begin(), y_data_.end());
...
enum line_spec::marker_style network::marker_style() const { return line_spec().marker(); }

enum line_spec::marker_style network::marker() const { return line_spec().marker(); }
...
class network& network::marker_size(const std::vector<double>& size_vector) {
...
const std::array<float, 4>& network::color() const { return line_spec().color(); }

const std::array<float, 4>& network::marker_color() const { return line_spec().marker_color(); }

const std::array<float, 4>& network::marker_face_color() const { return line_spec().marker_face_color(); }
...
const std::vector<std::string>& network::edge_labels() const { return edge_labels_; }

class network& network::edge_labels(const std::vector<std::string>& edge_labels) {
    edge_labels_ = edge_labels;
    touch();
    return *this;
}

const std::vector<std::string>& network::node_labels() const { return node_labels_; }

class network& network::node_labels(const std::vector<std::string>& node_labels) {
    node_labels_ = node_labels;
    touch();
    return *this;
}

const std::vector<double>& network::line_widths() const { return line_widths_; }

class network& network::line_widths(const std::vector<double>& line_widths) {
    line_widths_ = line_widths;
    touch();
    return *this;
}

const std::vector<std::pair<size_t, size_t>>& network::edges() const { return edges_; }

class network& network::edges(const std::vector<std::pair<size_t, size_t>>& edges) {
    edges_ = edges;
    touch();
    return *this;
}
...
matplot::network::layout network::layout_algorithm() const { return layout_algorithm_; }

class network& network::layout_algorithm(matplot::network::layout layout_algorithm) {
...
const std::vector<float>& network::marker_sizes() const { return marker_sizes_; }

class network& network::marker_sizes(const std::vector<float>& marker_sizes) {
    marker_sizes_ = marker_sizes;
    touch();
    return *this;
}

const std::vector<double>& network::marker_colors() const { return marker_colors_; }
