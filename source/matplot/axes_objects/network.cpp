...
std::string network::plot_string() {
  ...
  for (const auto& style : styles_to_plot()) {
    ...
    if (marker_size_is_variable && we_are_plotting_marker) {
      str = std::regex_replace(str, std::regex(" pointsize \\d*\\.?\\d* "),
                               " pointsize variable ");
    }
    if (color_is_variable && we_are_plotting_marker) {
      str = std::regex_replace(str, std::regex(" linecolor rgb +[^ ]+ "),
                               " linecolor palette ");
    }
    if (use_y2_) {
      str += " xlim x1y2";
...
std::string network::legend_string(const std::string& title) {
  if (line_spec_.has_line() && line_spec_.has_non_custom_marker()) {
    ...
    return " keyentry " +
           line_spec_.plot_string(line_spec::style_to_plot::plot_marker_only) +
           " title \"" + escape(title) + "\"";
  }
}
  ...
  if (show_labels_ || !edge_labels_.empty()) {
    ...
    if (!edge_labels_.empty()) {
      for (size_t i = 0; i < edge_labels_.size(); ++i) {
        ss << "    "
           << 0.5 * (x_data_[edges_[i].first] + x_data_[edges_[i].second]);
        ss << "  "
           << 0.5 * (y_data_[edges_[i].first] + y_data_[edges_[i].second]);
        if (plot_z_data) {
          ss << "  "
             << 0.5 * (z_data_[edges_[i].first] + z_data_[edges_[i].second]);
        }
...
void network::maybe_update_line_spec() {
  if (line_spec_.has_line() && !line_spec_.user_color()) {
    ...
    line_spec_.color(c);
  } else if (line_spec_.has_non_custom_marker() && !line_spec_.user_color() &&
...
class network& network::edge_labels(
...
class network& network::node_labels(
...
class network& network::layout_algorithm(
    matplot::network::layout layout_algorithm) {
