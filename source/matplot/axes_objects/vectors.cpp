...
vectors::vectors(class axes* parent, const std::vector<double>& u_data, const std::vector<double>& v_data,
                 const std::string& line_spec)
    : axes_object(parent), u_data_(u_data), v_data_(v_data), line_spec_(this, line_spec) {}
...
vectors::vectors(class axes* parent, const std::vector<double>& x_data, const std::vector<double>& y_data,
                 const std::vector<double>& u_data, const std::vector<double>& v_data, const std::string& line_spec)
    : axes_object(parent),
      x_data_(x_data),
      y_data_(y_data),
      u_data_(u_data),
...
vectors::vectors(class axes* parent, const std::vector<double>& x_data, const std::vector<double>& y_data,
                 const std::vector<double>& z_data, const std::vector<double>& u_data,
                 const std::vector<double>& v_data, const std::vector<double>& w_data, const std::string& line_spec)
    : axes_object(parent),
      x_data_(x_data),
      y_data_(y_data),
      z_data_(z_data),
...
std::string vectors::legend_string(const std::string& title) {
    return " keyentry with vectors " + line_spec_.plot_string(line_spec::style_to_plot::plot_line_only, false) +
           " title \"" + escape(title) + "\"";
}
...
std::string vectors::data_string() {
    ...
    if (visible_) {
        for (size_t i = 0; i < v_data_.size(); ++i) {
            ...
            bool is_end_of_series = !isfinite(x_value) || !isfinite(y_value) || !isfinite(z_value) ||
                                    !isfinite(u_value) || !isfinite(v_value) || !isfinite(w_value);
            if (is_end_of_series) {
                ss << "    \n";
                continue;
            }
...
const std::vector<size_t>& vectors::marker_indices() const { return marker_indices_; }
...
enum line_spec::marker_style vectors::marker_style() const { return line_spec().marker(); }

enum line_spec::marker_style vectors::marker() const { return line_spec().marker(); }
...
const std::array<float, 4>& vectors::color() const { return line_spec().color(); }

const std::array<float, 4>& vectors::marker_color() const { return line_spec().marker_color(); }

const std::array<float, 4>& vectors::marker_face_color() const { return line_spec().marker_face_color(); }
