...
std::string vectors::data_string() {
  ...
  if (visible_) {
    for (size_t i = 0; i < v_data_.size(); ++i) {
      double x_value = x_data_.size() > i
                           ? x_data_[i]
                           : is_polar() ? 0 : static_cast<double>(i + 1);
      ...
      bool is_end_of_series = !isfinite(x_value) || !isfinite(y_value) ||
                              !isfinite(z_value) || !isfinite(u_value) ||
                              !isfinite(v_value) || !isfinite(w_value);
      if (is_end_of_series) {
        ss << "    \n";
        continue;
      }
...
class vectors& vectors::marker_indices(
    const std::vector<size_t>& marker_indices) {
