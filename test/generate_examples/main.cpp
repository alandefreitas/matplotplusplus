...
int main() {
    ...
    map<string, string> dir_map = {{"line_plot", "Line Plots"},
    ...
    while (!path_queue.empty()) {
        ...
        if (is_source_file) {
            ...
            bool dont_have_the_large_image_yet =
                !fs::exists(figure_destination_svg) && !fs::exists(figure_destination_png);
            if (dont_have_the_large_image_yet) {
                ...
                if (fs::exists(figure_origin_svg)) {
                    ...
                    cout << "Could not find " << figure_origin_svg.filename() << " for " << executable.filename()
                         << endl;
                }
            }
