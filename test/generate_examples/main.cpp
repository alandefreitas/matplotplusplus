...
int main() {
  ...
  // This will create .svg and .gif files with all the examples for the
  // documentation Some examples, such as heatmaps and some contours, are better
  // generated
  ...
  // - Gallery with one large and rest of thumbnails for examples linking to
  // source files
  ...
  auto queue_paths = [&]() {
    ...
    // Enqueue all executable source files in the order defined by
    // CMakeLists.txt
    vector<fs::path> cpps = {};
  ...
  while (!path_queue.empty()) {
    ...
    // If it's a source file, we will include its respective image in the
    // galleries
    ...
    if (is_source_file) {
      ...
      executable /= "example_";
      executable += p.stem();
      auto executable_filename =
          executable.lexically_proximate(root_path).filename();
      cout << executable_filename << " -> ";

      // Calculate figure destinations
      // Destination directory in documentation
      fs::path figure_destination_dir =
          documentation_path / p.lexically_proximate(root_path);
      ...
      const auto svg_filename =
          figure_destination_svg.lexically_proximate(root_path).filename();
      cout << svg_filename << '\n';
      ...
      if (fs::exists(maybe_svg_duplicate)) {
        std::cout << "Found duplicate! File " << maybe_svg_duplicate.string()
                  << std::endl;
      ...
      const auto update_gallery = [&]() {
        ...
        std::string figure_file =
            final_figure_destination.lexically_proximate(documentation_path)
                .string();
        std::string source_file =
            p.lexically_relative(documentation_path).string();
        gallery << "[![" << exe_file << "](" << figure_file << ")]("
                << source_file << ")\n\n";

        // Append image with link to anchor to the thumbnail gallery
        auto thumb_file =
            figure_destination_thumbnail_png.lexically_proximate(root_path)
                .string();
        auto gallery_file = gallery_path.lexically_relative(root_path).string();
        thumbnail_gallery << "[![" << exe_file << "](" << thumb_file << ")]("
                          << gallery_file << "#" << anchor << ")  ";

        // Append image to the examples gallery
        const bool first_image_in_dir =
            p.parent_path() != last_example_parent_dir;
        auto source_file_2 = p.lexically_relative(root_path).string();
        if (first_image_in_dir) {
          // First image is large
          const auto large_figure_figure =
              final_figure_destination.lexically_proximate(root_path).string();
          examples_gallery << "[![" << exe_file << "](" << large_figure_figure
                           << ")](" << source_file_2 << ")\n\n";
          ...
          examples_gallery << "[![" << exe_file << "](" << thumb_file << ")]("
                           << source_file_2 << ")  ";
        }
      };

      bool dont_have_the_large_image_yet =
          !fs::exists(figure_destination_svg) &&
          !fs::exists(figure_destination_png);
      ...
      if (fs::exists(figure_destination_svg)) {
        ...
        string cmd = "rsvg-convert \"" + figure_destination_svg.string() +
                     "\" > \"" + figure_destination_png.string() + "\"";
        system(cmd.c_str());
        ...
        cmd = "rsvg-convert -h 100 \"" + figure_destination_svg.string() +
              "\" > \"" + figure_destination_thumbnail_png.string() + "\"";
        system(cmd.c_str());
      ...
      const auto thumbnail_exists =
          fs::exists(figure_destination_thumbnail_png);
      if (thumbnail_exists) {
      ...
      const auto update_gallery = [&]() {
        ...
        if (fs::exists(maybe_gif)) {
          std::cout << "Found duplicate gif! File " << maybe_gif.string()
                    << std::endl;
          fs::remove(maybe_gif);
        }
        ...
        if (it == dir_map.end()) {
          std::cerr << "Could not find a header for " << header_title
                    << std::endl;
        } else {
